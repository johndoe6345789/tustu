...
public class HttpFileAccess implements RemoteFileAccess {
  ...
  public File readRemoteFile(File paramFile, RemoteFileDescriptor paramRemoteFileDescriptor) {
    ...
    if (paramRemoteFileDescriptor.getName() == null || paramRemoteFileDescriptor.getName().isEmpty())
    ...
    if (this.fileAccessPreferences != null && this.fileAccessPreferences.getFileExistsPreference() != 1)
      if (this.fileAccessPreferences.getFileExistsPreference() == 0) {
        ...
        D.c("Preference to return local, but local file and remote file are different sizes, going to overwrite.");
      } else {
        while (file.exists())
    ...
    try {
      ...
      throw new RemoteAccessException("Failed to download " + paramRemoteFileDescriptor.getName() + ", Error: " + iOException.getLocalizedMessage());
    } 
    return file;
  }
  
  public boolean deleteFile(RemoteFileDescriptor paramRemoteFileDescriptor) {
    String str = getCoreUrl(URLEncoder.encode(paramRemoteFileDescriptor.getDirectory().getDirectoryId()));
    if (paramRemoteFileDescriptor.getName() == null || paramRemoteFileDescriptor.getName().isEmpty())
      throw new RemoteAccessException("fileName is required!"); 
    ...
    try {
      ...
      throw new RemoteAccessException("Delete file " + paramRemoteFileDescriptor.getName() + " failed, Error: " + str1);
    } catch (IOException iOException) {
      Logger.getLogger(HttpFileAccess.class.getName()).log(Level.WARNING, "Unable to delete file", iOException);
      throw new RemoteAccessException("Unable to delete file " + paramRemoteFileDescriptor.getName() + ", Error: " + iOException.getLocalizedMessage());
    } 
  }
  ...
  public DirectoryInformation getDirectoryInformation(DirectoryIdentifier paramDirectoryIdentifier) {
    String str2;
    String str1 = getCoreUrl((paramDirectoryIdentifier == null) ? null : paramDirectoryIdentifier.getDirectoryId());
