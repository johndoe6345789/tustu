...
public class aU extends aS {
    ...
    if (this.j != null) {
      ...
      for (aX aX1 : this.j) {
        if (aX1.b() > 0.0D) {
          ...
          paramGraphics.drawString("Gap: " + X.b(aX1.a(), 3) + " to " + X.b(aX1.c(), 3), i + paramGraphics.getFont().getSize(), rectangle.y + paramGraphics.getFont().getSize() * 3);
        } 
      } 
    } 
    graphics2D.setStroke(this.b);
    ...
    for (byte b = 0; b < this.a.a(); b++) {
      ...
      for (byte b1 = 0; b1 < arrayOfInt1.length; b1++) {
        if (!((aZ)arrayList.get(b1)).a())
          paramGraphics.drawLine(arrayOfInt1[b1], arrayOfInt2[b1] - 1, arrayOfInt1[b1], arrayOfInt2[b1] + 1); 
      } 
    } 
  }
  ...
  public void b(Graphics paramGraphics) {
    ...
    if (w()) {
      j = v();
      if (j <= 0)
        j = rectangle.width * i / rectangle.height; 
      for (byte b = 1; b < j; b++) {
  ...
  private int f(double paramDouble) {
    ...
    return (int)Math.round((paramDouble - d - this.l) / (this.m - this.l - this.k) * rectangle.width + rectangle.x);
  }
  ...
  private int g(double paramDouble) {
    Rectangle rectangle = k();
    return rectangle.height + rectangle.y - (int)Math.round((paramDouble - h()) / (i() - h()) * rectangle.height);
  }
  ...
  private void a(double paramDouble1, double paramDouble2) {
    ...
    while (iterator.hasNext())
      ((aY)iterator.next()).a(paramDouble1, paramDouble2); 
  }
