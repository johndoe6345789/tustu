...
public class m extends JComponent implements l {
  ...
  private void b(Graphics paramGraphics) {
    ...
    if (this.t != null && this.t.length > k && (this.t[0]).length > n)
      paramGraphics.drawString("Hits : " + a(k, n), j, this.z.top + paramGraphics.getFont().getSize() * 3); 
    if (!Double.isNaN(q.a()) && !Double.isInfinite(q.a())) {
      j = this.z.left + i - paramGraphics.getFontMetrics().stringWidth(this.V);
      paramGraphics.drawString(this.V + " : " + X.b(q.a(), this.M), j, this.z.top + paramGraphics.getFont().getSize() * 4);
    } 
    Point point = this.x;
    if (point != null)
      paramGraphics.drawRect(point.x, point.y, h() - point.x, i() - point.y); 
  }
  ...
  private double b(int paramInt1, int paramInt2) {
    ...
    for (i = paramInt1 - 2; i <= paramInt1 + 2 && i < this.w.length; i++) {
      for (int j = paramInt2 - 2; j <= paramInt2 + 2 && j < (this.w[0]).length; j++) {
        if (this.w[i][j] != Double.NEGATIVE_INFINITY)
          b++; 
      } 
    } 
    for (i = paramInt1 - 2; i <= paramInt1 + 2 && i < this.w.length; i++) {
      for (int j = paramInt2 - 2; j <= paramInt2 + 2 && j < (this.w[0]).length; j++) {
        if (k() == b) {
          ...
          if (this.w[i][j] != Double.NEGATIVE_INFINITY && (Double.isNaN(d) || d > this.w[i][j]))
            d = this.w[i][j]; 
        } else if (this.w[i][j] != Double.NEGATIVE_INFINITY && (Double.isNaN(d) || d < this.w[i][j])) {
          d = this.w[i][j];
        } 
      } 
    } 
  ...
  public Rectangle c() {
    return new Rectangle(this.z.left, this.z.top, getWidth() - this.z.left - this.z.right + 1, getHeight() - this.z.top - this.z.bottom + 1);
  }
    ...
    for (int j = paramp.f(); j < paramp.e(); j++) {
      ...
      if (!q.b()) {
        ...
        if (rectangle.contains(point1)) {
          ...
          if (i < this.t[point1.x][point1.y])
            i = this.t[point1.x][point1.y]; 
          arrayList.add(q);
        } 
      } 
    } 
      ...
      if (!Double.isNaN(paramp.i()) && !Double.isNaN(paramp.h()))
        if (k() == b) {
          if (this.w[point1.x][point1.y] == Double.NEGATIVE_INFINITY) {
            ...
            this.w[point1.x][point1.y] = this.w[point1.x][point1.y] + q.a() / this.t[point1.x][point1.y];
          } 
        } else if (k() == c) {
          if (q.a() < this.w[point1.x][point1.y] || this.w[point1.x][point1.y] == Double.NEGATIVE_INFINITY)
  ...
  private void b(Graphics paramGraphics, p paramp) {
    ...
    for (byte b2 = 0; b2 <= n; b2++) {
      ...
      if (b2 == 0) {
        ...
        paramGraphics.setColor(a(0));
        paramGraphics.drawString(str, i1 - i4 - 3, i3 + i2 - i2 - paramGraphics.getFont().getSize());
      } else {
        this.W.add(new n(this, d3 - d2, d3 + d2, color));
        paramGraphics.setColor(a(0));
        paramGraphics.drawString(str, i1 - i4 - 3, i3 + i2 - (i2 - paramGraphics.getFont().getSize()) / 2);
      } 
    } 
  }
  ...
  private Color a(p paramp, double paramDouble, int paramInt) {
    ...
    if (!this.W.isEmpty() && this.P < Integer.MAX_VALUE)
      for (n n : this.W) {
        if (paramDouble >= n.a() && paramDouble <= n.b())
          return n.c(); 
      }  
  ...
  private void u() {
    if (this.F) {
      this.z = new Insets(this.A.top, this.A.left + eJ.a(25) * this.d.size(), this.A.bottom + eJ.a(25) * this.d.size(), this.A.right);
    } else {
      this.z = new Insets(this.A.top, this.A.left + eJ.a(25), this.A.bottom + eJ.a(25), this.A.right);
    } 
  }
  ...
  private void a(double paramDouble1, double paramDouble2, double paramDouble3, double paramDouble4) {
    for (k k : this.f)
      k.a(paramDouble1, paramDouble2, paramDouble3, paramDouble4); 
  }
