...
public class HashMapUsingArrayList {
  ...
  public List a(a parama) {
    ...
    for (c c : this.b.values()) {
      if (parama == null || parama.a(c)) arrayList.add(c);
    }
    for (f f : this.c) arrayList.addAll(f.a(parama));
    return arrayList;
  }
  ...
  public void a(String paramString, Properties paramProperties) {
    c c = b(paramString);
    if (c == null) throw new EInterfaceEcho("No Action found for action name: " + paramString);
    try {
      c.a(paramProperties);
    } catch (Exception exception) {
      exception.printStackTrace();
      throw new EInterfaceEcho(
          "An Exception occurred action name: " + paramString + "\n" + exception.getMessage());
    } catch (Error error) {
      error.printStackTrace();
      a(paramString);
      throw new EInterfaceEcho(
          "An Error occurred action name: "
              + paramString
              + ", Action removed.\n"
              + error.getMessage());
    }
