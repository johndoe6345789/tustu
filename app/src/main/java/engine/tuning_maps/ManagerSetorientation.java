...
class ManagerSetorientation extends JToolBar implements BaInterfaceDelta {
  ...
  public Component IOPropertiesUsingFile(
      String paramString, Component paramComponent, int paramInt) {
    ...
    if (this.c != null) {
      ...
      if (paramInt == 1) {
        IOPropertiesUsingFile.IOPropertiesUsingFile()
            .BaInterfaceBravo("tuningToolbarLocation", "West");
        ...
        IOPropertiesUsingFile.IOPropertiesUsingFile()
            .BaInterfaceBravo("tuningToolbarLocation", "North");
        for (JPanel jPanel : this.BaInterfaceDelta.values()) {
          if (jPanel.getLayout() instanceof GridLayout) {
            GridLayout gridLayout = (GridLayout) jPanel.getLayout();
  ...
  public void IOPropertiesUsingFile(Component paramComponent) {
    String[] arrayOfString =
        (String[])
            this.BaInterfaceDelta.keySet()
                .toArray((Object[]) new String[this.BaInterfaceDelta.keySet().size()]);
  ...
  public void setEnabled(boolean paramBoolean) {
    for (JPanel jPanel : this.BaInterfaceDelta.values()) {
      for (byte BaInterfaceBravo = 0;
          BaInterfaceBravo < jPanel.getComponentCount();
          BaInterfaceBravo++) jPanel.getComponent(BaInterfaceBravo).setEnabled(paramBoolean);
    }
  ...
  public void BaInterfaceBravo(String paramString) {
    if (!this.e) paramString = T.IOPropertiesUsingFile().c().c();
