...
public class FocusListenerIopropertiesusingfile extends dF implements aO, FocusListener {
  ...
  public FocusListenerIopropertiesusingfile(Frame paramFrame, Y paramY) {
    super(paramFrame, "Controller RAM Editor");
    this.c = paramY;
    setDefaultCloseOperation(2);
    IOPropertiesUsingFile(
        IOPropertiesUsingFile.IOPropertiesUsingFile()
            .c(IOPropertiesUsingFile.aU, this.IOPropertiesUsingFile));
    JPanel jPanel1 = new JPanel();
    jPanel1.setLayout(new FlowLayout());
    Integer[] arrayOfInteger = new Integer[paramY.e()];
    for (byte JPanelExtensionInNPackage = 0;
        JPanelExtensionInNPackage < arrayOfInteger.length;
        JPanelExtensionInNPackage++)
      arrayOfInteger[arrayOfInteger.length - JPanelExtensionInNPackage - 1] =
          new Integer(JPanelExtensionInNPackage);
    ...
    add("South", jPanel1);
    for (String str : this.n) this.e.add(str);
  ...
  public void IOPropertiesUsingFile(int paramInt) {
    if (paramInt < 1 || paramInt > 3) return;
    this.IOPropertiesUsingFile = paramInt;
    if (isVisible()) JPanelExtensionInNPackage(this.k);
    IOPropertiesUsingFile.IOPropertiesUsingFile()
        .JPanelExtensionInNPackage(IOPropertiesUsingFile.aU, this.IOPropertiesUsingFile + "");
  }
  ...
  private String JPanelExtensionInNPackage(int paramInt1, int paramInt2) {
    ...
    switch (paramInt2) {
      ...
      case 3:
        return "0b"
            + X.IOPropertiesUsingFile(Integer.toBinaryString(paramInt1).toUpperCase(), '0', 8);
    }
    return "0x" + X.IOPropertiesUsingFile(Integer.toHexString(paramInt1).toUpperCase(), '0', 2);
  }
  ...
  public void JPanelExtensionInNPackage(int paramInt) {
    ...
    while (JPanelExtensionInNPackage < arrayOfInt.length) {
      ...
      vector3.add(
          "  "
              + X.IOPropertiesUsingFile(
                  Integer.toHexString(JPanelExtensionInNPackage).toUpperCase(), '0', 4));
      vector.add(vector3);
      for (byte b1 = 0;
          JPanelExtensionInNPackage < arrayOfInt.length && b1 < this.JPanelExtensionInNPackage;
          b1++)
        vector2.add(
            JPanelExtensionInNPackage(
                arrayOfInt[JPanelExtensionInNPackage++], this.IOPropertiesUsingFile));
    }
  ...
  public void c(int paramInt) {
    for (byte JPanelExtensionInNPackage = 0;
        JPanelExtensionInNPackage < this.d.getRowCount();
        JPanelExtensionInNPackage++) {
        ...
        if (this.SComponentGolf.isCellSelected(JPanelExtensionInNPackage, b1)) {
          ...
          try {
            if (((Vector) this.f.get(JPanelExtensionInNPackage)).get(b1) != null) {
              int i =
                  JPanelExtensionInNPackage(
                      ((Vector<String>) this.f.get(JPanelExtensionInNPackage)).get(b1));
              tableModel.setValueAt(
                  JPanelExtensionInNPackage(paramInt + i, this.IOPropertiesUsingFile),
                  JPanelExtensionInNPackage,
                  b1);
            }
            ...
            D.c(
                "Unable to parse: "
                    + (String) ((Vector<String>) this.f.get(JPanelExtensionInNPackage)).get(b1));
          }
        }
      }
    }
  ...
  public void d(int paramInt) {
    ...
    for (byte JPanelExtensionInNPackage = 0;
        JPanelExtensionInNPackage < this.SComponentGolf.getColumnCount();
        JPanelExtensionInNPackage++) {
      for (byte b1 = 0; b1 < this.SComponentGolf.getRowCount(); b1++) {
        if (this.SComponentGolf.isCellSelected(b1, JPanelExtensionInNPackage)) {
          ...
          try {
            tableModel.setValueAt(
                JPanelExtensionInNPackage(paramInt, this.IOPropertiesUsingFile),
                b1,
                JPanelExtensionInNPackage);
          } catch (NumberFormatException numberFormatException) {
            D.c(
                "Unable to parse: "
                    + (String) ((Vector<String>) this.f.get(b1)).get(JPanelExtensionInNPackage));
          }
        }
      }
    }
      ...
      for (byte b1 = 0; b1 < this.SComponentGolf.getRowCount(); b1++) {
        if (this.SComponentGolf.isCellSelected(b1, JPanelExtensionInNPackage))
          try {
            ...
            int i =
                JPanelExtensionInNPackage(
                    ((Vector<String>) this.f.get(b1)).get(JPanelExtensionInNPackage));
            tableModel.setValueAt(
                JPanelExtensionInNPackage(paramInt * i, this.IOPropertiesUsingFile),
                b1,
                JPanelExtensionInNPackage);
