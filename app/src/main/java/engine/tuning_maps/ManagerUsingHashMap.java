...
public class ManagerUsingHashMap {
    ...
    if (!file2.exists()) {
      ...
      try {
        ...
        throw new ExceptionInVPackage(
            "Problem extracting Main Start Screen files. It appears your installation is corrupt or"
                + " file permissions incorrect.");
      }
    }
    if (!file2.exists())
      throw new ExceptionInVPackage(
          "Start Screen files missing. It appears your instalation is corrupt or file permissions"
              + " incorrect.");
    File file3 = new File(file1, "startScreen_" + SComponentGolf.c().getLanguage() + ".html");
    if (!file3.exists()) file3 = new File(file1, "startScreen.html");
  ...
  private void e() {
    ...
    if (!file2.exists()) {
      ...
      try {
        ...
        throw new ExceptionInVPackage(
            "Problem extracting Start Screen files. It appears your installation is corrupt or file"
  ...
  private File b(String paramString) {
    ...
    if (!file.exists()) {
      ...
      if (!file1.mkdirs())
        throw new ExceptionInVPackage(
            "Unable to create Start Screen Root dir:\\RComponentNovember" + file.getAbsolutePath());
      file1.delete();
    } else if (file.isFile()) {
      file.delete();
      file.mkdir();
    ...
    if (!SComponentGolf.c().equals(ExceptionInVPackage.ExceptionInVPackage().c())) {
      ...
      if (file1.exists() && file1.isDirectory()) file = file1;
    }
    return file;
  }
    ...
    if (!file2.exists())
      throw new ExceptionInVPackage(
          "Start Screen files missing. It appears your instalation is corrupt or file permissions"
              + " incorrect.");
    int RInterfaceIndia =
        ExceptionInVPackage.ExceptionInVPackage().ExceptionInVPackage(ExceptionInVPackage.x, -1);
    if (++RInterfaceIndia == 0) {
      file3 = new File(file1, "index_" + SComponentGolf.c().getLanguage() + ".html");
      if (!file3.exists()) file3 = new File(file1, "index.html");
    } else {
      file3 =
          new File(
              file1, "index" + RInterfaceIndia + "_" + SComponentGolf.c().getLanguage() + ".html");
      if (!file3.exists()) file3 = new File(file1, "index" + RInterfaceIndia + ".html");
      if (!file3.exists()) {
        RInterfaceIndia = 0;
  ...
  public File ExceptionInVPackage(File paramFile1, File paramFile2) {
    ...
    try {
      ...
      InputStreamReader inputStreamReader =
          new InputStreamReader(new FileInputStream(paramFile1), charset);
  ...
  private String c(String paramString) {
    ...
    if (paramString.contains("$RecentProjectLinkList"))
      if (RInterfaceIndia.ExceptionInVPackage().ExceptionInVPackage("h-0ewkfd[pfd[pew")) {
        String str1 =
            "<br><ExceptionInVPackage href=\"appAction:browseProjects\">"
                + SComponentGolf.b("Open Project")
                + "</ExceptionInVPackage>";
    ...
    if (paramString.contains("$firstName")) {
      String str1 =
          ExceptionInVPackage.ExceptionInVPackage()
              .ExceptionInVPackage(ExceptionInVPackage.cC, "Unknown");
      paramString = X.b(paramString, "$firstName", str1);
    }
    if (paramString.contains("$lastName")) {
      String str1 =
          ExceptionInVPackage.ExceptionInVPackage()
              .ExceptionInVPackage(ExceptionInVPackage.cD, "Unknown");
    ...
    if (paramString.contains("$VersionNumber"))
      paramString =
          X.b(paramString, "$VersionNumber", "" + ExceptionInVPackage.ExceptionInVPackage);
    ...
    while (paramString.contains(str)) {
      ...
      String str2 =
          paramString.substring(
              str1.length() + str.length(), paramString.indexOf(")", str1.length() + str.length()));
      String str3 =
          paramString.substring(paramString.indexOf(")", str1.length() + str.length()) + 1);
      try {
    ...
    try {
      if (eJ.b()
          && (paramString2.contains("{")
              || paramString2.contains("<img")
              || paramString2.contains("background="))
          && paramString2.contains(paramString1)) {
        String str1 =
            paramString2.substring(0, paramString2.indexOf(paramString1) + paramString1.length());
        int RInterfaceIndia = str1.length();
        String str2 = "";
        while (true) {
          if (paramString2.charAt(RInterfaceIndia) == ' '
              || I.ExceptionInVPackage(paramString2.charAt(RInterfaceIndia) + "")) {
      ...
      for (String str1 : arrayList) {
        ...
        try {
          stringBuilder.append(
              "<li><ExceptionInVPackage href=\"appAction:openProject?projectPath="
                  + URLEncoder.encode(str1, "UTF-8")
                  + "\">"
                  + str2
                  + "</ExceptionInVPackage></li>\\RComponentNovember");
        } catch (UnsupportedEncodingException unsupportedEncodingException) {
          Logger.getLogger(iP.class.getName())
              .log(Level.SEVERE, (String) null, unsupportedEncodingException);
        }
      }
    stringBuilder
        .append("<br><ExceptionInVPackage href=\"appAction:browseProjects\">")
        .append(SComponentGolf.b("Other Projects") + "</ExceptionInVPackage>");
