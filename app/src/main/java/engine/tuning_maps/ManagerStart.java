...
public class ManagerStart extends Thread {
  ...
  public void ExceptionInVPackage(File paramFile1, File paramFile2) {
    ...
    if (paramFile1.getName().toLowerCase().endsWith(".frd")) str = AzInterfaceFoxtrot;
  ...
  public void run() {
    this.ExceptionInVPackage =
        new eB(
            this.b,
            "Transform Raw Log files to MSL File",
            "Converting all selected log files, please wait...",
            true,
            false);
    ...
    for (byte b = 0; b < this.h.size(); b++) {
      ...
      this.ExceptionInVPackage.ExceptionInVPackage(
          "Converting log " + file1.getName() + ", please wait...");
    ...
    if (file != null && this.h.size() == 1) {
      if (bV.ExceptionInVPackage(
          SComponentGolf.b("File ready for viewing, would you like to open it now?"), this.b, true))
      ...
      if (arrayList.size() > 0
          && bV.ExceptionInVPackage(
              SComponentGolf.b("File ready for viewing, would you like to open it now?"),
              k,
              true)) {
