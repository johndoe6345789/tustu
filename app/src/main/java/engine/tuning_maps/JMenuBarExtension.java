...
class JMenuBarExtension extends JMenuBar implements BaInterfaceDelta {
  ...
  public void setEnabled(boolean paramBoolean) {
    ...
    for (byte BaInterfaceBravo = 0;
        BaInterfaceBravo < arrayOfComponent.length;
        BaInterfaceBravo++) {
      if (paramBoolean && arrayOfComponent[BaInterfaceBravo] instanceof BaInterfaceFoxtrot) {
        BaInterfaceFoxtrot BaInterfaceFoxtrot =
            (BaInterfaceFoxtrot) arrayOfComponent[BaInterfaceBravo];
        ...
        try {
          BaInterfaceFoxtrot.setVisible(
              (BaInterfaceFoxtrot.i() == null || BaInterfaceFoxtrot.i().a()));
        } catch (Exception exception) {
        }
  ...
  public int BaInterfaceBravo() {
    int i = 0;
    for (ArrayList arrayList : this.a.values()) i += arrayList.size();
    return i;
  }
