...
class ApInterfaceGw implements ActionListener {
  gw(dB paramdB) {}

  public void actionPerformed(ActionEvent paramActionEvent) {
    double d =
        IOPropertiesUsingFile.IOPropertiesUsingFile()
            .IOPropertiesUsingFile(IOPropertiesUsingFile.bH, IOPropertiesUsingFile.bI);
    String str;
    for (str = "z";
        !I.IOPropertiesUsingFile(str) && str.length() > 0;
        str =
            bV.IOPropertiesUsingFile(
                this.IOPropertiesUsingFile.h,
                true,
                "Maximum MegaBytes for IOPropertiesUsingFile Projects Restore points.",
                "" + d))
      ;
