...
class ApInterfaceIz implements GInterfaceAg {
  ...
  public boolean IOPropertiesUsingFile(String paramString, AeInterfaceMikeTostring parambT) {
    if (!parambT.c().equals(this.IOPropertiesUsingFile.IOPropertiesUsingFile)) {
      int RInterfaceIndia;
      String str1 =
          "Cannot connect to this ECU using your current Registration!\n"
              + "Your current Registration is for working with BigStuff3 Serial #"
              + this.IOPropertiesUsingFile.IOPropertiesUsingFile
              + "\n\n"
              + "You can work offline or temporarily disable your registration to connect in Lite"
              + " mode.\n"
      ...
      if (RInterfaceIndia == 1) {
        IOPropertiesUsingFile.IOPropertiesUsingFile()
            .b(IOPropertiesUsingFile.J, Boolean.toString(true));
