...
public class NetworkDatagramSocketInAjPackage extends d {
  ...
  protected void e() {
    if (this.w != null) {
      try {
        ...
        if (J.I()) D.c("UDP Logger start command successfully sent.");
      } catch (UnknownHostException unknownHostException) {
        Logger.getLogger(k.class.getName())
            .log(Level.SEVERE, "Unable to connect to UDP server", unknownHostException);
      } catch (SocketException socketException) {
        Logger.getLogger(k.class.getName())
            .log(Level.SEVERE, "Unable to connect to UDP server", socketException);
      } catch (IOException iOException) {
        Logger.getLogger(k.class.getName())
            .log(Level.SEVERE, "Unable to connect to UDP server", iOException);
      }
    } else {
  ...
  public void c() {
    ...
    if (this.x != null) {
      try {
        ...
        Logger.getLogger(k.class.getName())
            .log(Level.WARNING, "Unable to connect to UDP server", unknownHostException);
      } catch (SocketException socketException) {
        Logger.getLogger(k.class.getName())
            .log(Level.WARNING, "Unable to connect to UDP server", socketException);
      } catch (IOException iOException) {
        Logger.getLogger(k.class.getName())
            .log(Level.WARNING, "Unable to connect to UDP server", iOException);
  ...
  protected DatagramSocket k() {
    if (this.v == null || this.v.isClosed()) {
      ...
      System.out.println(
          "Listen on "
              + this.v.getLocalAddress()
              + " from "
              + this.v.getInetAddress()
              + " port "
              + this.v.getPort());
    }
    return this.v;
  ...
  private void q() {
    ...
    if (J.I()) D.c("Received UDP Logger Packet:\n" + c.b(arrayOfInt));
    a(arrayOfInt);
  }
