...
public class AeInterfaceLima {
  public static k a(File paramFile) {
    ...
    if (str.endsWith("zip")) return b(paramFile);
    ...
    for (File file : paramFile.listFiles()) {
      if (file.isFile()
        ...
        paramk.a(file);
          ...
          && (file.getName().toLowerCase().endsWith("s19")
              || file.getName().toLowerCase().endsWith("mot")
              || file.getName().toLowerCase().endsWith("bsf"))) {
        paramk.b(file);
        ...
        paramk.d(file);
          ...
          && file.getName().toLowerCase().startsWith("release")
          && file.getName().toLowerCase().contains("notes")) {
        paramk.g(file);
      }
    }
    return paramk;
