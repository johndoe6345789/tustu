...
public class KInterfaceHotel implements S, SInterfaceDelta {
    ...
    if (T.SInterfaceAlpha().c(paramR.c()) != null) {
      try {
        SInterfaceEcho.SInterfaceAlpha()
            .SInterfaceAlpha(paramR.c(), (SInterfaceAlpha) this.SInterfaceAlpha);
      } catch (q q) {
        D.SInterfaceAlpha(
            "EcuConfiguration '"
                + paramR.c()
                + "' not loaded, cannot add TriggeredPageRead: "
                + this.SInterfaceAlpha.SInterfaceAlpha());
      }
    } else {
  ...
  public void c(R paramR) {
    if (paramR.c().equals(this.b.c()))
      try {
        SInterfaceEcho.SInterfaceAlpha()
            .SInterfaceAlpha(this.b.c(), (SInterfaceAlpha) this.SInterfaceAlpha);
      } catch (q q) {
        D.SInterfaceAlpha(
            "EcuConfiguration '"
                + this.b.c()
