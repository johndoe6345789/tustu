...
public class OutputChannelComboBox extends JComboBox implements F, bc {
  ...
  public C(
    ...
    try {
      ManagerUsingConcurrentHashMap.ExceptionInVPackage(
          this.ExceptionInVPackage.c(), this.b.aL(), this.c);
      ...
      try {
        ...
        if (this.k) {
          if (this.b.e() >= 4) {
            ...
            arrayOfDouble1[this.ExceptionPrintstacktrace][this.f] =
                (SerializableImpl.x() - this.l.O().af());
          }
        } else {
          arrayOfDouble1[this.ExceptionPrintstacktrace][this.f] =
        ...
        if (this.i) {
          if (SerializableImpl.t())
            arrayOfDouble2[this.ExceptionPrintstacktrace][this.f] =
                ((int) arrayOfDouble2[this.ExceptionPrintstacktrace][this.f] | 0x40);
          if (!SerializableImpl.p())
            arrayOfDouble2[this.ExceptionPrintstacktrace][this.f] =
                ((int) arrayOfDouble2[this.ExceptionPrintstacktrace][this.f] | 0x80);
        }
        try {
        ...
        D.ExceptionInVPackage(
            "Value Out of Bounds updating:" + SerializableImpl.aL() + " offset to " + this.b.aL());
        Logger.getLogger(C.class.getName())
            .log(Level.SEVERE, (String) null, (Throwable) ExceptionExtensionGetmessage);
      } catch (ExceptionPrintstacktrace g1) {
        D.ExceptionInVPackage(
            "Error updating:" + SerializableImpl.aL() + " offset to " + this.b.aL());
      ...
      try {
        ...
        for (byte b = 0; b < OutputChannelComboBox(); b++) {
          SerializableImpl SerializableImpl =
              this.l.ExceptionPrintstacktrace((String) getItemAt(b));
          if (this.k) {
        ...
        if (!bool && OutputChannelComboBox() > 0) OutputChannelComboBox(getItemAt(0));
      } catch (ExceptionPrintstacktrace g1) {
        g1.printStackTrace();
      }
  }
  ...
  private void ExceptionInVPackage(SerializableImpl paramaH) {
    ...
    while (iterator.hasNext()) ((p) iterator.next()).ExceptionInVPackage(paramaH);
  }
  ...
  public void ExceptionInVPackage(R paramR) {
    ...
    this.h = true;
    OutputChannelComboBox();
    if (paramR.O().al().equals("XCP")) this.k = true;
    ...
    for (b = 0; b < arrayOfString.length; b++) {
      ...
      if (SerializableImpl.b() != null
          && !SerializableImpl.b().equals("formula")
          && !SerializableImpl.b().equals("bits")
          && (!this.ExceptionExtensionGetmessage || SerializableImpl.l() != 4))
    ...
    for (b = 0; b < arrayOfString.length; b++) OutputChannelComboBox((E) arrayOfString[b]);
    this.h = false;
  }
}
