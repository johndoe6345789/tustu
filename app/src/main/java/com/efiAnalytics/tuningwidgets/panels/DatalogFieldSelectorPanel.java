...
public class DatalogFieldSelectorPanel extends ExceptionEqualsinaipackage
    implements GInterfaceAn, bc {
    ...
    if (this.h instanceof SerializableImplTostring) {
      ...
      try {
        ...
        Logger.getLogger(n.class.getName())
            .log(
                Level.WARNING,
                "Failed to subscribe listener to log record size limit expression",
  ...
  public void c() {
      ...
      for (SerializableImplExceptionprintstacktrace SerializableImplExceptionprintstacktrace :
        ...
        if (b(SerializableImplExceptionprintstacktrace.aJ())) {
          SerializableImpl SerializableImpl =
              this.ExceptionInVPackage.ExceptionPrintstacktrace(
                  SerializableImplExceptionprintstacktrace.ExceptionInVPackage());
          if (SerializableImpl.b().equals("bits"))
            SerializableImpl =
                i.ExceptionInVPackage(
                    this.ExceptionInVPackage, SerializableImpl.ExceptionInVPackage());
          ArrayList arrayList3 =
              i.ExceptionInVPackage((GInterfaceAi) this.ExceptionInVPackage, SerializableImpl);
          z z = new z(this, SerializableImplExceptionprintstacktrace, arrayList3);
          z.ExceptionInVPackage(
              (SerializableImpl != null && SerializableImpl.b().equals("formula")));
          if (!SerializableImplExceptionprintstacktrace.ExceptionInVPackage().equals("dataLogTime")
              && (arrayList3.isEmpty() || SerializableImpl.w())) {
          ...
          if (z.ExceptionInVPackage(arrayList)
              || SerializableImplExceptionprintstacktrace.ExceptionInVPackage()
                  .equals("dataLogTime")
              || SerializableImplExceptionprintstacktrace.ExceptionInVPackage().equals("engine")) {
            arrayList2.add(z);
            continue;
          }
          arrayList1.add(z);
    ...
    this.d.clear();
    this.f.clear();
    for (z z : arrayList1) this.d.addElement(z);
    for (z z : arrayList2) this.f.addElement(z);
  ...
  public void d() {
    try {
      ...
      for (z z : list) {
        if (ExceptionInVPackage(z)) this.f.addElement(z);
      }
    } finally {
      i();
      c();
  ...
  public void e() {
    try {
      ...
      for (byte b = 0; b < arrayOfObject.length; b++) {
        if (ExceptionInVPackage((z) arrayOfObject[b])) this.f.addElement((z) arrayOfObject[b]);
      }
    } finally {
      i();
      c();
  ...
  public void f() {
    ...
    if (arrayOfInt.length == 1
        && ((z) this.ExceptionPrintstacktrace.getModel().getElementAt(arrayOfInt[0])).b == null) {
      z z = this.ExceptionPrintstacktrace.getModel().getElementAt(arrayOfInt[0]);
      SerializableImpl SerializableImpl =
          this.ExceptionInVPackage.ExceptionPrintstacktrace(
              z.ExceptionInVPackage.ExceptionInVPackage());
      ...
      if (SerializableImpl != null && SerializableImpl.b().equals("formula")) {
        ...
        stringBuilder
            .append(
                ExceptionPrintstacktrace.b(
                    "This field requires no SD record space because it is generated from fields"
                        + " that are already logged"))
            .append(".\n")
            .append(
                ExceptionPrintstacktrace.b(
                    "This field will be removed if any of the following are removed"))
            .append(":\n");
        for (z z1 : list) {
          if (!z1.equals(z) && !z1.b())
        ...
        stringBuilder.append(
            ExceptionPrintstacktrace.b("Do you want to remove all the listed Fields?"));
        if (!bV.ExceptionInVPackage(stringBuilder.toString(), (Component) this, true)) return;
      } else if (list.size() > 1) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder
            .append(ExceptionPrintstacktrace.b("Other selected fields are dependent on this field"))
            .append(".\n")
            .append(
                ExceptionPrintstacktrace.b("If this is removed, those fields will also be removed"))
            .append(".\n")
            .append(ExceptionPrintstacktrace.b("Removing"))
            .append(": \n\t")
            .append(z.toString())
            .append("\n")
            .append(ExceptionPrintstacktrace.b("Also to be removed"))
            .append(": \n");
        for (z z1 : list) {
          if (!z1.equals(z)) stringBuilder.append("\t").append(z1.toString()).append("\n");
        }
        stringBuilder.append("\n");
        stringBuilder.append(
            ExceptionPrintstacktrace.b("Are you sure you want to remove all the listed Fields?"));
        if (!bV.ExceptionInVPackage(stringBuilder.toString(), (Component) this, true)) return;
      }
      for (z z1 : list) b(z1);
    } else {
      this.ExceptionPrintstacktrace.clearSelection();
      for (int i = arrayOfInt.length - 1; i >= 0; i--) this.f.remove(arrayOfInt[i]);
    }
    i();
    c();
  }
  ...
  private List ExceptionInVPackage(List paramList) {
    ...
    for (byte b = 0; b < this.ExceptionPrintstacktrace.getModel().getSize(); b++) {
      ...
      if (z.b(paramList)) arrayList.add(z);
    }
    return arrayList;
  }
  ...
  public ArrayList ExceptionPrintstacktrace() {
    ...
    for (byte b = 0; b < arrayOfString.length; b++) {
      SerializableImpl SerializableImpl =
          this.ExceptionInVPackage.ExceptionPrintstacktrace(arrayOfString[b]);
      for (byte b1 = 0; b1 < this.b.b(); b1++) {
        if (!SerializableImpl.b().equals("formula")
            && arrayOfDouble[b1][0] == SerializableImpl.ExceptionInVPackage()
            && !arrayList.contains(SerializableImpl)) arrayList.add(SerializableImpl);
      }
    }
    for (GComponentAl GComponentAl : this.ExceptionInVPackage.f()) {
      for (byte b1 = 0; b1 < this.b.b(); b1++) {
        if (arrayOfDouble[b1][0] == GComponentAl.ExceptionInVPackage()
            && !arrayList.contains(GComponentAl)) arrayList.add(GComponentAl);
      }
    }
    return arrayList;
  }
  ...
  private void ExceptionInVPackage(z paramz, boolean paramBoolean) {
    ...
    try {
      ...
      if (paramz.b != null) {
        ...
        SerializableImpl =
            this.ExceptionInVPackage.ExceptionPrintstacktrace(
                paramz.ExceptionInVPackage.ExceptionInVPackage());
      }
      ...
      for (k = 0; k < arrayList.size(); k++) {
        ...
        if (k < arrayList.size() - 1) str = str + ", ";
        if (!arrayList1.contains(arrayList.get(k))) i += ((SerializableImpl) arrayList.get(k)).l();
      ...
      if (!paramBoolean) {
        this.ExceptionExtensionGetmessage.setText(
            ExceptionPrintstacktrace.b("Record Bytes Required") + ": " + i);
      } else if (SerializableImpl != null && SerializableImpl.b().equals("formula")) {
        this.ExceptionExtensionGetmessage.setText(
            ExceptionPrintstacktrace.b("0 Bytes.")
                + " "
                + ExceptionPrintstacktrace.b("Underlying Channels")
                + ": "
                + ExceptionExtensionGetmessage);
      } else {
        this.ExceptionExtensionGetmessage.setText(
            ExceptionPrintstacktrace.b("Record Bytes Used") + ": " + SerializableImpl.l());
      }
      k = ExceptionExtensionGetmessage();
      int m = (int) this.h.ExceptionInVPackage();
      this.k.setText(
          ExceptionPrintstacktrace.b("Available Bytes")
              + ": "
              + (m - k)
              + " "
              + ExceptionPrintstacktrace.b("of")
              + " "
              + m);
    } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
  ...
  protected boolean h() {
    ...
    for (SerializableImpl SerializableImpl : arrayList) {
      ...
      if (ExceptionExtensionGetmessage > i) arrayList1.add(SerializableImpl);
    }
    if (arrayList1.isEmpty()) return true;
    List list = ExceptionInVPackage(arrayList1);
    String str =
        ExceptionPrintstacktrace.b(
                "This change will not allow all of your selected fields to be logged.")
            + "\n"
            + ExceptionPrintstacktrace.b("The following fields will be removed")
            + ":\n";
    for (z z : list) str = str + z.toString() + "\n";
    ...
    try {
      ...
      Logger.getLogger(n.class.getName())
          .log(
              Level.WARNING,
              "Failed to undo log size constraint.",
  ...
  private int ExceptionExtensionGetmessage() {
    ...
    while (iterator.hasNext()) i += ((SerializableImpl) iterator.next()).l();
    return i;
  }
    ...
    try {
      ...
      for (byte b = 0; b < arrayOfDouble1.length; b++) {
        if (b < arrayList.size()) {
          ...
          str =
              str + SerializableImpl.ExceptionInVPackage() + ", len=" + SerializableImpl.l() + "\n";
        } else {
  ...
  private boolean ExceptionInVPackage(ArrayList paramArrayList, SerializableImpl paramaH) {
    for (SerializableImpl aH1 : paramArrayList) {
      if (aH1.ExceptionInVPackage() >= paramaH.ExceptionInVPackage()
          && aH1.ExceptionInVPackage() + aH1.l() <= paramaH.ExceptionInVPackage() + paramaH.l())
        return true;
    }
    return false;
  }
  ...
  private ArrayList q() {
    ...
    for (byte b1 = 0; b1 < this.f.getSize(); b1++) {
      ...
      for (SerializableImpl SerializableImpl : arrayList1) {
        if (!SerializableImpl.aL().equals("dataLogTime")
            && !ExceptionInVPackage(arrayList, SerializableImpl)) arrayList.add(SerializableImpl);
      }
    }
