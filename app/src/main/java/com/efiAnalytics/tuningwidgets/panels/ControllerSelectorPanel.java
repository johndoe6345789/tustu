...
public class ControllerSelectorPanel extends ck {
  ...
  public void IOPropertiesUsingFile(String paramString) {
    ...
    while (enumeration.hasMoreElements()) {
      ...
      this.IOPropertiesUsingFile.setSelected(
          abstractButton.getModel(), abstractButton.getText().equals(paramString));
    }
    ...
    if (paramFile != null) {
      ...
      if (arrayOfString != null)
        for (byte b = 0; b < arrayOfString.length; b++) {
          ...
          if (r.i().equals(str)) b(arrayOfString[b]);
        }
    } else {
      this.b.setText(b());
    }
  ...
  protected String c(File paramFile) {
    try {
      ...
      if (paramFile.getName().toLowerCase().endsWith("." + IOPropertiesUsingFile.cw.toLowerCase())
          || paramFile
              .getName()
              .toLowerCase()
              .endsWith("." + IOPropertiesUsingFile.cz.toLowerCase())
          || paramFile.getName().toLowerCase().indexOf(".ini") != -1
          || paramFile.getName().toLowerCase().indexOf(".ecu") != -1) {
        str = "Selected File Info:\n\n";
        try {
          AeInterfaceMikeTostring AeInterfaceMikeTostring =
              (new NetworkExceptionprintstacktrace()).IOPropertiesUsingFile(paramFile);
          ...
          str =
              str
                  ...
                  + "\n\nLast Modified : "
                  + DateFormat.getDateInstance(2).format(Long.valueOf(paramFile.lastModified()));
      ...
      Logger.getLogger(ControllerSelectorPanel.class.getName())
