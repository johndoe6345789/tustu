...
public class VerticalDashedBarPainter implements GaugePainter {
  ...
  protected Image getBackImage(Gauge paramGauge) {
    if (this.backImage != null
        && this.backImage.getWidth(null) == paramGauge.getWidth()
        && this.backImage.getHeight(null) == paramGauge.getHeight()) return this.backImage;
    ...
    int j =
        paramGauge.isCounterClockwise()
            ? (this.inset + (int) d1)
            : (paramGauge.getHeight() - this.inset - (int) d1);
    ...
    for (byte b = 0; b < this.barCount; b++) {
      ...
      if (k - d1 / 4.0D < i * d2)
        if (paramGauge.isCounterClockwise()) {
          int m = j + k;
          graphics.fillRect(
              this.inset, m, paramGauge.getWidth() - 2 * this.inset, (int) (d1 * 0.7D));
        } else {
          int m = j - k;
          graphics.fillRect(
              this.inset, m, paramGauge.getWidth() - 2 * this.inset, (int) (d1 * 0.7D));
  ...
  protected void paintBar(Graphics paramGraphics, Gauge paramGauge, boolean paramBoolean) {
    double d1 =
        paramBoolean
            ? 1.0D
            : ((paramGauge.getSmoothedValue() - paramGauge.min())
    ...
    int j =
        paramGauge.isCounterClockwise()
            ? (this.inset + (int) d4)
            : (paramGauge.getHeight() - this.inset - (int) d4);
    paramGraphics.setColor(getFaded(paramGauge.getFontColor(), paramBoolean));
    for (byte b = 0; b < this.barCount; b++) {
      int k = (int) (b * d4);
      if (k - d4 / 4.0D < i * d1) {
        if (k > d2 * i) paramGraphics.setColor(getFaded(paramGauge.getWarnColor(), paramBoolean));
        if (k > d3 * i)
          paramGraphics.setColor(getFaded(paramGauge.getCriticalColor(), paramBoolean));
        if (paramGauge.isCounterClockwise()) {
          int m = j + k;
          paramGraphics.fillRect(
              this.inset, m, paramGauge.getWidth() - 2 * this.inset, (int) (d4 * 0.7D));
        } else {
          int m = j - k;
          paramGraphics.fillRect(
              this.inset, m, paramGauge.getWidth() - 2 * this.inset, (int) (d4 * 0.7D));
  ...
  private Color getFaded(Color paramColor, boolean paramBoolean) {
    return paramBoolean
        ? new Color(paramColor.getRed(), paramColor.getGreen(), paramColor.getBlue(), 35)
        : paramColor;
  }
