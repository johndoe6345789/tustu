...
public class AsymetricSweepRenderer implements aR, GaugePainter {
  ...
  private void paintNumbers(Graphics2D paramGraphics2D, Gauge paramGauge) {
    if (paramGauge.getFontColor().getAlpha() > 0) {
      ...
      int i =
          Math.round(((getBarShape(paramGauge)).j - paramGauge.getBorderWidth()) * 0.38F)
      ...
      for (c c : list) {
        ...
        float f2 =
            getBarShape(paramGauge).d((getBarShape(paramGauge)).c, f1)
  ...
  private void paintNeedle(Graphics2D paramGraphics2D, Gauge paramGauge, boolean paramBoolean) {
    ...
    Color[] arrayOfColor = {
      paramGauge.getNeedleColor().darker(), getBrighter(paramGauge.getNeedleColor())
    };
    float f1 = eJ.a(450);
    byte b1 =
        (paramGauge.getWidth() > f1) ? (20 + Math.round((paramGauge.getWidth() - f1) / 22.4F)) : 20;
    float f2 = 0.065F;
    ...
    for (byte b2 = 0; b2 < b1; b2++) {
      ...
      float f6 =
          paramGauge.isCounterClockwise()
              ? ((paramGauge.getWidth() - paramGauge.getBorderWidth()) - b2 * f3 - f3 * f2)
              : (paramGauge.getBorderWidth() + b2 * f3 + f3 * f2);
      float f7 =
          paramGauge.isCounterClockwise()
              ? ((paramGauge.getWidth() - paramGauge.getBorderWidth()) - (b2 + 1) * f3 + f3 * f2)
              : (paramGauge.getBorderWidth() + (b2 + 1) * f3 - f3 * f2);
      ...
      if (f5 < d2) {
        ...
        if (paramBoolean) {
          paramGraphics2D.setColor(
              getDarker(getDarker(getDarker(getDarker(getNeedleColor(paramGauge, f5))))));
          paramGraphics2D.fill((Shape) dL);
        }
        float f =
            (f6 - paramGauge.getBorderWidth())
                / (paramGauge.getWidth() - 2.0F * paramGauge.getBorderWidth());
        if (d2 > f) {
          float f12 =
              paramGauge.getBorderWidth()
                  + (float) d2 * (paramGauge.getWidth() - 2.0F * paramGauge.getBorderWidth());
          f7 = (f12 < f7) ? f12 : f7;
          arrayOfFloat2[1] = f7;
  ...
  private Color getNeedleColor(Gauge paramGauge, double paramDouble) {
    ...
    return ((d < paramGauge.lowCritical() && paramGauge.getValue() < paramGauge.lowCritical())
            || d > paramGauge.highCritical())
        ? paramGauge.getCriticalColor()
        : (((d < paramGauge.lowWarning() && paramGauge.getValue() < paramGauge.lowWarning())
                || d > paramGauge.highWarning())
            ? paramGauge.getWarnColor()
            : paramGauge.getNeedleColor());
  }

  protected Image getBottomOffImage(Gauge paramGauge) {
    if (this.bottomOffImage == null
        || this.bottomOffImage.getWidth(null) != paramGauge.getWidth()
        || this.bottomOffImage.getHeight(null) != paramGauge.getHeight()) {
  ...
  protected Image getTopOffImage(Gauge paramGauge) {
    if (this.topOffImage == null
        || this.topOffImage.getWidth(null) != paramGauge.getWidth()
        || this.topOffImage.getHeight(null) != paramGauge.getHeight()) {
      GraphicsEnvironment graphicsEnvironment = GraphicsEnvironment.getLocalGraphicsEnvironment();
      GraphicsDevice graphicsDevice = graphicsEnvironment.getDefaultScreenDevice();
      GraphicsConfiguration graphicsConfiguration = graphicsDevice.getDefaultConfiguration();
      this.topOffImage =
  ...
  private void paintBorder(Graphics2D paramGraphics2D, Gauge paramGauge) {
    ...
    if (paramGauge.getBorderWidth() > 0 && color.getAlpha() > 0) {
      ...
      paramGraphics2D.setStroke(
          new BasicStroke(paramGauge.getBorderWidth(), 1, 1, 10.0F, null, 0.0F));
  ...
  private List getNumberPositions(Gauge paramGauge) {
    if (this.numberPositions == null) {
      ...
      float f2 =
          (paramGauge.max() > 3000.0D) ? 1000.0F : ((paramGauge.max() > 500.0D) ? 100.0F : 1.0F);
      int j = (int) (Math.round(Math.log10(paramGauge.max() - paramGauge.min())) - 1L);
      double d = Math.pow(10.0D, j);
      int k;
      for (k = (int) Math.round((paramGauge.max() - paramGauge.min()) / d);
          k > 0 && (paramGauge.getWidth() / k) < f1;
          k = (int) ((paramGauge.max() - paramGauge.min()) / d)) d *= 2.0D;
      for (byte b1 = 1; b1 < k; b1++) {
        float f3 = (float) (paramGauge.min() + b1 * d);
        float f4 =
            (f3 - (float) paramGauge.getMin()) / (float) (paramGauge.max() - paramGauge.min());
  ...
  public boolean requiresBackgroundRepaint(s params) {
    return (this.topOffImage == null
        || params.getWidth() != this.topOffImage.getWidth(null)
        || params.getHeight() != this.topOffImage.getHeight(null));
  }
