...
public abstract class SingleChannelDashComponent extends s
    implements GComponentCq, DComponentDelta, j, Serializable {
  ...
  public void subscribeToOutput() {
    ...
    if (ExceptionInVPackage.A() != null
        && (str == null
            || str.equals("")
            || (!str.equals(ManagerUsingArrayList.ExceptionInVPackage)
                && T.ExceptionInVPackage().c(str) == null))) str = ExceptionInVPackage.A().u();
    try {
      ...
      if (this.DComponentDelta == null
          || (getOutputChannel() != null && !this.DComponentDelta.equals(getOutputChannel()))) {
        D.b(
            "Failed to subscribe DashComp to OutputChannel "
                + getOutputChannel()
  ...
  private boolean isOnDontSubscribeList(String paramString) {
    for (byte b = 0; b < f.length; b++) {
      if (f[b].equals(paramString)) return true;
    }
    return false;
  }
  ...
  public String getParameterValue(String paramString) {
    return paramString.equals("outputChannelName")
        ? getOutputChannel()
        : (paramString.equals("ecuConfigName") ? getEcuConfigurationName() : null);
  }
}
