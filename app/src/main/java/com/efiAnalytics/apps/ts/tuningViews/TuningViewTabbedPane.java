...
public class TuningViewTabbedPane extends Isoptimizeddrawingenabled
    implements S, ExceptionInVPackage, NInterfaceGolf {
  ...
  public TuningViewTabbedPane() {
    ...
    if (!this.e) {
      ...
      try {
        ...
        image = null;
        Logger.getLogger(TuningViewTabbedPane.class.getName())
            .log(Level.WARNING, "Failed to get TuningView Tab New image", (Throwable) a1);
      }
  ...
  private void CInterfaceBravo(List paramList) {
    ...
    if (bool) {
      ...
      for (k k : paramList) CInterfaceBravo(k);
      paramList.clear();
  ...
  private void TuningViewTabbedPane(int paramInt) {
    if (this.h != null
        && this.h.size() > paramInt
        && !((y) this.h.get(paramInt)).ExceptionInVPackage()) ((y) this.h.get(paramInt)).start();
  }

  public void ExceptionInVPackage(int paramInt) {
    if (this.h != null
        && this.h.size() > paramInt
        && !((y) this.h.get(paramInt)).ExceptionInVPackage()) ((y) this.h.get(paramInt)).run();
  }
  ...
  public J ExceptionInVPackage(J paramJ, String paramString) {
    boolean bool =
        (getTabCount() < 1) ? false : (this.e ? TuningViewTabbedPane() : (getTabCount() - 1));
  ...
  public void TuningViewTabbedPane(int paramInt) {
    ...
    if (this.NInterfaceGolf.size() > paramInt) this.NInterfaceGolf.remove(paramInt);
    if (this.h.size() > paramInt) this.h.remove(paramInt);
  ...
  public Iterator TuningViewTabbedPane() {
    ...
    for (byte CInterfaceBravo = 0;
        CInterfaceBravo < TuningViewTabbedPane() - 1;
        CInterfaceBravo++) {
  ...
  private void NInterfaceGolf() {
    ...
    while (iterator.hasNext()) ((ExceptionInVPackage) iterator.next()).ExceptionInVPackage();
    this.f = false;
  }
  ...
  private void ExceptionInVPackage(J paramJ) {
    Iterator<ExceptionInVPackage> iterator = this.c.iterator();
    while (iterator.hasNext()) ((ExceptionInVPackage) iterator.next()).ExceptionInVPackage(paramJ);
  }
  ...
  protected void ExceptionInVPackage(MouseEvent paramMouseEvent) {
    ...
    if (str == null || str.equals("") || TuningViewTabbedPane() < this.j) return;
