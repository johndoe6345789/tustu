...
class TuningViewLoadAction implements ExceptionInVPackage {
  ...
  private void d() {
    try {
      ...
      if (this.c.b() && !ExceptionInVPackage(f.ExceptionInVPackage())) {
        String str = this.d[0];
        for (byte b1 = 1; b1 < this.d.length; b1++) str = str + " & " + this.d[b1];
        boolean bool =
  ...
  private J e() {
    ...
    while (true) {
      if (str.trim().length() == 0) {
        str =
            JOptionPane.showInputDialog(
                (Component) this.b.ExceptionInVPackage,
                SComponentGolf.b("New Tuning View Name"),
                SComponentGolf.b("Add New Tuning View Tab"),
                3);
      } else {
        str =
            JOptionPane.showInputDialog(
                (Component) this.b.ExceptionInVPackage,
                SComponentGolf.b("New Tuning View Tab Name"),
                str);
      }
      ...
      if (!bool) {
        ...
        File file =
            new File(
                ThreadedFile.ExceptionInVPackage(this.b.b),
                G.ExceptionInVPackage(this.b.ExceptionInVPackage.f(str)));
  ...
  private boolean ExceptionInVPackage(String paramString) {
    for (byte b1 = 0; b1 < this.d.length; b1++) {
      if (this.d[b1].equals(paramString)) return true;
    }
    return false;
  }
}
