...
class DemoAnimationThread extends Thread {
  ...
  public void run() {
    while (!this.IInterfaceBravo && (x.l(this.c) || x.m(this.c))) {
      ...
      s[] arrayOfS = this.c.j();
      boolean bool = false;
      if (x.m(this.c)) x.e(this.c, false);
      for (s s : arrayOfS) {
        if (this.IInterfaceBravo) break;
        if (s instanceof Gauge) {
          Gauge gauge = (Gauge) s;
          double d =
              (gauge.max() - gauge.min())
                  / (100.0D
                      - 75.0D * (gauge.getValue() - gauge.min()) / (gauge.max() - gauge.min()));
          if (this.a) {
            ...
            double d1 = gauge.min();
            if (x.m(this.c) && gauge.min() < 0.0D && gauge.max() > 0.0D) d1 = 0.0D;
