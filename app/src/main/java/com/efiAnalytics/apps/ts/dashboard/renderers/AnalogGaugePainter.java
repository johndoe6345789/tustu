...
public class AnalogGaugePainter implements aR, GaugePainter {
  ...
  public void paintGauge(Graphics paramGraphics, s params) {
    ...
    if (parentGauge() == null) setParentGauge(gauge);
    ...
    if (gauge.max() > gauge.min()) drawNeedle(paramGraphics, gauge, d2);
  ...
  protected double getMajorTickSpacing(Gauge paramGauge) {
    ...
    int i;
    for (i = (int) ((paramGauge.max() - paramGauge.min()) / d);
        i > 0 && paramGauge.getSweepAngle() / i < b;
        i = (int) ((paramGauge.max() - paramGauge.min()) / d)) d *= 2.0D;
    return d;
  }

  protected void drawStrings(Graphics paramGraphics, Gauge paramGauge) {
    int i =
        (getGaugeHeight(paramGauge) - 2 * paramGauge.getBorderWidth()) / 12
            + paramGauge.getFontSizeAdjustment();
    ...
    if (!paramGauge.isDisplayValueAt180()) {
      ...
      while (paramGraphics.getFontMetrics(font1).stringWidth(paramGauge.title())
          > getGaugeWidth(paramGauge) * 0.5D) {
        i = (int) (i * 0.8D);
      ...
      String str1 =
          paramGauge.units().equals("")
              ? paramGauge.getDisplayValue()
              : (paramGauge.getDisplayValue() + "(" + paramGauge.units() + ")");
      double d =
          Double.isNaN(paramGauge.getRelativeBorderWidth2())
              ? 0.0D
              : paramGauge.getRelativeBorderWidth2();
      Point point =
          getPoint(
              getGaugeWidth(paramGauge),
              getGaugeHeight(paramGauge),
              (paramGauge.getSweepBeginDegree() + paramGauge.getSweepAngle() / 2),
              getGaugeWidth(paramGauge) / 2,
              getGaugeHeight(paramGauge) / 2,
              getOuterEdge(paramGauge) - 0.6D + d,
      ...
      paramGraphics.drawString(
          paramGauge.title(), j + getXOffset(paramGauge), k + getYOffset(paramGauge));
      paramGraphics.setFont(font2);
      if (this.digitsTrimmed > 0 && paramGauge.max() > this.maxFaceNumber) {
        String str2 = "x1";
        for (byte b = 0; b < this.digitsTrimmed; b++) str2 = str2 + "0";
        bool1 = paramGauge.isItalicFont() ? true : false;
      ...
      while (paramGraphics.getFontMetrics(font1).stringWidth(str1)
          > getGaugeWidth(paramGauge) * 0.5D) {
        i = (int) (i * 0.9D);
        font1 = cq.a().a(str, bool1, i);
        bool2 = true;
      ...
      paramGraphics.setColor(paramGauge.getFontColor());
      int k =
          (getGaugeHeight(paramGauge) - 2 * paramGauge.getBorderWidth()) / 14
              + paramGauge.getFontSizeAdjustment();
      ...
      Point point =
          getPoint(
              ...
              getGaugeWidth(paramGauge) / 2,
              getGaugeHeight(paramGauge) / 2,
              getOuterEdge(paramGauge) - 0.55D + d,
              paramGauge.isCounterClockwise());
      ...
      if (paramGauge.units() != null && !paramGauge.units().equals("")) {
        ...
        paramGraphics.drawString(
            paramGauge.units(), m + getXOffset(paramGauge), n + getYOffset(paramGauge));
      }
      if (str2 == null) {
        Point point1 =
            getPoint(
                ...
                getGaugeWidth(paramGauge) / 2,
                getGaugeHeight(paramGauge) / 2,
                getOuterEdge(paramGauge) - 0.66D + d,
                paramGauge.isCounterClockwise());
        paramGraphics.setFont(font1);
        int i1 = point1.x - paramGraphics.getFontMetrics().stringWidth(paramGauge.title()) / 2;
        int i2 = point1.y;
        paramGraphics.drawString(
            paramGauge.title(), i1 + getXOffset(paramGauge), i2 + getYOffset(paramGauge));
        paramGraphics.setFont(font2);
        if (this.digitsTrimmed > 0 && paramGauge.max() > this.maxFaceNumber) {
          String str4 = "x1";
          for (byte b1 = 0; b1 < this.digitsTrimmed; b1++) str4 = str4 + "0";
          paramGraphics.setFont(cq.a().a(str, bool1, (int) (i * 0.6D)));
        ...
        paramGraphics.drawString(
            str2,
            i2 + getXOffset(paramGauge),
            i3 + (int) (font1.getSize() * 1.1D) + getYOffset(paramGauge));
        paramGraphics.setFont(font2);
        if (this.digitsTrimmed > 0 && paramGauge.max() > this.maxFaceNumber) {
          String str4 = "x1";
          for (byte b1 = 0; b1 < this.digitsTrimmed; b1++) str4 = str4 + "0";
  ...
  protected double getMinMajorTickValue(Gauge paramGauge) {
    return (paramGauge.max() > 2.0D)
        ? Math.round(paramGauge.min())
        : (Math.round(paramGauge.min() * 10.0D) / 10.0D);
  }
  ...
  protected void drawNumbers(Graphics paramGraphics, Gauge paramGauge) {
    ...
    paramGraphics.setFont(
        cq.a()
            .a(
                paramGauge.getFontFamily(),
                bool,
                (getGaugeHeight(paramGauge) - 2 * paramGauge.getBorderWidth()) / 14
                    + paramGauge.getFontSizeAdjustment()));
  ...
  protected double getNumberOfMinorTickPerMajor(Gauge paramGauge) {
    return (paramGauge.getHeight() > 450 || paramGauge.getWidth() > 450)
        ? 5.0D
        : ((paramGauge.getHeight() > 325)
            ? 4.0D
            : ((paramGauge.getHeight() > 175 || paramGauge.getWidth() > 175) ? 2.0D : 1.0D));
  }
  ...
  protected void drawWarningIndicator(Graphics paramGraphics, Gauge paramGauge) {
    if (paramGauge.getValue() > paramGauge.lowWarning()
        && paramGauge.getValue() < paramGauge.highWarning()) return;
    ...
    paramGraphics.fillArc(
        Math.round((getGaugeWidth(paramGauge) - f) / 2.0F + getXOffset(paramGauge)),
        Math.round((getGaugeHeight(paramGauge) - f) / 2.0F + getYOffset(paramGauge)) - 1,
        Math.round(f),
        Math.round(f),
        paramGauge.getStartAngle(),
        paramGauge.getFaceAngle());
  }
  ...
  private Polygon getNeedleShape(Gauge paramGauge, double paramDouble) {
    ...
    int k =
        (getGaugeWidth(paramGauge) > getGaugeHeight(paramGauge))
            ? (j - 2 * paramGauge.getBorderWidth())
            : (i - 2 * paramGauge.getBorderWidth());
    Point point1 =
        getPoint(
            getGaugeWidth(paramGauge),
            getGaugeHeight(paramGauge),
  ...
  protected void drawNeedle(Graphics paramGraphics, Gauge paramGauge, double paramDouble) {
    ...
    if (paramGauge.needleImage(k) != null && k > 0) {
      ...
      double d2 =
          paramGauge.isCounterClockwise()
              ? ((-90.0D + d1) * 2.0D * Math.PI / 360.0D)
              : ((d1 - 90.0D) * 2.0D * Math.PI / 360.0D);
      Graphics2D graphics2D = (Graphics2D) paramGraphics;
      graphics2D.setRenderingHint(
          RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
      graphics2D.setRenderingHint(
      ...
      if (paramGauge.isShowHistory()) {
        if (this.lastHistoryValue != paramGauge.getHistoricalPeakValue()
        ...
        double d =
            (paramGauge.getHistoricalPeakValue() - paramGauge.getValue())
      ...
      paramGraphics.drawLine(
          i + getXOffset(paramGauge),
          j + getYOffset(paramGauge),
          point.x + getXOffset(paramGauge),
          point.y + getYOffset(paramGauge));
    }
  }
  ...
  protected Image paintBackground(Graphics paramGraphics, Gauge paramGauge, int paramInt) {
    ...
    bufferedImage =
        graphicsConfiguration.createCompatibleImage(
            getGaugeWidth(paramGauge), getGaugeHeight(paramGauge), 3);
    ...
    this.lastPaintTime = System.nanoTime();
    if (paramInt == 2) paintBgImage(graphics2D, paramGauge);
    graphics.setColor(paramGauge.getBackColor());
    graphics.setColor(getFaceColor(paramGauge, paramInt));
    graphics.fillArc(
        paramGauge.getBorderWidth(),
        paramGauge.getBorderWidth(),
        getGaugeWidth(paramGauge) - 2 * paramGauge.getBorderWidth(),
        getGaugeHeight(paramGauge) - 2 * paramGauge.getBorderWidth(),
        paramGauge.getStartAngle(),
        paramGauge.getFaceAngle());
    if (paramInt != 2) paintBgImage(graphics2D, paramGauge);
    ...
    if (paramGauge.max() > paramGauge.min()) drawNumbers(graphics, paramGauge);
  ...
  protected Color getDarker(Color paramColor) {
    return (paramColor.getBlue() + paramColor.getRed() + paramColor.getGreen() > 650)
        ? paramColor.darker().darker()
        : paramColor.darker();
  }
  ...
  protected void drawBorder(Graphics paramGraphics, Gauge paramGauge) {
    ...
    if (paramGauge.getBorderWidth() > 0 && color.getAlpha() > 0) {
      ...
      paramGraphics.drawArc(
          paramGauge.getBorderWidth() / 4,
          paramGauge.getBorderWidth() / 4,
          getGaugeWidth(paramGauge) - paramGauge.getBorderWidth() / 2,
          getGaugeHeight(paramGauge) - paramGauge.getBorderWidth() / 2,
          paramGauge.getStartAngle(),
          paramGauge.getFaceAngle());
      ...
      ((Graphics2D) paramGraphics)
          .setStroke(new BasicStroke(((paramGauge.getBorderWidth() + 4) / 2)));
      paramGraphics.drawArc(
          3 * paramGauge.getBorderWidth() / 4,
          3 * paramGauge.getBorderWidth() / 4,
          getGaugeWidth(paramGauge) - paramGauge.getBorderWidth() - paramGauge.getBorderWidth() / 2,
          getGaugeHeight(paramGauge)
              - paramGauge.getBorderWidth()
              - paramGauge.getBorderWidth() / 2,
          paramGauge.getStartAngle(),
          paramGauge.getFaceAngle());
    }
  }
  ...
  protected Point getPoint(
    ...
    double d =
        paramBoolean
            ? ((180.0D - paramDouble1) * 2.0D * Math.PI / 360.0D)
            : (paramDouble1 * 2.0D * Math.PI / 360.0D);
    float f =
        (float)
            (Math.sqrt(
                    Math.pow(paramInt1 / 2.0D, 2.0D)
                        * Math.pow(paramInt2 / 2.0D, 2.0D)
                        / (Math.pow(paramInt1 / 2.0D * Math.sin(d), 2.0D)
                            + Math.pow(paramInt2 / 2.0D * Math.cos(d), 2.0D)))
                * paramDouble2);
  ...
  public boolean requiresBackgroundRepaint(s params) {
    ...
    return (this.lastBgState != i
        || params.getWidth() != this.lastWidth
  ...
  public boolean isComponentPaintedAt(int paramInt1, int paramInt2) {
    if (parentGauge() == null) return true;
    ...
    Rectangle2D.Double double_ =
        new Rectangle2D.Double(d1, d2, getGaugeWidth(parentGauge()), getGaugeHeight(parentGauge()));
    Arc2D.Double double_1 =
        new Arc2D.Double(double_, parentGauge().getStartAngle(), parentGauge().getFaceAngle(), 2);
  ...
  public int currentState(Gauge paramGauge) {
    return (paramGauge.getValue() > paramGauge.lowWarning()
            && paramGauge.getValue() < paramGauge.highWarning())
        ? 0
        : ((paramGauge.getValue() > paramGauge.highCritical()) ? 2 : 1);
  }
    ...
    if (paramGauge.backgroundImage() != null) {
      Rectangle2D.Double double_ =
          new Rectangle2D.Double(
              0.0D, 0.0D, getGaugeWidth(parentGauge()), getGaugeHeight(parentGauge()));
      Arc2D.Double double_1 =
          new Arc2D.Double(double_, paramGauge.getStartAngle(), paramGauge.getFaceAngle(), 2);
      paramGraphics2D.clip(double_1);
      paramGraphics2D.setRenderingHint(
          RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
      paramGraphics2D.drawImage(
          paramGauge.backgroundImage(),
          1,
          1,
          getGaugeWidth(paramGauge) - 2,
          getGaugeHeight(paramGauge) - 2,
  ...
  public Area areaPainted(s params) {
    ...
    Rectangle2D.Double double_ =
        new Rectangle2D.Double(
            getXOffset(gauge), getYOffset(gauge), getGaugeWidth(gauge), getGaugeHeight(gauge));
    Arc2D.Double double_1 =
        new Arc2D.Double(double_, gauge.getStartAngle(), gauge.getFaceAngle(), 2);
    Area area = new Area();
