    ...
    do {
      font = new Font(font.getFamily(), font.getStyle(), ++JPanelExtensionInNPackage);
            ...
            > this.ExceptionInVPackage.c
                .getFontMetrics(font)
                .stringWidth(this.ExceptionInVPackage.c.getText())
        && JPanelExtensionInNPackage < j / 2);
    try {
      Image image =
          cO.ExceptionInVPackage().ExceptionInVPackage(cO.W, this.ExceptionInVPackage.c, k);
