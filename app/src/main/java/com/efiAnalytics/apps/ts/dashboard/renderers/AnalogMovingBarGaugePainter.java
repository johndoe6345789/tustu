...
public class AnalogMovingBarGaugePainter extends RoundAnalogGaugePainter {
  ...
  protected void drawNeedle(Graphics paramGraphics, Gauge paramGauge, double paramDouble) {
    double d1 =
        (getSmoothedValue(paramGauge) - paramGauge.min()) / (paramGauge.max() - paramGauge.min());
    ...
    int i = Math.round(getNeedleWidth(paramGauge));
    int j =
        (paramGauge.getWidth() - getGaugeWidth(paramGauge)) / 2
            + paramGauge.getBorderWidth()
            + i / 2;
    int k =
        (paramGauge.getHeight() - getGaugeHeight(paramGauge)) / 2
            + paramGauge.getBorderWidth()
            + i / 2;
    ...
    if (paramGauge.isCounterClockwise()) {
      ...
      if (d3 - d1 > 0.01D) {
        ...
        paramGraphics.drawArc(
            ...
            getGaugeHeight(paramGauge) - i - 2 * paramGauge.getBorderWidth(),
            180
                - paramGauge.getSweepBeginDegree()
                - paramGauge.getSweepAngle()
                + (int) Math.round(d4),
            1);
      }
    } else {
      ...
      if (d3 - d1 > 0.01D) {
        ...
        paramGraphics.drawArc(
            ...
            getGaugeWidth(paramGauge) - i - 2 * paramGauge.getBorderWidth(),
            getGaugeHeight(paramGauge) - i - 2 * paramGauge.getBorderWidth(),
            paramGauge.getSweepBeginDegree() + paramGauge.getSweepAngle() - (int) Math.round(d4),
            1);
      }
    }
  }
