...
public class MouseEventShield extends JComponent {
  ...
  protected void processMouseEvent(MouseEvent paramMouseEvent) {
    if (paramMouseEvent.getID() == 500
        || paramMouseEvent.getID() == 501
        || paramMouseEvent.getID() == 502) {
  ...
  private Component a(int paramInt1, int paramInt2) {
    ...
    synchronized (getTreeLock()) {
      for (Component component1 : this.a.getComponents()) {
        if (!(component1 instanceof g)
            && component1.contains(
                paramInt1 - component1.getX(),
                paramInt2 + (this.a.getInsets()).top - component1.getY())
            && component1 instanceof Container) {
