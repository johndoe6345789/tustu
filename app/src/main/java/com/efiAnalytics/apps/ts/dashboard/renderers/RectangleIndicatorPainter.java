...
public class RectangleIndicatorPainter implements IndicatorPainter {
  ...
  public RectangleIndicatorPainter() {
    this.maskImages =
        IOPropertiesUsingFile.IOPropertiesUsingFile().c(IOPropertiesUsingFile.al, false);
  }
    ...
    if (indicator.getValue() == 0.0D) {
      ...
      if (indicator.offImage() != null) {
        ...
        paramGraphics.drawImage(
            image, (indicator.getWidth() - i) / 2, (indicator.getHeight() - j) / 2, i, j, null);
  ...
  private void drawBackground(Graphics paramGraphics, Indicator paramIndicator, Color paramColor) {
    if (paramColor != null && paramColor.getAlpha() > 0) {
      ...
      paramGraphics.draw3DRect(
          1, 1, paramIndicator.getWidth() - 3, paramIndicator.getHeight() - 3, true);
    }
  }

  private void drawText(
      Graphics paramGraphics, String paramString, Indicator paramIndicator, Color paramColor) {
    ...
    int i =
        (paramIndicator.getWidth() - paramIndicator.getFontMetrics(font).stringWidth(paramString))
            / 2;
  ...
  protected Font getTextFont(Indicator paramIndicator) {
    if (this.textFont != null) return this.textFont;
    int i =
        (int)
            ((paramIndicator.getHeight()
                    - (paramIndicator.insets()).top
                    - (paramIndicator.insets()).bottom)
                * 0.98D);
    int j =
        paramIndicator.getWidth()
            - (paramIndicator.insets()).left
            - (paramIndicator.insets()).right;
    ...
    if (font.canDisplayUpTo(paramIndicator.onText()) >= 0
        && font.canDisplayUpTo(paramIndicator.offText()) >= 0)
      font = cq.IOPropertiesUsingFile().IOPropertiesUsingFile("", bool, i);
    while (true) {
      FontMetrics fontMetrics = paramIndicator.getFontMetrics(font);
      if ((fontMetrics.stringWidth(paramIndicator.onText()) < j
              && fontMetrics.stringWidth(paramIndicator.offText()) < j)
          || i < 1) {
        this.textFont = font;
  ...
  public boolean requiresBackgroundRepaint(s params) {
    ...
    return (params.getWidth() != this.lastWidth
        ...
        || indicator.getValue() != this.lastValue
        || this.textFont == null);
  }
  ...
  private Area getImageOutline(Indicator paramIndicator, Image paramImage, double paramDouble) {
    ...
    BufferedImage bufferedImage =
        new BufferedImage(paramIndicator.getWidth(), paramIndicator.getHeight(), 6);
    Graphics graphics = bufferedImage.getGraphics();
    graphics.drawImage(
        paramImage,
        (paramIndicator.getWidth() - i) / 2,
        (paramIndicator.getHeight() - j) / 2,
        i,
        j,
        null);
  ...
  private Area getImageOutline(BufferedImage paramBufferedImage) {
    ...
    if (paramBufferedImage.getHeight() < 256 && paramBufferedImage.getWidth() < 256) {
      ...
      if (m > 0 && j > 0) area.add(new Area(new Rectangle(k, i, m - k + 1, j - i + 1)));
    }
