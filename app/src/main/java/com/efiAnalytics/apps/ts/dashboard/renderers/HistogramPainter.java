...
public class HistogramPainter implements GaugePainter {
  ...
  private Image paintBackground(Graphics paramGraphics, Gauge paramGauge) {
    ...
    if (paramGauge.getWidth() <= 0 || paramGauge.getHeight() <= 0) return null;
    if (paramGauge.isAntialiasingOn()) {
      ...
      bufferedImage =
          graphicsConfiguration.createCompatibleImage(
              paramGauge.getWidth(), paramGauge.getHeight(), 2);
      graphics = bufferedImage.getGraphics();
    }
    ...
    if (paramGauge.backgroundImage() != null)
      graphics.drawImage(
          paramGauge.backgroundImage(), 0, 0, paramGauge.getWidth(), paramGauge.getHeight(), null);
  ...
  private void drawTitle(Graphics paramGraphics, Gauge paramGauge) {
    ...
    if (!paramGauge.units().equals("")) str = str + "(" + paramGauge.units() + ")";
    if (str.length() > 0) {
  ...
  private double[] getLineValues(Gauge paramGauge) {
    ...
    synchronized (this.historicalData) {
      ...
      for (byte b = 0; b < j; b++) arrayOfDouble[b] = this.historicalData.a(b);
    }
  ...
  private void drawGridLines(Graphics paramGraphics, Gauge paramGauge) {
    ...
    paramGraphics.drawString(
        str, 3 + paramGauge.getBorderWidth(), font.getSize() + paramGauge.getBorderWidth());
  }

  private void drawAGridLine(
      Graphics paramGraphics, Gauge paramGauge, double paramDouble, boolean paramBoolean) {
    int i = paramGauge.getBorderWidth();
    int j = paramGauge.getWidth() - 2 * i;
    int k =
        (int) Math.round((paramGauge.getHeight() - paramGauge.getBorderWidth() * 2) * paramDouble)
            + paramGauge.getBorderWidth();
    paramGraphics.setColor(Color.LIGHT_GRAY);
    ((Graphics2D) paramGraphics)
        .setStroke(paramBoolean ? this.gridLineStroke : this.dashedLineStroke);
    paramGraphics.drawLine(i, k, j + i, k);
    paramGraphics.setColor(paramGauge.getFontColor());
    if (paramBoolean) {
      String str =
          paramGauge.formatDouble(
              paramGauge.max() - paramDouble * (paramGauge.max() - paramGauge.min()),
              paramGauge.getLabelDigits());
      Font font = getLineFont(paramGauge);
      paramGraphics.setFont(font);
  ...
  public boolean requiresBackgroundRepaint(s params) {
    return (this.backImage == null
        || params.getWidth() != this.backImage.getWidth(null)
        || params.getHeight() != this.backImage.getHeight(null));
  }
