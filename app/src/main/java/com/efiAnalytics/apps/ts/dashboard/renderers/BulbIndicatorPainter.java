...
public class BulbIndicatorPainter implements IndicatorPainter {
  ...
  private void drawText(
    ...
    if (paramColor != null && paramString != null) {
      ...
      paramGraphics.drawString(
          paramString, i, paramIndicator.getHeight() - (int) (font.getSize() * 0.1D));
    }
  }
  ...
  protected Font getTextFont(Indicator paramIndicator) {
    ...
    int i =
        paramIndicator.getHeight()
            - (paramIndicator.getInsets()).top
            - (paramIndicator.getInsets()).bottom
            - this.minimumBulbSize;
    i = (i > 20) ? 20 : i;
    int j =
        paramIndicator.getWidth()
            - (paramIndicator.getInsets()).left
            - (paramIndicator.getInsets()).right;
    ...
    if (font.canDisplayUpTo(paramIndicator.onText()) >= 0
        && font.canDisplayUpTo(paramIndicator.offText()) >= 0) font = cq.a().a("", bool, i);
    while (true) {
  ...
  public Area areaPainted(s params) {
    Indicator indicator = (Indicator) params;
    Area area = new Area();
    if ((indicator.getOffBackgroundColor() != null
            && indicator.getOffBackgroundColor().getAlpha() > 0)
        || (indicator.getOnBackgroundColor() != null
            && indicator.getOnBackgroundColor().getAlpha() > 0)) {
