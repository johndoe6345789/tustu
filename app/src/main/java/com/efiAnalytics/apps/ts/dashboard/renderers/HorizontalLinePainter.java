...
public class HorizontalLinePainter extends HorizontalBarPainter {
  protected void paintBar(Graphics paramGraphics, Gauge paramGauge) {
    ...
    int i =
        (int) ((paramGauge.getWidth() - paramGauge.getBorderWidth() * 2) * d1)
    ...
    j = (j < 1) ? 1 : j;
    j = (j > 20) ? 20 : j;
    if (paramGauge.getSmoothedValue() > paramGauge.getHighCritical()
        || paramGauge.getSmoothedValue() < paramGauge.getLowCritical()) {
      paramGraphics.setColor(paramGauge.getCriticalColor());
    } else if (paramGauge.getSmoothedValue() > paramGauge.getHighWarning()
        || paramGauge.getSmoothedValue() < paramGauge.getLowWarning()) {
    ...
    if (paramGauge.isShowHistory() && d4 - d1 > 0.008D) {
      ...
      i =
          (int) ((paramGauge.getWidth() - paramGauge.getBorderWidth() * 2) * d4)
              + paramGauge.getBorderWidth();
