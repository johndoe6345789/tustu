  ...
  protected Image getBackImage(Gauge paramGauge) {
    ...
    for (byte b = 0; b < this.barCount; b++) {
      ...
      if (k - d1 / 4.0D < i * d2) {
        int m = j - k;
        graphics.fillRect(
            m, this.inset, (int) (d1 * 0.7D), paramGauge.getHeight() - 2 * this.inset);
      }
    }
  ...
  protected void paintBar(Graphics paramGraphics, Gauge paramGauge) {
    ...
    int j =
        paramGauge.isCounterClockwise()
            ? (paramGauge.getWidth() - this.inset - (int) d4)
            : this.inset;
    paramGraphics.setColor(paramGauge.getFontColor());
    for (byte b = 0; b < this.barCount; b++) {
      int k = (int) (b * d4);
      if (k - d4 / 4.0D < i * d1) {
        if (k > d2 * i) paramGraphics.setColor(paramGauge.getWarnColor());
        if (k > d3 * i) paramGraphics.setColor(paramGauge.getCriticalColor());
        if (paramGauge.isCounterClockwise()) {
          int m = j - k;
          paramGraphics.fillRect(
              m, this.inset, (int) (d4 * 0.7D), paramGauge.getHeight() - 2 * this.inset);
        } else {
          int m = j + k;
          paramGraphics.fillRect(
              m, this.inset, (int) (d4 * 0.7D), paramGauge.getHeight() - 2 * this.inset);
