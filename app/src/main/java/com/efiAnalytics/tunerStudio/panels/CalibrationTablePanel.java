...
public class CalibrationTablePanel extends d implements bc {
  ...
  JLabel ExceptionPrintstacktrace =
      new JLabel(
          "<html>"
              + ExceptionPrintstacktrace.b("Select settings, click")
              + " <br>\""
              + ExceptionPrintstacktrace.b("Write to Controller")
              + "\"<br> &nbsp; </html>",
          0);
  ...
  public CalibrationTablePanel(R paramR, SerializableImplUsingArrayList paramaX) {
    ...
    if (paramaX.ExceptionInVPackage() > 1) jPanel1.add(jPanel2);
    if (paramaX.ExceptionInVPackage() == 0) {
      bV.d(
          ExceptionPrintstacktrace.b("Table identifier isn't defined in the ecu configuration file")
              + "\n"
              + ExceptionPrintstacktrace.b("for reference table")
              + ": "
              + paramaX.aL(),
          (Component) this);
      return;
    ...
    if (paramaX.ExceptionInVPackage() > 0) {
      jPanel2.setBorder(
          BorderFactory.createTitledBorder(ExceptionPrintstacktrace.b("Sensor Table")));
    ...
    if (paramaX.j() > 0) {
      ...
      jPanel.setBorder(
          BorderFactory.createTitledBorder(ExceptionPrintstacktrace.b("Table Input Solution")));
      ...
      if (paramaX.j() > 1) jPanel1.add(jPanel);
    ...
    while (iterator.hasNext()) {
      ...
      try {
        ...
        D.ExceptionInVPackage(
            ExceptionInVPackage.getMessage(), (Exception) ExceptionInVPackage, this);
      }
    }
  ...
  private byte[] ExceptionInVPackage(String paramString) {
    ...
    try {
      ...
      if (m != null) {
        ...
        if (o.ExceptionInVPackage() == 1) return c.ExceptionInVPackage(o.e());
        this.n = false;
        return null;
      }
  ...
  private Tostring ExceptionInVPackage(byte[] paramArrayOfbyte) {
    for (byte b = 0; b < this.d.getItemCount(); b++) {
      ...
      if (arrayOfByte != null && c.b(paramArrayOfbyte, arrayOfByte)) {
        D.c(
            "Found solution. Solution Checksum: "
                + c.d(arrayOfByte)
                + ", controller checksum: "
      ...
      if (arrayOfByte != null) {
        D.c(
            "Solution "
                + Tostring.ExceptionInVPackage()
                + " doesn't match. Solution Checksum: "
                + c.d(arrayOfByte)
                + ", controller checksum: "
                + c.d(paramArrayOfbyte));
      } else {
  ...
  public void ExceptionInVPackage(Tostring paramba) {
    if (paramba != null)
      for (byte b = 0; b < this.d.getItemCount(); b++) {
        ...
        if (ba1.equals(paramba)) this.d.setSelectedIndex(b);
      }
  }
  ...
  private void CalibrationTablePanel() {
    ...
    for (aL aL : this.NetworkHashMap.values())
      aL.setEnabled(
          (Tostring.b() != null
              && this.NetworkHashMap.get(Tostring.b()) != null
              && aL.equals(this.NetworkHashMap.get(Tostring.b()))));
  }

  private String CalibrationTablePanel() {
    SerializableImplEquals SerializableImplEquals =
        ((S) this.c.getSelectedItem()).ExceptionInVPackage();
  ...
  public void CalibrationTablePanel() {
    if (!this.ExceptionInVPackage.C().q()) {
      bV.d(
          ExceptionPrintstacktrace.b("Not currently Online!")
              + "\n"
              + ExceptionPrintstacktrace.b(
                  "You must be connected to the controller to write this calibration table."),
          (Component) this);
      return;
    }
    this.l = true;
    ...
    try {
      ...
      if (bool) {
        ...
        while (k < arrayOfInt.length) {
          ...
          arrayOfInt2[2] = c.d(e());
          if (arrayOfByte.length > 3) j = c.ExceptionInVPackage(arrayOfByte, 3, 2, true, false);
        ...
        if (arrayList.size() == 1) ((m) arrayList.get(0)).b(o);
        m.b(o);
        ...
        m m =
            m.ExceptionInVPackage(
                this.ExceptionInVPackage.O(), CalibrationTablePanel(), arrayOfInt);
      ...
      bV.d(
          "Configuration Error trying to write reference table:\n"
              + ExceptionPrintstacktrace.getMessage(),
          (Component) this);
      CalibrationTablePanel("Correct Errors.");
    } catch (Exception exception) {
      bV.d(
          "Application Error trying to write reference table.\nSee App Debug Log for more info",
          (Component) this);
  ...
  private int[] CalibrationTablePanel(Tostring paramba) {
    ...
    if (paramba.b().trim().length() == 0)
      throw new ExceptionInVPackage(
          "You must select ExceptionInVPackage valid input solution to generate table.");
    ...
    if (this.NetworkHashMap.get(str) != null) {
      ...
      for (byte b1 = 0; b1 < arrayOfDouble.length; b1++) {
        String str1 = str;
        if (str1.indexOf("table(") != -1) str1 = ExceptionInVPackage(str1, b1);
    ...
    while (b < arrayOfInt.length) {
      ...
      CalibrationTablePanel(
          (b / this.b.d())
              + "\t"
              + X.b(arrayOfDouble[b / this.b.d()], 2)
              + "\t"
              + i
              + " \t"
              + X.b(b * 5.0D / arrayOfInt.length, 3)
              + "\n");
      byte[] arrayOfByte = new byte[this.b.d()];
      arrayOfByte = c.ExceptionInVPackage(i, arrayOfByte, this.ExceptionInVPackage.O().e());
      for (byte b1 = 0; b1 < arrayOfByte.length; b1++) arrayOfInt[b++] = arrayOfByte[b1];
    }
    return arrayOfInt;
  }
  ...
  private void ExceptionPrintstacktrace() {
    ...
    for (aL aL : this.NetworkHashMap.values()) {
      if (aL.isShowing()) aL.ExceptionInVPackage(q);
    }
  }

  private void VInterfaceHotel() {
    Q q = new CalibrationTablePanel(this);
    for (aL aL : this.NetworkHashMap.values()) aL.b(q);
  }
  ...
  private String ExceptionInVPackage(String paramString, int paramInt) {
    ...
    try {
      ...
      throw new ExceptionInVPackage(
          "Unable to evaluate expression: " + str2 + "\nin solution formula: " + paramString);
    }
  ...
  public void CalibrationTablePanel() {
    NetworkHashMap.ExceptionInVPackage()
        .ExceptionInVPackage(
            this.ExceptionInVPackage, this.b.O().get(0), bV.ExceptionInVPackage((Component) this));
  }
  ...
  private aL ExceptionInVPackage(AbstractInGPackage parambF) {
    ...
    throw new ExceptionInVPackage(
        "Table Generator type '"
            + parambF.ExceptionPrintstacktrace()
            + "'\n not understood for table generator '"
            + parambF.VInterfaceHotel()
