...
class HighSpeedLoggerThrottleThread extends Thread {
  long a = 0L;
  ...
  HighSpeedLoggerThrottleThread(TriggerLoggerPanel paramTriggerLoggerPanel) {
    ...
    while (true) {
      ...
      if (this.b)
        try {
          ...
          Logger.getLogger(TriggerLoggerPanel.class.getName())
              .log(Level.SEVERE, "Error setting DataSet 1", interruptedException);
        } catch (InvocationTargetException invocationTargetException) {
          Logger.getLogger(TriggerLoggerPanel.class.getName())
              .log(Level.SEVERE, "Error setting DataSet 2", invocationTargetException);
        }
    }
  }
