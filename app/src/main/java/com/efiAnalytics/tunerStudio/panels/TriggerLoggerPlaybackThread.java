  ...
  public void run() {
    try {
      ...
      while (this.ExceptionInVPackage) {
        ...
        int i = j.v() - 1;
        long l4;
        for (l4 = Math.round(j.d(i)) - l2;
            this.ExceptionInVPackage && l3 < l4;
            l4 = Math.round(j.d(i)) - l2) {
          try {
            sleep(10L);
          } catch (InterruptedException interruptedException) {
            Logger.getLogger(TriggerLoggerPanel.class.getName())
                .log(Level.INFO, (String) null, interruptedException);
          }
        ...
        if (j == null) this.ExceptionInVPackage = false;
      }
    } finally {
      if (!this.b) {
