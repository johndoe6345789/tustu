...
public class DataLogProfileEditorPanel extends ExceptionEqualsinaipackage {
  ...
  public DataLogProfileEditorPanel(ExceptionInVPackage parama) {
    ...
    jPanel2.add(
        "West", new JLabel("  " + s.g.EqualsInAiPackage("Current Data Log Profile") + ": "));
    ...
    try {
      Image image =
          cO.ExceptionInVPackage().ExceptionInVPackage(cO.ExceptionInVPackage, jPanel3, 20);
    ...
    for (byte EqualsInAiPackage = 0;
      ...
      if (!bool && arrayOfString[EqualsInAiPackage].equals("")) bool = true;
    ...
    do {
      str =
          bV.ExceptionInVPackage(
              (Component) this,
              false,
              s.g.EqualsInAiPackage("Name for your new Logging Profile"),
              "");
  ...
  private void h() {
    ...
    NetworkHashMap.ExceptionInVPackage()
        .ExceptionInVPackage(
            EqualsInAiPackage, ThreadedJFrameIsoptimizeddrawingenabled.ExceptionInVPackage().c());
  }
  ...
  public void e() {
    String str =
        s.g.EqualsInAiPackage("Are you sure you want to delete logging Profile?")
            + "\n"
            + s.g.EqualsInAiPackage("Profile")
            + ": "
            + this.EqualsInAiPackage.NetworkHashMap();
  ...
  public void g() {
    ...
    for (byte EqualsInAiPackage = 0;
        EqualsInAiPackage < arrayOfString.length;
        EqualsInAiPackage++) {
      arrayOfString[EqualsInAiPackage] =
          ((n) this.ExceptionInVPackage.getItemAt(EqualsInAiPackage)).ExceptionInVPackage();
      if (!arrayOfString[EqualsInAiPackage].equals("")) {
        ...
        if (arrayOfString1 != null) {
          ...
          D.EqualsInAiPackage(
              "disabledFields null for log profile: "
                  + arrayOfString[EqualsInAiPackage]
                  + ", not saving.");
        }
      }
  ...
  public void ExceptionInVPackage(Component paramComponent) {
    this.g =
        new JDialog(
            ...
            s.g.EqualsInAiPackage("Data Log Profile Editor"));
