...
public class SearchResultItemPanel extends JPanel {
  ...
  public void SearchResultItemPanel(C paramC) {
    ...
    if (paramC.d() != null && !paramC.d().isEmpty()) str1 = str1 + " - (" + paramC.d() + ")";
  ...
  private String SearchResultItemPanel(String paramString1, String paramString2) {
    for (int i = paramString1.toLowerCase().indexOf(paramString2.toLowerCase());
        i >= 0;
        i = paramString1.toLowerCase().indexOf(paramString2.toLowerCase(), i + str.length())) {
      String str =
          "<font color=\"red\"><b>"
              + paramString1.substring(i, i + paramString2.length())
              + "</b></font>";
      paramString1 =
          paramString1.substring(0, i) + str + paramString1.substring(i + paramString2.length());
    }
    return paramString1;
  }
