...
public class VexFileParser {
  public s ExceptionInVPackage(s params, String paramString) {
    ...
    double d2 = 0.0D;
    try {
      for (String str = ExceptionInVPackage(fileReader);
          str != null;
        ...
        if (str.startsWith("Scale")) {
          ...
          int j =
              Integer.parseInt(
                  str.substring(str.lastIndexOf("[") + 1, str.lastIndexOf("]")).trim());
  ...
  public void ExceptionInVPackage(
      s params, String paramString, double paramDouble1, double paramDouble2) {
    File file = new File(paramString);
    try {
      ...
      fileWriter.write(
          "Date: " + calendar.get(2) + "-" + calendar.get(5) + "-" + calendar.get(1) + "\n");
      fileWriter.write("Time: " + calendar.get(11) + ":" + calendar.get(12) + "\n");
      fileWriter.write("Page 0\n");
      if (paramDouble1 > 0.0D) fileWriter.write("Scale " + paramDouble1 + "\n");
      ExceptionInVPackage(fileWriter, "VE Table RPM Range", "", params.b(), true);
      ExceptionInVPackage(
          fileWriter, "VE Table Load Range", params.v(), params.ExceptionInVPackage(), false);
      ExceptionInVPackage(fileWriter, params, paramDouble1);
      fileWriter.flush();
      fileWriter.close();
    } catch (Exception exception) {
      throw new ExceptionInVPackage(
          "Unable to write " + paramString + "\n" + exception.getMessage());
    }
  }
  ...
  private void ExceptionInVPackage(
    ...
    paramString1 =
        ExceptionInVPackage(paramString1, 32, true) + "[" + paramArrayOfString.length + "]";
    ...
    for (byte b = 0; b < paramArrayOfString.length; b++) {
      if (paramBoolean) {
        ...
        paramFileWriter.write(
            "   ["
                + ExceptionInVPackage(b + "", 3, false)
                + "] ="
                + ExceptionInVPackage(i + "", 4, false)
                + "\n");
      } else {
  ...
  private void ExceptionInVPackage(FileWriter paramFileWriter, s params, double paramDouble) {
    ...
    String str =
        ExceptionInVPackage("VE Table", 32, true)
            + "["
            + ExceptionInVPackage(i + "", 3, false)
            + "]["
            + ExceptionInVPackage(j + "", 3, false)
            + "]";
