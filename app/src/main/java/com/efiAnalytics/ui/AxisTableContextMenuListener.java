    ...
    if (paramMouseEvent.getButton() == 3) {
      ...
      if (arrayOfInt1.length == 0)
        arrayOfInt1 =
            new int[] {
              this.b
                  ? this.c.rowAtPoint(paramMouseEvent.getPoint())
                  : this.c.columnAtPoint(paramMouseEvent.getPoint())
            };
      ...
      if (arrayOfInt2.length > 2)
        jPopupMenu
            .add("Interpolate selected cell values")
            .addActionListener(new V(this, arrayOfInt2));
