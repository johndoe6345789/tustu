...
public class Table3DProjection {
  ...
  public eZ Table3DProjection() {
    ...
    double d3 =
        this.k.getHeight() * Table3DProjection()
            + 0.5D * this.k.getHeight() / 4.0D
            - Table3DProjection() * this.k.getHeight() / 4.0D;
    double d4 = this.k.getWidth() * this.e * 1.41D / 2.0D;
    int i = (int) (this.k.getWidth() / 2.0D) - (int) (d4 * Math.cos(d() + 0.7853981633974483D));
    int j =
        (int)
                (this.k.getHeight()
                    + (d1 * Math.sin(d()) + d2 * Math.cos(d())) * Math.sin(e() - Math.PI)
                    + d3 * Math.cos(e() + Math.PI))
            / 2;
  ...
  public eZ Table3DProjection(
      double paramDouble1, double paramDouble2, double paramDouble3, eZ parameZ) {
    if (parameZ == null) parameZ = new Table3DProjection();
    ...
    parameZ.a =
        (int) Math.round(this.f.a + d1 * Math.cos(d()) + d2 * Math.cos(d() + 1.5707963267948966D));
    parameZ.b =
        (int)
            Math.round(
                this.f.b
                    + d2 * Math.cos(d()) * Math.sin(e())
                    + d1 * Math.sin(d()) * Math.sin(e())
                    + d3 * Math.sin(e() + 1.5707963267948966D));
    return parameZ;
  }
        ...
        (int)
            Math.round(
                this.f.b
                    + d2 * Math.cos(d()) * Math.sin(e())
                    + d1 * Math.sin(d()) * Math.sin(e())
                    + d4 * Math.sin(e() + 1.5707963267948966D));
    int j =
        (int)
            Math.round(
                this.f.b
  ...
  public eZ[][] Table3DProjection() {
    ...
    for (byte b = 0; b < this.j.p(); b++) {
      for (byte b1 = 0; b1 < this.j.o(); b1++)
        this.g[b][b1] =
            Table3DProjection(this.j.a(b1), this.j.b(b), this.j.d(b, b1), this.g[b][b1]);
    }
    ...
    if (this.o) {
      for (byte b = 1; b < arrayOfDouble.length; b++) {
        if (arrayOfDouble[b] >= paramDouble) {
          ...
          return (b - 1) / arrayOfDouble.length
              + d / (arrayOfDouble[b] - arrayOfDouble[b - 1]) * arrayOfDouble.length;
        }
      }
      return arrayOfDouble[arrayOfDouble.length - 1];
    }
    return (paramDouble >= arrayOfDouble[arrayOfDouble.length - 1])
        ? 1.0D
        : ((paramDouble - arrayOfDouble[0])
            / (arrayOfDouble[arrayOfDouble.length - 1] - arrayOfDouble[0]));
  }
