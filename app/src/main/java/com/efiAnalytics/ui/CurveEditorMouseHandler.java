...
class CurveEditorMouseHandler implements MouseListener, MouseMotionListener {
  ...
  public void mousePressed(MouseEvent paramMouseEvent) {
    if (!this.c.isEnabled()) return;
    this.c.requestFocus();
    for (byte b = 0; b < bN.d(this.c).size(); b++) {
      bQ bQ = bN.d(this.c).get(b);
      for (byte b1 = 0; b1 < bQ.size(); b1++) {
        bP bP1 = (bP) bQ.get(b1);
        if (Math.abs(bP1.d() - paramMouseEvent.getX()) < this.c.b / 3
            && Math.abs(bP1.f() - paramMouseEvent.getY()) < this.c.b / 3) {
    ...
    if (bN.f(this.c)) this.b = paramMouseEvent.getPoint();
  }
  ...
  public void mouseReleased(MouseEvent paramMouseEvent) {
    if (!this.c.isEnabled()) return;
    if (this.a != null) bN.b(this.c, this.c.x(), this.c.w());
    this.a = null;
    if (this.c.D != null) {
      this.c.a(this.c.D);
      this.c.D = null;
