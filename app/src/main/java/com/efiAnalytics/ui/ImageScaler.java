...
public class ImageScaler {
  ...
  public static final ConvolveOp c =
      new ConvolveOp(
          new Kernel(3, 3, new float[] {0.0F, 0.08F, 0.0F, 0.08F, 0.68F, 0.08F, 0.0F, 0.08F, 0.0F}),
          1,
          null);
  ...
  protected static void a(int paramInt, String paramString, Object... paramVarArgs) {
    if (a) {
      System.out.print(b);
      for (byte b = 0; b < paramInt; b++) System.out.print("\t");
  ...
  protected static BufferedImage a(BufferedImage paramBufferedImage, int paramInt1, int paramInt2) {
    if (paramInt1 < 0 || paramInt2 < 0)
      throw new IllegalArgumentException(
          "width [" + paramInt1 + "] and height [" + paramInt2 + "] must be >= 0");
    return new BufferedImage(
        paramInt1, paramInt2, (paramBufferedImage.getTransparency() == 1) ? 1 : 2);
  }

  public static BufferedImage a(
      BufferedImage paramBufferedImage, int paramInt1, int paramInt2, Object paramObject) {
  ...
  public static BufferedImage a(Image paramImage) {
    if (paramImage instanceof BufferedImage) return (BufferedImage) paramImage;
    BufferedImage bufferedImage =
        new BufferedImage(paramImage.getWidth(null), paramImage.getHeight(null), 2);
    Graphics2D graphics2D = bufferedImage.createGraphics();
