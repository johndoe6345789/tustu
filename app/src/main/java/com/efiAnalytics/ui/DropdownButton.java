...
public class DropdownButton extends cp implements ItemSelectable, ActionListener {
  ...
  public boolean b(String paramString) {
    ...
    for (byte b = 0; b < this.A.getComponentCount(); b++) {
      ...
      if (jMenuItem.getText().equals(paramString)
          || (jMenuItem.getActionCommand() != null
              && jMenuItem.getActionCommand().equals(paramString))) {
        a(paramString);
        return true;
      }
    }
  ...
  public void actionPerformed(ActionEvent paramActionEvent) {
    if (paramActionEvent.getSource() instanceof JMenuItem
        && b(paramActionEvent.getActionCommand())) {
  ...
  private void d(String paramString) {
    ...
    for (ItemListener itemListener : this.z) itemListener.itemStateChanged(itemEvent);
  }
