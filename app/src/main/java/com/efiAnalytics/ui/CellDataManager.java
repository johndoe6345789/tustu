...
public class b {
  ...
  public void a(Double paramDouble, double paramDouble1) {
    double d1;
    if (this.p == null) this.p = new c();
    if (paramDouble1 <= this.p.a() || this.o || Double.isNaN(paramDouble.doubleValue())) return;
    if (this.d >= this.p.i()) {
      d1 =
          (this.h * this.p.i() + paramDouble.doubleValue() * paramDouble1)
              / (this.p.i() + paramDouble1);
    } else if (Double.isNaN(this.h)) {
      d1 = paramDouble.doubleValue();
    } else {
      d1 =
          (this.h * (this.d + this.p.h()) + paramDouble.doubleValue() * paramDouble1)
              / (this.d + this.p.h() + paramDouble1);
    }
    ...
    if (paramDouble.doubleValue() > this.i) this.i = paramDouble.doubleValue();
    if (paramDouble.doubleValue() < this.j) this.j = paramDouble.doubleValue();
  ...
  public Double i() {
    return (this.p.c() > Math.abs(this.h - this.c))
        ? Double.valueOf(this.c)
        : ((Double.isNaN(this.h) || this.p.b() > this.d)
            ? null
            : (this.n ? new Double(Math.round(this.h)) : Double.valueOf(this.h)));
  }
