...
class Keypad extends JPanel {
  ...
  private final Key[][] b = new Key[][] { { Key.p, Key.q, Key.r, Key.t }, { Key.h, Key.i, Key.j, Key.k }, { Key.e, Key.f, Key.g, Key.l }, { Key.b, Key.c, Key.d, Key.m }, { Key.a, Key.s, Key.n, Key.o } };
  ...
  private void a() {
    ...
    Arrays.<Key[]>stream(this.b).flatMap(Arrays::stream).map(paramKey -> new Keypad$KeyButton(this, paramKey)).forEach(this::add);
  }
}
