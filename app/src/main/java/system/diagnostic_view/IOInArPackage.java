...
public class IOInArPackage implements DComponentCharlie {
  ...
  public void a(Properties paramProperties) {
    ...
    if (str == null) throw new ExceptionInDPackage(a + " is required");
    File file = new File(str);
    if (!file.exists()) throw new ExceptionInDPackage("Sound File not found.");
  ...
  public ArrayListExtensionInDPackage ExceptionInDPackage() {
    if (this.DComponentCharlie == null) {
      ...
      DInterfaceIndia.DComponentCharlie(
          "Set the full Path to the file you would like play when this action is triggered."
              + " Supportde formats are: wav, aiff, au and mp3. For MP3, you must have JavaFX"
              + " libraries. Windows and OS X installers always include these.");
