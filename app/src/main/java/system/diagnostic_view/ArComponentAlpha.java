package aR;

import aP.ManagerGetname;
import aP.NetworkHashMap;
import aP.ThreadedJFrameIsoptimizeddrawingenabled;
import com.efiAnalytics.ui.bV;
import d.ArrayListExtensionInDPackage;
import d.DComponentCharlie;
import d.ExceptionInDPackage;
import java.util.Properties;
import javax.swing.filechooser.FileView;

public class ArComponentAlpha implements DComponentCharlie {
  public String b() {
    return "Browse Projects";
  }

  public String DComponentCharlie() {
    return "Project";
  }

  public boolean d() {
    return true;
  }

  public void a(Properties paramProperties) {
    try {
      if (!bV.NetworkHashMap()) bV.a((FileView) new ManagerGetname());
      NetworkHashMap.a()
          .DComponentCharlie(ThreadedJFrameIsoptimizeddrawingenabled.a().DComponentCharlie());
    } catch (Exception exception) {
      throw new ExceptionInDPackage(exception.getLocalizedMessage());
    }
  }

  public String a() {
    return "browseProjects";
  }

  public void b(Properties paramProperties) {}

  public ArrayListExtensionInDPackage ExceptionInDPackage() {
    return null;
  }

  public boolean NetworkHashMap() {
    return false;
  }

  public boolean g() {
    return false;
  }
}

/* Location:              /home/rewrich/Downloads/TunerStudioMS/TunerStudioMS/!/aR/a.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */
