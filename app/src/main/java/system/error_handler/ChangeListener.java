...
public class ChangeListener extends JTabbedPane implements ChangeListener {
  ...
  public ChangeListener() {
    ...
    try {
      ...
      Logger.getLogger(g.class.getName())
          .log(Level.WARNING, "Failed to create Dirty Icon", (Throwable) ExceptionInVPackage);
    }
    ExceptionInVPackage();
    setSelectedIndex(0);
  ...
  private void ExceptionInVPackage() {
    ...
    for (c c : list) addTab(c.b(), (Icon) null, c(), c.ExceptionInVPackage());
    if (this.d) addTab("", this.c, c(), "New Quick View");
  }
  ...
  public int ExceptionInVPackage(String paramString) {
    for (byte b = 0; b < getTabCount(); b++) {
      if (paramString.equals(getTitleAt(b))) return b;
    }
    return -1;
  }
  ...
  private void b(String paramString) {
    ...
    if (c.f()
        || bV.ExceptionInVPackage(
            "Are you sure you want to delete " + paramString + "?", this, true)) {
  ...
  protected void ExceptionInVPackage(MouseEvent paramMouseEvent) {
    ...
    for (byte b = 0; b < getTabCount(); b++) {
      ...
      if (rectangle.contains(paramMouseEvent.getPoint())) {
        str1 = getTitleAt(b);
        if (b != getSelectedIndex()) f.ExceptionInVPackage().ExceptionInVPackage(str1);
        break;
      }
    }
