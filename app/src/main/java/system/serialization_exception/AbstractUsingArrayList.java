...
public abstract class AbstractUsingArrayList implements GInterfaceAf {
  ...
  protected abstract void ExceptionInVPackage(
      OutputStream paramOutputStream, byte[][] paramArrayOfbyte);
  ...
  public void ExceptionInVPackage(ExceptionInVPackage parama) {
    if (u()) throw new A("Can not modify LoggerFields while logging.");
    if (!this.ExceptionInVPackage.contains(parama)) this.ExceptionInVPackage.add(parama);
  }
  ...
  public synchronized void ExceptionInVPackage(String[] paramArrayOfString, String paramString) {
    ...
    for (byte b3 = 0; b3 < arrayOfR.length; b3++) {
      ...
      if (!arrayList2.contains(j)) j.b(this);
      arrayList2.add(j);
    ...
    try {
      ...
      String str2 =
          "Channels active on main config "
              + str1
              + ": "
              + ManagerUsingArrayList.ExceptionInVPackage().b(str1);
      D.d(str2);
    } catch (Exception exception) {
    }
  }
  ...
  protected void m() {
    ...
    try {
      for (byte b = 0; b < this.d.size(); b++) {
        ...
        if (r == null)
          throw new ExceptionInVPackage(
              "Configuration '" + str + "' not currently loaded. \nCan't stop data Log.");
    ...
    if (this.k != null) this.k.ExceptionInVPackage();
  }

  protected boolean ExceptionInVPackage(
      R paramR, SerializableImplExceptionprintstacktrace paramac) {
    String str = T.ExceptionInVPackage().c().equals(paramR) ? "" : paramR.c();
    return (!ExceptionInVPackage(str, this.r, paramac.b())
        && r.ExceptionInVPackage(paramR, paramac));
  }

  private boolean ExceptionInVPackage(
      String paramString1, String[] paramArrayOfString, String paramString2) {
    if (paramArrayOfString == null || paramString2 == null) return false;
    for (byte b = 0; b < paramArrayOfString.length; b++) {
      if ((paramString1 == null || paramString1.isEmpty())
          && paramArrayOfString[b].equals(paramString2)) return true;
      if (paramArrayOfString[b].equals(paramString1 + "." + paramString2)) return true;
    }
    return false;
  }
  ...
  private boolean ExceptionInVPackage(ArrayList paramArrayList, String paramString) {
    for (q q : paramArrayList) {
      if (q.ExceptionInVPackage().equals(paramString)) return true;
    }
    return false;
  }
  ...
  public synchronized void ExceptionInVPackage(String paramString, byte[] paramArrayOfbyte) {
    ...
    if (m == null || paramArrayOfbyte == null) return;
    ...
    try {
      ...
      if (v != null)
        ManagerUsingArrayList.ExceptionInVPackage()
            .ExceptionInVPackage("dataLogTime", v.ExceptionInVPackage());
      ...
      if ((System.nanoTime() - l2) > 4.0E7D) {
        ...
        D.c(
            "Time to write Data Log Record: "
                + numberFormat.format(l4 / 1000000.0D)
                + "ms. Time to notify Listeners: "
                + (l5 / 1000000.0D)
                + "ms. Prework time:"
                + (l3 / 1000000.0D));
      }
    ...
    if (this.n && q == 0) {
      try {
        ...
        D.ExceptionInVPackage(
            ExceptionInVPackage.getLocalizedMessage(), (Exception) ExceptionInVPackage, null);
      }
    } else if (this.n && q > 0) {
      q--;
    }
  ...
  private m ExceptionInVPackage(String paramString) {
    for (m m : this.d) {
      if (m.ExceptionInVPackage().equals(paramString)) return m;
    }
    return null;
  }
  ...
  protected void b(File paramFile) {
    if (this.t) {
      ...
      while (iterator.hasNext()) ((f) iterator.next()).ExceptionInVPackage(paramFile);
    }
  }
  ...
  protected void p() {
    if (this.t) {
      Iterator<f> iterator = e.iterator();
      while (iterator.hasNext()) ((f) iterator.next()).c();
    }
  }
  ...
  protected void q() {
    if (this.t) {
      Iterator<f> iterator = e.iterator();
      while (iterator.hasNext()) ((f) iterator.next()).d();
    }
  }
  ...
  protected void f(String paramString) {
    if (this.t) {
      Iterator<f> iterator = e.iterator();
      while (iterator.hasNext()) ((f) iterator.next()).b(paramString);
    }
  }
