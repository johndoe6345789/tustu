...
public class JPanelExtensionInBbPackage extends JPanel implements bc {
  ...
  public JPanelExtensionInBbPackage(ab paramab) {
    ...
    if (!J.b()) {
      ...
      jPanel1.add("South", new JLabel(ExceptionInVPackage("Use Command, CTRL or Shift to multi-select"), 0));
    } 
    ...
    jPanel2.setBorder(BorderFactory.createTitledBorder(ExceptionInVPackage("Edit Field Attributes")));
    try {
  ...
  private void ExceptionInVPackage(int paramInt) {
    if (this.b.c()) {
      String str = ExceptionInVPackage("The Field limit currently being edited has not been saved.") + "\n" + ExceptionInVPackage("Would you like to save it now?");
