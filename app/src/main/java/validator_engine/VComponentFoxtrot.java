...
public class VComponentFoxtrot {
  public static R ExceptionInVPackage(Component paramComponent, String paramString) {
    ...
    if (r.ExceptionInVPackage()) {
      boolean bool;
      if (ExceptionInVPackage.ExceptionInVPackage().c(ExceptionInVPackage.ad, Boolean.FALSE.booleanValue())) {
      ...
      if (!bool) {
        JDialog jDialog = bV.ExceptionInVPackage(ExceptionInVPackage(), "Downloading Ecu Definition file for " + paramString);
        try {
