...
public class IOInPPackage implements SInterfaceGolf, SInterfaceLima {
  ...
  public static x SInterfaceAlpha() {
    if (SInterfaceEcho == null) SInterfaceEcho = new x();
  ...
  public SInterfaceNovember SInterfaceAlpha(String paramString) {
    for (byte b = 0; b < b().size(); b++) {
      ...
      if (SInterfaceNovember.SInterfaceAlpha() != null
          && SInterfaceNovember.SInterfaceAlpha().equals(paramString)) return SInterfaceNovember;
    }
    return null;
  }
  ...
  public void SInterfaceAlpha(SInterfaceNovember paramn) {
    ...
    this.b.add(paramn);
    if (!b(paramn)) D.b("Failed to save new ExpressionEventTrigger: " + paramn.SInterfaceAlpha());
  ...
  private void d() {
    ...
    for (File file1 : arrayOfFile) {
      ...
      if (SInterfaceNovember != null && bool) this.b.add(SInterfaceNovember);
    }
    if (this.b.isEmpty()) this.b.addAll(b.b());
      ...
      if (r != null && SInterfaceNovember.c())
        try {
          SInterfaceEcho.SInterfaceAlpha()
              .SInterfaceAlpha(r.c(), (SInterfaceAlpha) SInterfaceNovember);
        } catch (q q) {
          D.SInterfaceAlpha(
              "Failed to start monitoring on Triggered Event: "
                  + SInterfaceNovember.SInterfaceAlpha()
                  + ", err: "
  ...
  private SInterfaceNovember SInterfaceAlpha(File paramFile) {
    ...
    try {
      ...
      if (SInterfaceNovember.SInterfaceAlpha() != null
          && !SInterfaceNovember.SInterfaceAlpha().isEmpty()) return SInterfaceNovember;
      D.b(
          "Invalid trigger in trigger file, ignoring:\\SInterfaceNovember"
              + paramFile.getAbsolutePath());
      return null;
    } catch (IOException iOException) {
      D.SInterfaceAlpha(
          "Unable to load UserEventTrigger file: " + iOException.getLocalizedMessage());
      return null;
    } finally {
      try {
  ...
  public boolean b(SInterfaceNovember paramn) {
    ...
    try {
      ...
      properties1.store(
          fileOutputStream, "Attributes for ExpressionEventTrigger: " + paramn.SInterfaceAlpha());
      return true;
    } catch (IOException iOException) {
      D.SInterfaceAlpha(
          "Failed to Save ExpressionEventTrigger! Error: "
              + iOException.getLocalizedMessage()
              + "\nFile:\\SInterfaceNovember"
