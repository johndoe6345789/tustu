...
public class GInterfaceCe {
  public static void a(R paramR, aM paramaM, String paramString) {
    if (paramString != null && paramString.indexOf("\"") != -1) {
      try {
        if (paramaM.C()) paramR.a(paramaM.aL(), paramString);
      } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
        D.b(
            "Attempt to set Parameter value, but not valid for current firmware: "
                + ExceptionPrintstacktrace.getMessage());
      } catch (Exception exception) {
        throw new VInterfaceIndia(
            "Error setting value: " + paramaM.aL() + " to value: " + paramString);
      }
        ...
        if (paramaM != null && paramaM.C() && !paramaM.VInterfaceIndia().equals("string")) {
          try {
            ...
            if (!paramaM.aL().startsWith("UNALLOCATED_SPACE"))
              throw new VInterfaceIndia(
                  "Invalid table data in Constant: " + paramaM.aL() + ", not loading values.");
          }
          A a = paramaM.c();
          boolean bool = false;
          ...
          if (!bool) paramR.a(paramaM.aL(), arrayOfDouble);
        }
      } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
        D.b(
            "Failed to set value for "
  ...
  public static String a(R paramR, aM paramaM) {
    try {
      if (paramaM.VInterfaceIndia().equals("bits")) return paramaM.f(paramR.p());
      if (paramaM.VInterfaceIndia().equals("string")) return paramaM.d(paramR.p());
      if (paramaM.VInterfaceIndia().equals("scalar")
          || paramaM.VInterfaceIndia().equals(ca.e)
          || paramaM.VInterfaceIndia().equals(ca.d)) {
