...
public class SerializableImpl implements GInterfaceCr, Serializable {
  ...
  public SerializableImpl() {
    ...
    for (byte b = 0; b < arrayOfString.length; b++) ManagerUsingArrayList.a().d(arrayOfString[b]);
    this.f.b();
  }

  public boolean b(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2) {
    int j = d(paramArrayOfbyte2);
    if ((j & 0x80) != 128) return true;
    if (j == 128) {
  ...
  public boolean a(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2) {
    this.j = null;
    if (!b(paramArrayOfbyte1, paramArrayOfbyte2)) return false;
    int j = d(paramArrayOfbyte2);
    ...
    try {
      ...
      if (k != paramArrayOfbyte2.length) {
        ...
        D.b(
            "Unexpeced array length from controller! Envelope declared size:"
                + (1 + k)
                + " actual measured size:"
                + (paramArrayOfbyte2.length - 6)
      ...
      if (!c.b(arrayOfByte3, arrayOfByte4)) {
        ...
        this.f.C();
        D.b(
            "CRC failure on received payload.\n\tReported CRC: "
                + c.d(arrayOfByte3)
                + "\n\tCalculated CRC:"
                + c.d(arrayOfByte4)
                + "\n\tResponse data:\n"
  ...
  public String c(byte[] paramArrayOfbyte) {
    ...
    if (j == 3) {
      ...
      if (arrayOfByte.length > 0) return new String(arrayOfByte);
    ...
    if (j == 146) {
      ...
      return "A Controller is responding, but not at the project assigned CAN ID.\n\n"
                 ...
                 + "- The CAN ID assigned in Project Settings matches that of your target"
                 + " controller.\n\n"
                 + "Please correct the problem and power cycle the device you are connected to.Then"
                 + " uncheck Work Offline";
    }
    if (j == 148) {
      byte[] arrayOfByte = e(paramArrayOfbyte);
      String str =
          (arrayOfByte.length > 0) ? new String(arrayOfByte) : "No controller Message recieved";
      if (arrayOfByte.length > 0) {
    ...
    return (this.b == 5)
        ? "Failed to get a valid read of OutputChannel data in 5 consecutive attempts.\n"
              + "Please check your project settings and ensure you have the correct ini for your"
              + " project.\n\n"
