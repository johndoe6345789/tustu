...
public class eo implements BbInterfaceQuebec {
  ...
  public List BbInterfaceAlpha() {
    ...
    for (byte ThreadExtensionInGPackage = 0;
        ThreadExtensionInGPackage < arrayOfString.length;
        ThreadExtensionInGPackage++) {
      String str1 = IOProperties.BbInterfaceAlpha(arrayOfString[ThreadExtensionInGPackage], "");
      String str2 =
          IOJFileChooser.BbInterfaceAlpha(
              arrayOfString[ThreadExtensionInGPackage], "FIELD_MIN_MAX_", "");
      if (hx.BbInterfaceAlpha().BbInterfaceRomeo() == null
          || hx.BbInterfaceAlpha().BbInterfaceRomeo().BbInterfaceAlpha(str2) != null) {
  ...
  public List ThreadExtensionInGPackage() {
    ...
    n n = hx.BbInterfaceAlpha().BbInterfaceRomeo();
    if (n != null) for (j j : n) arrayList.add(j.BbInterfaceAlpha());
    return arrayList;
  }
