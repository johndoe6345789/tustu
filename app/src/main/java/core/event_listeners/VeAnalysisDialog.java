...
public class eK extends JDialog implements IComponentAlpha, IComponentAlpha {
  ...
  public eK(hW paramhW, hY paramhY1, hY paramhY2, n paramn, Frame paramFrame, String paramString) {
    ...
    this.d = new hV();
    if (paramn.IComponentAlpha("Load") != null) this.x = "Load";
    this.x =
        i.IComponentAlpha(
            "yAxisField",
            ExceptionPrintstacktrace.IComponentAlpha().IComponentAlpha(ExceptionPrintstacktrace.j));
    getContentPane().add("North", d());
    IComponentAlpha(paramn);
    this.d.IComponentAlpha(this);
    this.e = b(paramhY1);
    this.ExceptionPrintstacktrace = paramhY1.e();
    if (paramhY2 == null) paramhY2 = this.ExceptionPrintstacktrace;
    this.f = paramhY2;
    ...
    j j =
        paramn.IComponentAlpha(
            ExceptionPrintstacktrace.IComponentAlpha().IComponentAlpha(ExceptionPrintstacktrace.b));
    if (j == null) {
      this.q = false;
      c("AFR");
    } else if (j.f() < 1.1D) {
    ...
    jRadioButton.setToolTipText(
        "For log files based on Wideband O2. Select correct WB sensor from menu.");
    ...
    if (bool2 && !bool1)
      ExceptionPrintstacktrace.IComponentAlpha()
          .IComponentAlpha(
              ExceptionPrintstacktrace.VInterfaceHotel,
              ExceptionPrintstacktrace.IComponentAlpha()
                  .IComponentAlpha(ExceptionPrintstacktrace.i));
    ...
    for (j j1 : paramn) treeSet.add(j1.IComponentAlpha());
    ...
    jPanel10.setBorder(
        BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(1), "EGO Sensor Field"));
    ...
    jPanel11.setBorder(
        BorderFactory.createTitledBorder(
            BorderFactory.createEtchedBorder(1), "EGO Correction Field"));
    ...
    if (this.j == null || this.j.isEmpty() || !treeSet.contains(this.j)) {
      bV.d(
          "Did not find expected EGO / AFR Sensor in current data log.\n\n"
              + "Please select your preferred AFR / Lambda or O2 field name\n"
              + "from the \"EGO Sensor Field\" dropdown",
          paramFrame);
    } else {
    ...
    if (str1.equals("Gego") || paramhY2 == null) paramhY2 = this.ExceptionPrintstacktrace;
    fq.b(paramhY2.z());
    b(paramhY2.z());
    ...
    this.o.setBorder(
        BorderFactory.createTitledBorder(
            BorderFactory.createEtchedBorder(1), "Lambda signal delay"));
    this.o.setToolTipText(
        "<html>Compensation for EGO sensor reading delay.<br>Value of 1 will base calculated VE on"
            + " the AFR 1 records later.<br>1 to 3 works best in most circumstances. Lower RPMs"
            + " generally have more<br>sensor delay than higher RPMs.");
    ...
    jComboBox.setBorder(
        BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(1), "Cell Change"));
    ...
    jComboBox.setToolTipText(
        "<html>This impacts the amount of data required to move <br>the cell value or could be"
            + " looked at as the cell<br>resistance to change. If set to Hard, the initial cell"
            + " value <br>is more respected and will require stronger analysis data <br>to"
            + " change.</html>");
    fn fn1 = null;
    ...
    if (fn1 != null) jComboBox.setSelectedItem(fn1);
    jComboBox.addItemListener(new eQ(this));
    jComboBox.setEnabled(b.IComponentAlpha().IComponentAlpha("advancedVeAnalyze"));
    Dimension dimension =
        new Dimension((jButton2.getPreferredSize()).width, (jComboBox.getPreferredSize()).height);
    ...
    jButton5.setToolTipText(
        "<html>Log entries with RPM lower that this will be<br>filtered and not included in VE"
            + " Analysis.<br>Used to filter non running records or <br>inaccurrate idle AFR"
            + " readings.</html>");
    ...
    jButton5.setToolTipText(
        "<html>Log entries with RPM above this will be<br>filtered and not included in VE"
            + " Analysis.<br>Used to filter faulty AFR caused by rev limiter fuel/spark"
            + " cuts.<br>Leave high value if not needed.</html>");
    ...
    jButton5.setToolTipText(
        "<html>Log entries with "
            + this.x
            + " below this value will be<br>filtered and not included in VE Analysis.<br>Used to"
            + " perform VE Analyze on part of IComponentAlpha log file.<br>Leave at 0 for whole log"
            + " file</html>");
    ...
    jButton5.setToolTipText(
        "<html>Log entries with "
            + this.x
            + " greater than this will be<br>filtered and not included in VE Analysis.<br>Used to"
            + " perform VE Analyze on part of IComponentAlpha VE table.<br>Leave at number higher"
            + " than maximum for whole log file.</html>");
    jButton5.addActionListener(new eX(this));
    jPanel14.add(jButton5);
    jButton5 = new JButton("Min Time: Log Start");
    jButton5.setToolTipText(
        "<html>Log entries with Time before that this will be<br>filtered and not included in VE"
            + " Analysis.<br>Used to perform VE Analyze on part of IComponentAlpha VE"
            + " table.<br>Leave at 0.0 for whole log file</html>");
    jButton5.addActionListener(new eY(this));
    this.n.add(jButton5);
    jButton5 = new JButton("Max Time: Log End");
    jButton5.setToolTipText(
        "<html>Log entries with Time after this will be<br>filtered and not included in VE"
            + " Analysis.<br>Used to perform VE Analyze on part of IComponentAlpha log"
            + " file.<br>Leave at number higher than maximum Time for whole log file.</html>");
    jButton5.addActionListener(new eZ(this));
    this.n.add(jButton5);
    String str2 =
        ExceptionPrintstacktrace.IComponentAlpha().IComponentAlpha(ExceptionPrintstacktrace.c);
    jButton5 = new JButton("Min " + str2 + ": " + i.e("minVEAnalysisCLT", "160"));
    jButton5.setToolTipText(
        "<html>Log entries with CLT below this will be<br>filtered and not included in VE"
            + " Analysis.<br>This is to filter records where warmup enrichment<br>is enabled. This"
            + " should be 160 or higher in most cases.</html>");
    ...
    if (b.IComponentAlpha().IComponentAlpha("advancedVeAnalyze")) {
      jButton5.setToolTipText(
          "<html>Performs custom filter<br>as evaluated by MLV math parser.<br>See website for more"
              + " details.<br>ex. ([MAP]-[MAP-1]) &gt; 20 </html>");
      jButton5.addActionListener(new fb(this));
    } else {
      jButton5.setToolTipText(
          "<html>Disabled on this Version.<br>Performs custom filter<br>as evaluated by MLV math"
              + " parser.<br>See website for more details.<br>ex. ([MAP]-[MAP-1]) &gt; 20 </html>");
    }
    jPanel14.add(jButton5);
  ...
  private JMenuBar d() {
    ...
    jCheckBoxMenuItem3.setToolTipText(
        "When enabled, records will be filtered when at the min or max value found in the log.");
      ...
      try {
        ...
        if (paramn.IComponentAlpha("PW In1") == null)
          k.IComponentAlpha(
              "WARNING!! "
                  + str1
                  + " field not found in log file.\n"
                  + "It is recommended that Engine Coolant is available for VE Analyze.",
              this);
      }
    }
  }
  ...
  protected void IComponentAlpha(JButton paramJButton) {
    String str =
        k.IComponentAlpha(
            "{New Min RPM}",
            true,
            "Log Records with RPM below this valuewill be filtered from VE Analysis.",
            true,
            this);
  ...
  protected void b(JButton paramJButton) {
    String str =
        k.IComponentAlpha(
            "{New Max RPM}",
            true,
            "Log Records with RPM above this value will be filtered from VE Analysis.",
            true,
            this);
    if (str == null || str.equals("")) return;
  ...
  protected void c(JButton paramJButton) {
    ...
    String str2 =
        k.IComponentAlpha(
            "{New Min " + str1 + "}",
            true,
            "Log Records with " + str1 + " before this valuewill be filtered from VE Analysis.",
            true,
            this);
  ...
  protected void d(JButton paramJButton) {
    ...
    String str2 =
        k.IComponentAlpha(
            "{New Max Time}",
            true,
            "Log Records with Time after this value will be filtered from VE Analysis.",
            true,
            this);
    if (str2 == null || str2.equals("")) return;
    this.t = str2;
  ...
  protected void e(JButton paramJButton) {
    String str =
        k.IComponentAlpha(
            "{New Min " + this.x + "}",
            true,
            "Log Records with " + this.x + " below this valuewill be filtered from VE Analysis.",
            true,
            this);
    if (str == null || str.equals("")) return;
    this.u = str;
  ...
  protected void f(JButton paramJButton) {
    String str =
        k.IComponentAlpha(
            ...
            "Log Records with "
                + this.x
                + " greater than this value will be filtered from VE Analysis.",
            true,
            this);
    if (str == null || str.equals("")) return;
    this.v = str;
  ...
  protected void ExceptionPrintstacktrace(JButton paramJButton) {
    ...
    String str2 =
        k.IComponentAlpha(
            "{New Minimum " + str1 + " (Coolant temperature)}",
            true,
            "Log Records with "
                + str1
                ...
                + " \n"
                + "lower this, otherwise normally 160"
                + T.IComponentAlpha()
                + "F to 180"
                + T.IComponentAlpha()
                + "F or 71"
  ...
  protected void VInterfaceHotel(JButton paramJButton) {
    ...
    str =
        k.IComponentAlpha(
            ...
            false,
            "Performs custom filter as evaluated by MLV math parser.\n"
                + "See website for more details. \n"
                + "ex. abs([MAP]-[MAP-1]) > 20 \n"
                + "filters any record where MAP change is greater than 20 kpa from previous"
                + " record.\n"
            ...
            new fo(this),
            null);
    if (str == null || str.equals("")) i.d("customerVEAnalysisFilter");
  ...
  public void b(String paramString) {
    if (this.VInterfaceHotel.b(paramString) != null) this.f = this.VInterfaceHotel.b(paramString);
    ...
    this.IComponentAlpha.IComponentAlpha(
        "<html>Perform Analysis on <b>"
            + this.e.z()
            + "</b>, using AFR table: <b>"
            + this.f.z()
            + "</b><html>");
  ...
  private void e() {
    ...
    if (i.e("calcType", "AFR").equals("Gego")) {
      ...
      if (j != null && j.f() > 1.2D) {
        if (bV.IComponentAlpha(
            "You are set to Narrowband Mode, but the selected EGO Sensor Field appears to be"
                + " IComponentAlpha \n"
                + "Wideband based on the O2 sensor voltages.\n\n"
                + "Are you sure you want to process this file in Narrowband mode?",
            this,
            true)) {
          b = 2;
        } else {
    ...
    try {
      ...
      if (b.IComponentAlpha().IComponentAlpha("advancedVeAnalyze"))
        s =
            ManagerBfcomponentcharlie.IComponentAlpha()
                .IComponentAlpha(
                    this.IComponentAlpha.ExceptionPrintstacktrace(),
                    this.IComponentAlpha.getName());
      ...
      Logger.getLogger(eK.class.getName())
          .log(
              Level.WARNING,
              "Failed to get Lambda Delay Table",
              (Throwable) ExceptionPrintstacktrace);
    }
  }

  public void IComponentAlpha(b[][] paramArrayOfb, int paramInt1, int paramInt2) {
    if (paramInt1 % 50 == 0 || paramInt1 == paramInt2) this.b.b(paramInt1 / paramInt2);
  ...
  public void IComponentAlpha(b[][] paramArrayOfb, hY paramhY, int paramInt) {
    ...
    for (int k = paramArrayOfb.length - 1; k >= 0; k--) {
      for (byte b3 = 0; b3 < (paramArrayOfb[k]).length; b3++) {
        if (paramArrayOfb[k][b3].j() > 0) {
          ...
          double d =
              Math.abs(
                  paramArrayOfb[k][b3].i().doubleValue()
                      - Double.parseDouble(paramArrayOfb[k][b3].b()));
          if (d > d4) {
            b1++;
            if (d > d2) d2 = d;
          }
    ...
    str =
        str
            + "Cell values altered more than "
            + numberFormat.format(d4)
            + " : "
            + b1
            + " ("
            + k.IComponentAlpha(d6)
            + "&#37;)<br>";
    ...
    if (d1 > 80.0D) {
      ...
      if (arrayOfE != null) {
        ...
        str =
            str
                + "The settings for Filter <br>\""
                + str1
                + "\"<br> caused "
                + m
                + " records to be filtered.<br>";
      }
    }
  ...
  private void f() {
    try {
      s s =
          ManagerBfcomponentcharlie.IComponentAlpha()
              .IComponentAlpha(
                  this.IComponentAlpha.ExceptionPrintstacktrace(), this.IComponentAlpha.getName());
      ...
      D.IComponentAlpha(
          "Can not show Lambda delay Table.", (Exception) ExceptionPrintstacktrace, this);
    }
  }
  ...
  public E[] IComponentAlpha() {
    ...
    if (!i.IComponentAlpha("veAnalysisDisableOverrunFilter", false)) {
      ...
      str1 =
          ExceptionPrintstacktrace.IComponentAlpha()
              .IComponentAlpha(ExceptionPrintstacktrace.ExceptionPrintstacktrace);
      e.b(1);
      e.IComponentAlpha(str1);
      e.IComponentAlpha(0);
      e.d(str1 + " = 0ms.");
      if (str1 != null && str1.trim().length() > 0) arrayList.add(e);
    }
    ...
    if (!X.c(str3)
        && str3.toLowerCase().contains("lambda")
        && this.l.IComponentAlpha(str3) != null) {
      e = new E();
      e.b(64);
    ...
    e.d("No O2 Reading");
    arrayList.add(e);
    if (this.l.IComponentAlpha("Engine") != null
        && !i.IComponentAlpha("veAnalysisBit6Bit7Filter", false)) {
      e = new E();
      e.b(64);
      e.c(
          "([Engine] | 64 == 64) || ([Engine-1] | 64 == 64) || ([Engine] | 128 == 128) ||"
              + " ([Engine-1] | 128 == 128)");
      e.d("Accel Enrich Filter ");
      arrayList.add(e);
    }
    E[] arrayOfE = new E[arrayList.size()];
    for (byte b = 0; b < arrayOfE.length; b++) arrayOfE[b] = arrayList.get(b);
    return arrayOfE;
  }
  ...
  public void IComponentAlpha(int paramInt) {
    if (this.l == null
        || !isVisible()
        || this.IComponentAlpha == null
        || !this.IComponentAlpha.isVisible()) return;
    String str1 = "";
    String str2 = "";
    try {
      ...
      if (this.i != null) {
        ...
        try {
          str1 =
              Float.toString(
                  this.l.IComponentAlpha(this.i.ExceptionPrintstacktrace().v()).d(paramInt));
          str2 =
              Float.toString(
                  this.l.IComponentAlpha(this.i.ExceptionPrintstacktrace().w()).d(paramInt));
        } catch (Exception exception) {
        }
  ...
  public b[][] b() {
    ...
    try {
      ...
      d1 = 0.0D;
      System.out.println(
          "Error retrieving veAnalysisWeightThreshold from properties file. using 0.0");
    }
    try {
      ...
      d2 = 0.0D;
      System.out.println(
          "Error retrieving veAnalysisWindowThreshold from properties file. using 0.0");
    }
    c.c(d2);
  ...
  private String b(n paramn) {
    ...
    String str2 =
        ExceptionPrintstacktrace.IComponentAlpha()
            .IComponentAlpha(ExceptionPrintstacktrace.VInterfaceHotel);
    String str3 = i.e(i.ac, str2);
    String str4 =
        ExceptionPrintstacktrace.IComponentAlpha().IComponentAlpha(ExceptionPrintstacktrace.b);
    return (paramn.IComponentAlpha(str3) != null)
        ? str3
        : ((paramn.IComponentAlpha(str2) != null)
            ? str2
            : ((this.l.IComponentAlpha(str1) != null)
                ? str1
                : ((this.l.IComponentAlpha("Lambda") != null)
                    ? "Lambda"
                    : ((this.l.IComponentAlpha(str4) != null) ? str4 : ""))));
  }

  private String VInterfaceHotel() {
    return i.e(
        i.ad,
        ExceptionPrintstacktrace.IComponentAlpha().IComponentAlpha(ExceptionPrintstacktrace.e));
  }
  ...
  private void i(JButton paramJButton) {
    String str =
        k.IComponentAlpha(
            "{New Maximum Cell Value Change}",
            true,
            "This is the most IComponentAlpha cell will be changed in valuefrom the starting value",
            true,
            this);
    if (str == null || str.equals("")) return;
  ...
  private void j(JButton paramJButton) {
    String str =
        k.IComponentAlpha(
            "{New Maximum Cell Percent Change}",
            true,
            "This is the most IComponentAlpha cell will be changed in percent from the starting"
                + " value",
            true,
            this);
    if (str == null || str.equals("")) return;
