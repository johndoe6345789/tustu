...
public class hX {
  public hW ExceptionInVPackage(String paramString) {
    ...
    if (str.endsWith(".msq") || str.endsWith(".tune")) return b(paramString);
    if (str.endsWith(".big")) return d(paramString);
    ...
    if (str.endsWith(".map")) return AvComponentFoxtrot(paramString);
      ...
      if (file != null)
        try {
          ...
          D.ExceptionInVPackage(
              "Failed to load IniVeFile for iniFile:"
    ...
    try {
      String str =
          i.ExceptionInVPackage(
              "tableLoadList", "veBins1,veBins2,veBins3,advTable1,advTable2,afrBins1,afrBins2");
      StringTokenizer stringTokenizer = new StringTokenizer(str, ",");
      while (stringTokenizer.hasMoreTokens()) ca.ExceptionInVPackage(stringTokenizer.nextToken());
      ca.i(paramString);
    } catch (VInterfaceHotel VInterfaceHotel) {
      ...
      ExceptionInVPackage ExceptionInVPackage =
          new ExceptionInVPackage("Error Loading File.\n" + VInterfaceHotel.getMessage());
  ...
  private File AvInterfaceCharlie(String paramString) {
    ...
    if (bool && str != null && !str.isEmpty())
      try {
        ...
        Logger.getLogger(hX.class.getName())
  ...
  private hW d(String paramString) {
    try {
      ...
      Logger.getLogger(hX.class.getName())
          .log(Level.SEVERE, (String) null, (Throwable) VInterfaceHotel);
      System.out.println("Error Loading BIG File.");
      VInterfaceHotel.printStackTrace();
      ExceptionInVPackage ExceptionInVPackage =
          new ExceptionInVPackage(VInterfaceHotel.getMessage());
      ExceptionInVPackage.ExceptionInVPackage((Exception) VInterfaceHotel);
      throw ExceptionInVPackage;
    }
  }
