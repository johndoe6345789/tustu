...
class cT extends Thread {
  ...
  public void run() {
    ...
    try {
      file =
          new File(
              HInterfaceHotel.ExceptionInVPackage(),
              str1 + "SupportPkg" + System.currentTimeMillis());
      t.b(file);
      file.mkdir();
      ...
      if (this.ExceptionInVPackage.t != null
          && this.ExceptionInVPackage.t.exists()
          && this.ExceptionInVPackage.t.isFile()) {
        D.d(
            "Making copy of last opened log file "
                + this.ExceptionInVPackage.t.getName()
                + " log file for Support Package.");
        try {
          t.ExceptionInVPackage(
              this.ExceptionInVPackage.t, new File(file, this.ExceptionInVPackage.t.getName()));
        } catch (ExceptionInVPackage ExceptionInVPackage) {
          Logger.getLogger(b.class.getName())
              .log(Level.SEVERE, (String) null, (Throwable) ExceptionInVPackage);
        }
      ...
      String str =
          "A Debug Package named "
              + file3.getName()
              + " has been created.\n\nYou will find it "
              + (bool ? "on your desktop" : "in your home folder")
              + ".\n\n"
