...
public class bm {
  ...
  public void a(boolean paramBoolean) {
    for (bl bl : this.b) bl.d(paramBoolean);
  }

  public void a(String paramString, j paramj) {
    if (paramString.indexOf('.') == -1) a(paramString, paramj, 0);
    String str = paramString.substring(0, paramString.lastIndexOf('.'));
    int i =
        Integer.parseInt(
            paramString.substring(paramString.lastIndexOf('.') + 1, paramString.length()));
    a(str, paramj, i);
  }
  ...
  private void b(String paramString, j paramj, int paramInt) {
    for (bl bl : this.b) bl.a(paramString, paramj, paramInt);
  }
