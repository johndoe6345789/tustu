...
public class hg extends JComponent {
  ...
  public void paint(Graphics paramGraphics) {
    ...
    paramGraphics.drawLine(
        (getInsets()).left + 1,
        getHeight() / 2 - 1,
        getWidth() - (getInsets()).left - 2,
        getHeight() / 2 - 1);
    paramGraphics.setColor(Color.lightGray);
    this.b.setLocation(c(), d());
    paramGraphics.fill3DRect(
        this.b.x, this.b.y, this.IComponentAlpha.width, this.IComponentAlpha.height, true);
  }

  private int c() {
    return (int) ((getWidth() - (getInsets()).left - (getInsets()).right) * this.d)
        + (getInsets()).left
        - this.IComponentAlpha.width / 2;
  }
  ...
  private void e(int paramInt) {
    for (IComponentAlpha IComponentAlpha : this.h) IComponentAlpha.IComponentAlpha(paramInt);
  }
