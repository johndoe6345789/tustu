...
public class by extends JPanel {
  ...
  public by(List paramList, boolean paramBoolean) {
    ...
    for (bB bB : paramList) {
      ...
      if (!bB.c().isEmpty()) jPanel.add("East", (Component) new cF(bB.c(), null));
      jPanel1.add(jPanel);
    }
    if (!paramBoolean) {
      JPanel jPanel = new JPanel();
      ...
      jPanel.add(
          ...
          (Component)
              new cF(
                  "Replace Current Settings: Settings for each category will be removed and"
                      + " replaced with those in this import file.<br>Merge with Current Settings:"
                      + " Current settings will be left in place and only overwridden when a"
                      + " setting with the same name is imported.",
                  null));
      if (jPanel1.getComponentCount() % 2 > 0) jPanel1.add(new JLabel(""));
      jPanel1.add(jPanel);
    }
    add("Center", jPanel1);
  ...
  public boolean a() {
    return (this.d.getSelectedItem() != null
        && this.d.getSelectedItem().equals("Replace Current Settings"));
  }
  ...
  public List b() {
    ...
    for (bB bB : this.a) {
      if (a(bB.a())) {
        arrayList.add(bB.a());
        for (String str : bB.d()) arrayList.add(str);
      }
    }
    return arrayList;
  }
  ...
  private boolean a(String paramString) {
    for (JCheckBox jCheckBox : this.b) {
      if (jCheckBox.getActionCommand().equals(paramString)) return jCheckBox.isSelected();
    }
    return false;
  }
