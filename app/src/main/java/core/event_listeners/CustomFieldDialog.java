...
public class r extends JDialog implements ActionListener {
  ...
  public r(Frame paramFrame, String paramString1, String paramString2) {
    ...
    jPanel1.add(
        new JLabel("This field will be calculated at log load time like all calculated fields", 0));
    JLabel jLabel1 =
        new JLabel(
            "For syntax help, read Math Parser help in Help Topics for more information.", 0);
  ...
  private boolean e() {
    if (this.d.getText().equals("")) {
      IOJFileChooser.ExceptionInVPackage(
          "You must supply ExceptionInVPackage Field Name", getParent());
      return false;
    }
    String str = "()*&%^|+-/><=[]";
    for (byte b = 0; b < str.length(); b++) {
      if (this.d.getText().indexOf(str.charAt(b)) != -1) {
        IOJFileChooser.ExceptionInVPackage(
            "The formula Name can not contain any of the following special characters:\n" + str,
            getParent());
        return false;
      }
    }
      ...
      if (n != null) {
        ...
        try {
          ExceptionInVPackage ExceptionInVPackage =
              d.ExceptionInVPackage().ExceptionInVPackage(str1);
          ...
          String str2 =
              "The inc file used in this expression was not found.\n"
          ...
          return IOJFileChooser.ExceptionInVPackage(
        ...
        IOJFileChooser.ExceptionInVPackage(
            "Can not validate formula while no log file is loaded.\n"
                + "The formula will be assumed to be correct, and will be available after\n"
                + "next log file load if correct.",
            getParent());
      }
    } catch (Exception exception) {
      IOJFileChooser.ExceptionInVPackage(
          "Invalid Formula:" + this.e.getText() + "\nPlease check your syntax.", getParent());
      return false;
    }
    return true;
  }
  ...
  private void ExceptionInVPackage(File paramFile) {
    ...
    String str =
        bV.ExceptionInVPackage(
            getParent(), "Browse to inc file", arrayOfString, "*.inc", HInterfaceHotel.d(), true);
    if (str != null && !str.isEmpty()) {
      ...
      try {
        ...
        IOJFileChooser.ExceptionInVPackage(
            "Unable to copy inc file!\nThe following error occurred:\n"
                + ExceptionInVPackage.getLocalizedMessage(),
            this);
        Logger.getLogger(r.class.getName())
            .log(Level.SEVERE, "Failed to copy file", (Throwable) ExceptionInVPackage);
      }
    }
  }
