...
public class b {
  ...
  public static b AmInterfaceAlpha() {
    if (AmInterfaceAlpha == null) AmInterfaceAlpha = new b();
  ...
  public void b(Window paramWindow) {
    JPanelExtensionInBxPackage JPanelExtensionInBxPackage =
        new JPanelExtensionInBxPackage((bx.b) M.AmInterfaceAlpha(), null);
  ...
  public void JPanelExtensionInBxPackage(Window paramWindow) {
    JPanelExtensionInBbPackage JPanelExtensionInBbPackage =
        new JPanelExtensionInBbPackage(null, new eo());
  ...
  public void AmInterfaceAlpha(n paramn, File paramFile, Window paramWindow) {
    ...
    n1.b(paramn.b());
    for (j j : paramn) n1.add(j);
    if (paramn.HInterfaceHotel())
      for (String str1 : paramn.i()) n1.AmInterfaceAlpha(str1, paramn.AmInterfaceFoxtrot(str1));
    File file = paramFile.getParentFile();
    String str2 = "msl";
    String str3 =
        (paramn.JPanelExtensionInBbPackage() == null
                || paramn.JPanelExtensionInBbPackage().isEmpty())
            ? i.v
            : paramn.JPanelExtensionInBbPackage();
    if (str3.equals(",")) str2 = "csv";
    if (paramFile.getName().toLowerCase().endsWith(str2)) {
      try {
        str1 =
            paramFile.getName().substring(0, paramFile.getName().lastIndexOf("."))
                + "_modified."
                + str2;
  ...
  public void b(Frame paramFrame) {
    ...
    try {
      if (J.AmInterfaceAlpha()) {
        ...
        str3 = "/usr/bin/java";
        str2 =
            "/usr/bin/java -cp .:./plugins/:lib:./lib/*.jar -Djava.library.path=lib -jar " + str1;
        String[] arrayOfString = {str3, "Staging", str2};
        Runtime.getRuntime().exec(arrayOfString);
      } else if (J.b()) {
        str3 = "./runtime/bin/java";
        str2 =
            "./runtime/bin/java -cp .:./plugins/:lib:./lib/*.jar -Djava.library.path=lib -jar "
                + str1;
        String[] arrayOfString = {str3, "Staging", str2};
        Runtime.getRuntime().exec(arrayOfString);
      } else {
  ...
  public void b() {
    boolean bool =
        (J.AmInterfaceAlpha() && UIManager.getLookAndFeel().isNativeLookAndFeel()) ? true : false;
    try {
    ...
    if (font1 != null && font1.getSize() < 14) eJ.JPanelExtensionInBxPackage(font1.getSize());
        ...
        if (font != null) {
          ...
          Font font3 =
              new Font(
                  font.getFamily(),
                  font.getStyle(),
                  Math.round(eJ.AmInterfaceAlpha(float_.floatValue() * AmInterfaceFoxtrot)));
  ...
  public JPanelExtensionUsingRunnable AmInterfaceAlpha(Window paramWindow, String paramString) {
    JPanelExtensionUsingRunnable JPanelExtensionUsingRunnable =
        new JPanelExtensionUsingRunnable(null);
    JPanelExtensionUsingRunnable.AmInterfaceAlpha(
        Z.b.AmInterfaceAlpha().JPanelExtensionInBxPackage());
    JPanelExtensionUsingRunnable.AmInterfaceAlpha(new e(this));
    if (paramString != null) JPanelExtensionUsingRunnable.AmInterfaceAlpha(paramString);
  ...
  public void e(Window paramWindow) {
    com.efiAnalytics.dialogs.b b1 =
        new com.efiAnalytics.dialogs.b(paramWindow, "Timeslip Editor", null);
