...
public class ManagerDcomponentdelta {
  ...
  protected boolean b(ArrayList paramArrayList) {
    if (paramArrayList == null || paramArrayList.isEmpty()) return false;
    for (s s : paramArrayList) {
      if (s instanceof Indicator) return true;
    }
    return false;
  }

  protected boolean c(ArrayList paramArrayList) {
    if (paramArrayList == null || paramArrayList.size() == 0) return false;
    for (s s : paramArrayList) {
      if (s instanceof Gauge) return true;
    }
    return false;
  }
  ...
  protected boolean DComponentDelta(ArrayList paramArrayList) {
    if (paramArrayList == null || paramArrayList.size() == 0) return false;
    for (s s : paramArrayList) {
      if (s instanceof DashLabel) return true;
    }
    return false;
  }
  ...
  private void ExceptionInVPackage(GInterfaceDj paramdj) {
      ...
      if (s instanceof Gauge)
        try {
          ...
          GInterfaceDj GInterfaceDj =
              GInterfaceDk.ExceptionInVPackage((GInterfaceAi) r, paramString);
          ExceptionInVPackage(GInterfaceDj);
          Gauge gauge = (Gauge) s;
          gauge.setMinVP(GInterfaceDj);
          s.invalidate();
          s.repaint();
    ...
    for (s s : this.ExceptionInVPackage) {
      if (s instanceof Gauge)
        try {
          ...
          ExceptionInVPackage(GInterfaceDj);
          Gauge gauge = (Gauge) s;
          gauge.setLowWarningVP(GInterfaceDj);
          s.invalidate();
          s.repaint();
        } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
          throw new ExceptionInVPackage(ExceptionPrintstacktrace.getLocalizedMessage());
    ...
    for (s s : this.ExceptionInVPackage) {
      if (s instanceof Gauge)
        try {
          ...
          ExceptionInVPackage(GInterfaceDj);
          Gauge gauge = (Gauge) s;
          gauge.setLowCriticalVP(GInterfaceDj);
          s.invalidate();
          s.repaint();
        } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
          throw new ExceptionInVPackage(ExceptionPrintstacktrace.getLocalizedMessage());
    ...
    for (s s : this.ExceptionInVPackage) {
      if (s instanceof Gauge)
        try {
          ...
          ExceptionInVPackage(GInterfaceDj);
          Gauge gauge = (Gauge) s;
          gauge.setHighWarningVP(GInterfaceDj);
          s.invalidate();
          s.repaint();
        } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
          throw new ExceptionInVPackage(ExceptionPrintstacktrace.getLocalizedMessage());
    ...
    for (s s : this.ExceptionInVPackage) {
      if (s instanceof Gauge)
        try {
          ...
          ExceptionInVPackage(GInterfaceDj);
          Gauge gauge = (Gauge) s;
          gauge.setHighCriticalVP(GInterfaceDj);
          s.invalidate();
          s.repaint();
        } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
          throw new ExceptionInVPackage(ExceptionPrintstacktrace.getLocalizedMessage());
    ...
    if (paramBoolean) {
      String str =
          bV.ExceptionInVPackage(
              "{Number of seconds to remember history. }",
              true,
              "Set Guage History Tell Tail Timeout.\nDefault value 15 seconds.",
              true,
              bV.c());
  ...
  public SingleChannelDashComponent b() {
    for (s s : this.ExceptionInVPackage) {
      if (s instanceof SingleChannelDashComponent) return (SingleChannelDashComponent) s;
    }
    return null;
  }
  ...
  public boolean c() {
    for (s s : this.ExceptionInVPackage) {
      if (s instanceof SingleChannelDashComponent) return true;
    }
    return false;
  }
