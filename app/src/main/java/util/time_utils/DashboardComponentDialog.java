...
public abstract class DashboardComponentDialog extends JDialog {
  ...
  protected boolean a(Component paramComponent, String paramString) {
    if (c().a().size() <= 1 || !this.f) return true;
    this.f =
        !bV.a(
            "There is more than 1 item selected.\nAre you sure you want to set all "
                + paramString
                + " values?",
            paramComponent,
            true);
  ...
  public Gauge b(ArrayList paramArrayList) {
    for (s s : paramArrayList) {
      if (s instanceof Gauge) return (Gauge) s;
    }
    return null;
  }
  ...
  public Indicator c(ArrayList paramArrayList) {
    for (s s : paramArrayList) {
      if (s instanceof Indicator) return (Indicator) s;
    }
    return null;
  }
  ...
  public DashLabel d(ArrayList paramArrayList) {
    for (s s : paramArrayList) {
      if (s instanceof DashLabel) return (DashLabel) s;
    }
    return null;
  }
}
