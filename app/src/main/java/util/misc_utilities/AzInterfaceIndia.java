...
public class AzInterfaceIndia {
  ...
  public void a(int[] paramArrayOfint) {
    ...
    int j =
        arrayOfByte1[0] << 24
            | (arrayOfByte1[1] & 0xFF) << 16
            | (arrayOfByte1[2] & 0xFF) << 8
            | arrayOfByte1[3] & 0xFF;
    ...
    j =
        arrayOfByte2[0] << 24
            | (arrayOfByte2[1] & 0xFF) << 16
            | (arrayOfByte2[2] & 0xFF) << 8
            | arrayOfByte2[3] & 0xFF;
    ...
    if (paramArrayOfbyte == null || (paramArrayOfbyte.length != 4 && paramArrayOfbyte.length != 8))
      throw new h(
          "Expected 4 bytes, found: "
              + ((paramArrayOfbyte == null) ? "0" : Integer.valueOf(paramArrayOfbyte.length)));
    if (this.i == c) {
      long l =
          (0xFF & paramArrayOfbyte[0]) << 56L
              | (0xFF & paramArrayOfbyte[1]) << 48L
              | (0xFF & paramArrayOfbyte[2]) << 40L
              | (0xFF & paramArrayOfbyte[3]) << 32L
              | (0xFF & paramArrayOfbyte[4]) << 24L
              | (0xFF & paramArrayOfbyte[5]) << 16L
              | (0xFF & paramArrayOfbyte[6]) << 8L
              | (0xFF & paramArrayOfbyte[7]);
