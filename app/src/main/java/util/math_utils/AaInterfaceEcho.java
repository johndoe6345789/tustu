...
public class AaInterfaceEcho implements O {
  ...
  public String b() {
    return "    ;OutputChannels have to basic forms:\n"
               + "    ; 1) Controller Channels - values contained in the runtime data stream"
               + " recieved from the controller.\n"
               + "    ; 2) Expression based Channels - new channels based on any other channels or"
               + " Constant using \n"
               + "    ;    mathematical operations and TunerStudio functions.\n"
               + "    ; Type one are primarily used by Firmware developers, type 2 can easily be"
               + " user extensions.\n"
               + "    ;\n"
               + "    ; Type 1 format for scalar:\n"
               + "    ; channelName      = scalar, dataType,    offset, \"Units\",   scale,"
               + " translate\n"
               + "    ;   channelName can be any alphanumeric string. It must start with a letter"
               + " and contain no special characters or white spaces.\n"
               + "    ;   dataType will be U08, S08, U16, S16, U16, S32 or F32. For F32\n"
               + "    ;   offset - the index of the 1st byte in the read datastream, this can be"
               + " numeric or key words nextOffset and lastOffset\n"
               + "    ;   scale and translate will be applied to the raw value using the standard"
               + " formulas: \n"
               + "    ;      msValue   = userValue / scale - translate\n"
               + "    ;      userValue = (msValue + translate) * scale\n"
               + "    ;   Also of note, scale and translate can be expressions, units can use"
               ...
               + "    ; the bits of interest are described in the format [n:m] where n is the"
               + " starting bit and m the last bit.\n"
               + "    ; in the above examples, n=m so each channel is a single bit.\n"
               + "    ;\n"
               + "    ;\n"
               + "    ; Type 2 format:\n"
               + "    ; channelName = { someExpression }, \"Units\"\n"
               + "    ; someExpression can be made up of any set of constants and OutputChannels"
               + " using any \n"
               ...
               + "    ;------------------------------------------------------------------------------------------\n\n"
               + "    ;channelName      =  class,   dataType,   offset,     \"Units\",     scale,  "
               + "     translate\n"
               + "    ;------------        -------  ---------   -------     --------       ------  "
               + "     ---------\n\n";
  }
  ...
  private void a(R paramR, SerializableImpl paramaH, BufferedWriter paramBufferedWriter) {
    if (paramaH.b().equals("formula")) {
      ...
      for (int i = paramaH.aL().length(); i < P.a; i++) paramBufferedWriter.write(32);
      paramBufferedWriter.write("= {");
      paramBufferedWriter.write(paramaH.k());
      paramBufferedWriter.write("}, \"");
      paramBufferedWriter.write(paramaH.e());
      paramBufferedWriter.write("\"");
      if (!paramaH.s()) paramBufferedWriter.write(", hidden");
    } else if (paramaH.b().equals("scalar")) {
      paramBufferedWriter.write("     ");
      paramBufferedWriter.write(paramaH.aL());
      int i;
      for (i = paramaH.aL().length(); i < P.a; i++) paramBufferedWriter.write(32);
      ...
      paramBufferedWriter.write(", ");
      for (i = paramaH.e().length(); i < P.b; i++) paramBufferedWriter.write(32);
      paramBufferedWriter.write("\"");
      paramBufferedWriter.write(paramaH.e());
      paramBufferedWriter.write("\", ");
