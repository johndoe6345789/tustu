    ...
    while (this.b) {
      try {
        if (!this.d && (bool || (this.c.q() && System.currentTimeMillis() - e.q(this.c) > 2500L))) {
          D.b(
              "Comm Thread not heard from in "
                  + ((System.currentTimeMillis() - e.r(this.c)) / 1000.0D)
                  + " seconds, Reporting Thread where about.");
          if (this.c.e != null) {
            D.c(
                "Watchdog Report on Comm Thread. the following stack reports the current Location,"
                    + " not killing yet");
            a("comm", this.c.e);
          }
        }
        ...
        if (!this.d && (bool || (this.c.q() && l1 > (5000 + b)) || (this.c.q() && l2 < 0L))) {
          D.b(
              "Comm Thread not heard from in "
                  + ((System.currentTimeMillis() - e.u(this.c)) / 1000.0D)
                  + " seconds, resetting. Expected to hear from Comm Thread in: "
                  + l2
                  + "ms. , try again="
                  + bool);
        ...
        D.b(
            "CommWatchDog failed to restart "
                + this.c.e().u()
                + " on port "
                + this.c.e().s()
                + ", will try again.");
