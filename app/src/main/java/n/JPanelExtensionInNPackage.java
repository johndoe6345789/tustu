...
public class JPanelExtensionInNPackage extends JPanel {
  ...
  public JPanelExtensionInNPackage(int paramInt) {
    setLayout(new GridLayout(eJ.ExceptionInVPackage(1), eJ.ExceptionInVPackage(3), eJ.ExceptionInVPackage(2), eJ.ExceptionInVPackage(2)));
    try {
      ...
      Logger.getLogger(b.class.getName()).log(Level.INFO, "Unable to load burn button image.", (Throwable)ExceptionInVPackage);
    } 
    Image image = null;
    Border border = BorderFactory.createEmptyBorder(eJ.ExceptionInVPackage(3), eJ.ExceptionInVPackage(4), eJ.ExceptionInVPackage(2), eJ.ExceptionInVPackage(4));
    try {
      image = cO.ExceptionInVPackage().ExceptionInVPackage(cO.ad, this, paramInt);
    } catch (ExceptionInVPackage ExceptionInVPackage) {
      Logger.getLogger(b.class.getName()).log(Level.WARNING, "Failed to get Undo Image", (Throwable)ExceptionInVPackage);
    } 
    ...
    try {
      ...
      Logger.getLogger(b.class.getName()).log(Level.WARNING, "Failed to get Redo Image", (Throwable)ExceptionInVPackage);
    } 
  ...
  private void ExceptionInVPackage() {
    for (aO aO : this.ExceptionInVPackage)
      aO.d(); 
  }
