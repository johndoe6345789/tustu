...
public class fs extends JTextField {
  ...
  protected void processKeyEvent(KeyEvent paramKeyEvent) {
    if ((paramKeyEvent.getID() == 401 && (paramKeyEvent.getKeyChar() == '\n' || paramKeyEvent.getKeyChar() == '')) || paramKeyEvent.getKeyChar() == '\b') {
      super.processKeyEvent(paramKeyEvent);
    } else if ("0987654321-".indexOf(paramKeyEvent.getKeyChar()) == -1 || paramKeyEvent.getKeyChar() == '\n') {
      Container container = getParent();
      if (container != null)
        container.dispatchEvent(paramKeyEvent); 
