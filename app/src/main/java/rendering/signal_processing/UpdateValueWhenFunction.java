...
public class AbstractInAxPackage extends AbstractInAxPackage {
  ...
  protected AbstractInAxPackage(List<AxInterfaceAb> paramList) {
    ...
    if (paramList.size() > 2) this.c = paramList.get(2);
    this.f = paramList;
  }

  public synchronized double a(CloneableImplInAxPackage paramS) {
    if (!this.e && this.c != null && Double.isNaN(this.d)) this.d = this.c.b(paramS);
    if (this.a.b(paramS) != 0.0D) this.d = this.b.b(paramS);
    return this.d;
  }
  ...
  public String toString() {
    ...
    for (byte b = 0; b < this.f.size(); b++) {
      ...
      if (b + 1 < this.f.size()) stringBuilder.append(", ");
    }
