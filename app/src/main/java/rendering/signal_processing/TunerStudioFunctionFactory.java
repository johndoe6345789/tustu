...
public class TunerStudioFunctionFactory extends AbstractInAxPackage {
  ...
  public AbstractInAxPackage a(String paramString, List paramList) {
    ...
    if (paramString.equalsIgnoreCase("timeNow")) AbstractInAxPackage = b(paramString, paramList);
    ...
    } else if (paramString.equalsIgnoreCase("isAdvancedMathAvaliable")
        || paramString.equalsIgnoreCase("isAdvancedMathAvailable")) {
      if (c) {
  ...
  private AbstractInAxPackage b(String paramString, List paramList) {
    if (paramList.size() != 0) throw new AxComponentAlpha(paramString, paramList.size(), 0);
    return new X();
  }
  ...
  private AbstractInAxPackage AbstractInAxPackage(String paramString, List paramList) {
    if (!paramList.isEmpty()) throw new AxComponentAlpha(paramString, paramList.size(), 0);
    return new ad();
  }

  private AbstractInAxPackage f(String paramString, List paramList) {
    if (paramList.isEmpty()) return new D(this.b);
    throw new AxComponentAlpha(paramString, paramList.size(), 0);
  }

  private AbstractInAxPackage g(String paramString, List paramList) {
    if (paramList.size() >= 2 && paramList.size() <= 3) return new AbstractInAxPackage(paramList);
  ...
  private AbstractInAxPackage h(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() == 1) return new s(this.b, paramList.get(0));
    if (paramList.size() == 2) return new s(paramList.get(0), paramList.get(1));
  ...
  private AbstractInAxPackage i(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() == 1) return new w(this.b, paramList.get(0));
    if (paramList.size() == 2) return new w(paramList.get(0), paramList.get(1));
    throw new AxComponentAlpha(paramString, paramList.size(), 1);
  }

  private AbstractInAxPackage j(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() == 1) return new x(this.b, paramList.get(0));
    if (paramList.size() == 2) return new x(paramList.get(0), paramList.get(1));
    throw new AxComponentAlpha(paramString, paramList.size(), 1);
  }

  private AbstractInAxPackage ThreadedInIPackage(
      String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() == 1) return new t(this.b, paramList.get(0));
    if (paramList.size() == 2) return new t(paramList.get(0), paramList.get(1));
    throw new AxComponentAlpha(paramString, paramList.size(), 1);
  }

  private AbstractInAxPackage l(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() == 1) return new v(this.b, paramList.get(0));
    if (paramList.size() == 2) return new v(paramList.get(0), paramList.get(1));
    throw new AxComponentAlpha(paramString, paramList.size(), 1);
  }

  private AbstractInAxPackage m(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() == 1) return new u(this.b, paramList.get(0));
    if (paramList.size() == 2) return new u(paramList.get(0), paramList.get(1));
    throw new AxComponentAlpha(paramString, paramList.size(), 1);
  }

  private AbstractInAxPackage n(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() == 2) return new d(this.b, paramList.get(0), paramList.get(1));
    throw new AxComponentAlpha(paramString, paramList.size(), 2);
  }
  ...
  private AbstractInAxPackage o(String paramString, List<AxInterfaceAb> paramList) {
    ...
    throw new AxComponentAlpha(
        "Proper usage: tableLookup(array.zArray, array.xArray, array.yArray, xChannel, yChannel),"
            + " or for 1D tables: tableLookup(array.ValueArrayName, array.LookupArrayName,"
            + " LookupChannelName)"
            + paramString,
        paramList.size(),
        5);
  ...
  private AbstractInAxPackage r(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() != 1) throw new AxComponentAlpha(paramString, paramList.size(), 1);
