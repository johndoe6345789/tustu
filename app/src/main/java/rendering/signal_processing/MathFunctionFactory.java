...
public class MathFunctionFactory extends AbstractInAxPackage {
  ...
  public static n a() {
    if (AbstractInAxPackage == null) AbstractInAxPackage = new n();
  ...
  private AbstractInAxPackage b(String paramString, List<AxInterfaceAb> paramList) {
    if (paramList.size() != 2) throw new AxComponentAlpha(paramString, paramList.size(), 1);
  ...
  private AbstractInAxPackage c(String paramString, List<AxInterfaceAb> paramList) {
    ...
    return (paramList.size() == 2)
        ? new aa(paramList.get(0), paramList.get(1))
        : new aa(paramList.get(0));
  }
  ...
  private AbstractInAxPackage k(String paramString, List paramList) {
    if (paramList.size() != 0) throw new AxComponentAlpha(paramString, paramList.size(), 1);
    ...
    if (paramList.size() != 4)
      throw new AxComponentAlpha(
          paramString
              + "\n"
              + "Proper Usage:\n"
              + "aerodynamicDragHp(velocity (m/s), airDensity (kg/m^-3), dragCoefficent,"
              + " frontalArea (m^2))",
          paramList.size(),
          4);
