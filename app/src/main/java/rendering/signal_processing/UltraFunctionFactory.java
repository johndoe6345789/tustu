...
public class ab extends AbstractInAxPackage {
  ...
  public AbstractInAxPackage a(String paramString, List paramList) {
    ...
    f f = null;
    if (paramString.equalsIgnoreCase("isUltraFunctionsAvaliable")
        || paramString.equalsIgnoreCase("isUltraFunctionsAvailable")) {
      if (this.b) {
        f = new f(1.0D);
      } else {
        f = new f(0.0D);
  ...
  private AbstractInAxPackage b(String paramString, List paramList) {
    if (paramList.size() >= 2 && paramList.size() <= 4) return new Y(paramList);
