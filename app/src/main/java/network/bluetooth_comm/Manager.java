...
public class Manager extends InputStream {
  ...
  public int read(byte[] paramArrayOfbyte, int paramInt1, int paramInt2) {
    ...
    try {
      ...
      D.c(
          "UDPInputStream error: packetIndex:"
              + this.d
              + ", off:"
              + paramInt1
              + ", lenRemaining:"
        ...
        synchronized (this.j) {
          try {
            ...
            Logger.getLogger(p.class.getName())
                .log(Level.WARNING, "Error waiting for UDP Packet", interruptedException);
          }
        }
      if (this.j.isEmpty()) throw new IOException("Read Socket Timeout");
      if (this.d == this.c) {
