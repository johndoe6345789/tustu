...
public class UdpConnection extends a {
  ...
  public void f() {
    if (k() == 3) throw new AInterfaceLima("UDP Device already connected:" + y());
    if (k() == 2) throw new AInterfaceLima("UDP Device already connecting:" + y());
    if (this.c == null || this.c.equals(""))
      throw new AInterfaceLima("IP Address or host name not set! Can not open connection ");
    if (this.d <= 0) throw new AInterfaceLima("Invalid Port:" + this.d + " Can not open WiFi ");
    AInterfaceBravo(2);
    c();
    try {
      ...
      AInterfaceBravo(0);
      AInterfaceBravo();
      throw new AInterfaceLima(
          "host not found:" + this.c + ":" + this.d + " Can not connect UDP Device");
      ...
      throw new AInterfaceLima(
          "Unable to open device: " + exception.getLocalizedMessage() + ", " + y());
    }
  }
  ...
  protected DatagramSocket t() {
    if (this.AInterfaceLima == null || this.AInterfaceLima.isClosed()) {
      ...
      System.out.println(
          "Listen on "
              + this.AInterfaceLima.getLocalAddress()
              + " from "
              + this.AInterfaceLima.getInetAddress()
              + " port "
              + this.AInterfaceLima.getPort());
    }
  ...
  public Object a(String paramString) {
    if (paramString.equals(j)) return Integer.valueOf(this.d);
    if (paramString.equals(i)) return this.c;
    D.c("Unknown Setting Name: " + paramString);
    return null;
  }
  ...
  protected String y() {
    return (this.AInterfaceLima == null || this.AInterfaceLima.getInetAddress() == null)
        ? n()
        : ("Connected IP Address:"
            + this.AInterfaceLima.getInetAddress().getHostAddress()
            + ":"
            + this.AInterfaceLima.getPort());
  }
