...
public class AgComponentCharlie implements AeInterfaceSierra, AfComponentAlpha {
  ...
  public AeInterfaceDelta AfComponentAlpha(FileUsingHashMap paramk, IOInAePackage paramp, AeInterfaceUniform paramu) {
  ...
  private int b(IOInAePackage paramp) {
    ...
    try {
      ...
      return (arrayOfByte1 == null || arrayOfByte1.length == 0 || arrayOfByte1[0] != -36) ? -1 : bH.c.AfComponentAlpha(arrayOfByte1, 1, 2, true, false);
    } catch (ExceptionExtensionInAePackage ExceptionExtensionInAePackage) {
      Logger.getLogger(c.class.getName()).log(Level.SEVERE, "Protocol Error probing for monitor", (Throwable)ExceptionExtensionInAePackage);
      return -1;
    } 
  }
