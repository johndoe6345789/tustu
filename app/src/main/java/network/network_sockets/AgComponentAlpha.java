...
public class AgComponentAlpha implements AeInterfaceQuebec {
      ...
      try {
        ...
        if (a1.a().size() > 0) {
          ...
          if (!str.contains("megasquirt2.s19")
              ...
              && !str.contains("trans.s19")
              && !str.contains("mspnp2.s19")) {
            File file = list.get(b);
            if ((file.getName().toLowerCase().startsWith("monitor")
                    && file.getName().endsWith(".s19"))
                || (file.getName().toLowerCase().startsWith("lct")
                    && file.getName().endsWith(".s19"))
                || (file.getName().toLowerCase().startsWith("pico")
                    && file.getName().endsWith(".s19"))) return true;
          }
        }
  ...
  public AdInterfaceDelta a(
    ...
    if (AdInterfaceDelta.a() == AdInterfaceDelta.b) return AdInterfaceDelta;
  ...
  public String a(File paramFile) {
    return paramFile.getName().toLowerCase().startsWith("monitor")
        ? AeInterfaceOscar.ExceptionInAdPackage
        : "";
  }
