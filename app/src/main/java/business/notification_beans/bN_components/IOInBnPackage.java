...
public class IOInBnPackage {
  ...
  private void ExceptionInBrPackage(IOException paramIOException) {
    ...
    synchronized (this.BrInterfaceDelta) {
      if (!this.BrInterfaceDelta.isEmpty()) arrayList.addAll(this.BrInterfaceDelta);
    }
    for (f f1 : arrayList) f1.ExceptionInBrPackage(paramIOException);
  }
  ...
  private void ExceptionInBrPackage(t paramt) {
    ...
    for (f f1 : this.BrInterfaceDelta) f1.ExceptionInBrPackage(paramt);
  }

  private void ExceptionInBrPackage(int paramInt, String paramString) {
    for (f f1 : this.BrInterfaceDelta) f1.ExceptionInBrPackage(paramInt, paramString);
  }

  private void BrInterfaceDelta() {
    for (f f1 : this.BrInterfaceDelta) f1.ExceptionInBrPackage();
  }

  private void e() {
    for (f f1 : this.BrInterfaceDelta) f1.b();
  }

  private void ExceptionInBrPackage(byte[] paramArrayOfbyte, int paramInt) {
    if (f())
      D.BrInterfaceCharlie(
          "Adding back to prebuffer from offset: \n"
              + BrInterfaceCharlie.ExceptionInBrPackage(
                  paramArrayOfbyte, 16, paramInt, paramArrayOfbyte.length - paramInt));
    if (this.m == null || this.m.length == 0) {
      ...
      System.arraycopy(
          paramArrayOfbyte, paramInt, arrayOfByte, 0, paramArrayOfbyte.length - paramInt);
  ...
  private void g() {
    ...
    try {
      ...
      do {
        ...
        i += m;
        if (m == 0) ExceptionInBrPackage(0, 200);
        if (f()) {
          D.BrInterfaceCharlie(
              "Cleared "
                  + i
      ...
      if (i > 0) D.BrInterfaceDelta("Cleared Input Buffer of bytes: " + i);
  ...
  private BrInterfaceCharlie ExceptionInBrPackage(BrInterfaceCharlie paramc, boolean paramBoolean) {
    ...
    try {
      ...
      if (m < arrayOfByte1.length) throw new BrInterfaceCharlie();
      BrInterfaceDelta();
      ...
      if (this.f > -1 && n != paramc.ExceptionInBrPackage().BrInterfaceCharlie()) {
        if (!paramBoolean)
          D.b(
              "unexpected Counter Missing packets? Expected: x"
                  + Integer.toHexString(this.f + 1)
                  + ", found: x"
                  + Integer.toHexString(paramc.ExceptionInBrPackage().BrInterfaceCharlie()));
        if (!paramBoolean && this.t != null) {
          int i2 =
              (this.p == 0
                      && (paramc.ExceptionInBrPackage().BrInterfaceCharlie() == 0 || n < 10)
                      && (i1 <= this.b.i() || i1 <= this.b.JInterfaceJuliet()))
                  ? 0
                  : (Math.max(paramc.ExceptionInBrPackage().BrInterfaceCharlie(), n)
                      - Math.min(paramc.ExceptionInBrPackage().BrInterfaceCharlie(), n));
          if (paramc.ExceptionInBrPackage().BrInterfaceCharlie() < 127
              && n > Math.pow(2.0D, (8 * i / 2)) - 127.0D)
            i2 =
                (int)
                    (Math.pow(2.0D, (8 * i / 2))
                        + paramc.ExceptionInBrPackage().BrInterfaceCharlie()
                        - n);
          if (!paramBoolean) {
            D.BrInterfaceCharlie(
                "missing: "
                    + i2
                    + ", message.getHeader().getCounter()="
                    + paramc.ExceptionInBrPackage().BrInterfaceCharlie()
                    + ", nextCounter="
                    + n);
      ...
      arrayOfByte2 =
          this.BrInterfaceCharlie.b(i1 + paramc.BrInterfaceCharlie().ExceptionInBrPackage());
      ...
      arrayOfByte3 =
          new byte[arrayOfByte2.length - paramc.BrInterfaceCharlie().ExceptionInBrPackage()];
      ...
      if (arrayOfByte2 != null) this.BrInterfaceCharlie.ExceptionInBrPackage(arrayOfByte2);
      e();
    }
  }
  ...
  private synchronized void h() {
    if (this.b.b() == 2 || this.b.b() == 1) {
      BrInterfaceCharlie BrInterfaceCharlie =
          BrInterfaceDelta.ExceptionInBrPackage().ExceptionInBrPackage(this.b);
      try {
        ...
        D.BrInterfaceCharlie(
            "Invalid data read, does not look like ExceptionInBrPackage message: "
                + BrInterfaceCharlie.BrInterfaceDelta(e.ExceptionInBrPackage()));
        ...
        do {
          try {
            if (arrayOfByte1.length > 0) arrayList.add(Byte.valueOf(arrayOfByte1[0]));
            if (arrayOfByte1.length > 1) ExceptionInBrPackage(arrayOfByte1, 1);
            BrInterfaceCharlie = ExceptionInBrPackage(BrInterfaceCharlie, true);
            if (!BrInterfaceCharlie.BrInterfaceDelta()) {
              D.BrInterfaceCharlie(
                  "Got what looked to be ExceptionInBrPackage message, but it failed CRC.");
              try {
                throw new e(BrInterfaceCharlie.h());
              } catch (ExceptionInBrPackage ExceptionInBrPackage) {
                Logger.getLogger(p.class.getName())
                    .log(Level.WARNING, "This shouldn't happen?", (Throwable) ExceptionInBrPackage);
              }
            }
            ...
            try {
              ...
              System.arraycopy(
                  arrayOfByte4, 0, arrayOfByte3, arrayOfByte2.length, arrayOfByte4.length);
            ...
            if (arrayOfByte != null && arrayOfByte.length > 0) {
              ...
              System.arraycopy(
                  arrayOfByte, 0, arrayOfByte3, arrayOfByte1.length, arrayOfByte3.length);
              arrayOfByte1 = arrayOfByte3;
            }
            if (arrayOfByte1.length > 1) arrayList.add(Byte.valueOf(arrayOfByte1[0]));
          }
        } while (b1 < 6 && b2++ < c1);
        if (b2 >= c1)
          throw new IOException("Failed to resync data stream after lost packet bytes.");
        D.BrInterfaceDelta(
            "Resync'BrInterfaceDelta. Lost Bytes: "
                + b2
                + ", Good bytes added back: "
                + arrayOfByte2.length);
        ...
        try {
          ...
          D.BrInterfaceCharlie(
              "Error in resync data, pre-buffer should contain ExceptionInBrPackage valid"
                  + " message.");
          return;
        }
      } catch (IOException iOException) {
        ...
        D.ExceptionInBrPackage(
            "Unhandled Exception reading message. Resync? " + exception.getLocalizedMessage());
      ...
      if (!BrInterfaceCharlie.BrInterfaceDelta()) {
        ...
        D.ExceptionInBrPackage(
            "Unexpected counter on CTO Response Message! Setting status to error.");
  ...
  public void ExceptionInBrPackage(int paramInt) {
    if (this.e != null) this.e.ExceptionInBrPackage = false;
    ...
    if (paramInt == h) {
      ...
      try {
        ...
        Logger.getLogger(p.class.getName())
            .log(Level.INFO, "input.available() problem", iOException);
        this.l = true;
      }
    } else {
      this.k = ": RX:\n";
  ...
  private void ExceptionInBrPackage(List paramList) {
    if (paramList.size() > 0) {
      ...
      byte[] arrayOfByte =
          BrInterfaceCharlie.ExceptionInBrPackage(
              (Byte[]) paramList.toArray((Object[]) new Byte[paramList.size()]));
