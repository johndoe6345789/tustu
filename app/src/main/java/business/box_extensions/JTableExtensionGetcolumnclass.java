...
public class JTableExtensionGetcolumnclass extends JTable {
  ab a = null;
  ...
  public JTableExtensionGetcolumnclass(ab paramab) {
    this.a = paramab;
    setModel(this.b);
    setSelectionMode(0);
  ...
  private void b(j paramj) {
    for (s s : this.d) s.a(paramj);
  }

  private void c(j paramj) {
    for (s s : this.d) s.b(paramj);
  }
  ...
  public j e(String paramString) {
    for (byte b = 0; b < this.g.size(); b++) {
      if (((j) this.g.get(b)).a().equals(paramString)) return this.g.get(b);
    }
    return null;
  }
