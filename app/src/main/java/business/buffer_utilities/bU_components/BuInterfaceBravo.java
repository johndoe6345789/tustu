  ...
  public void a(BnInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.a() != a())
      throw new ExceptionInBsPackage(
          "PID:USER_CMD_RUNTIME_READ_CONTROL Wrong handler! this handler is for 0x"
              + Integer.toHexString(a()).toUpperCase());
    byte[] arrayOfByte = paramt.c();
    if (arrayOfByte == null)
      throw new ExceptionInBsPackage(
          "PID:USER_CMD_RUNTIME_READ_CONTROL valid packet data not found.");
    if (arrayOfByte.length < 5)
      throw new ExceptionInBsPackage(
          "PID:USER_CMD_RUNTIME_READ_CONTROL invalid packet data length, must have atleast 5"
              + " bytes");
    ...
    if (b1 == a) {
      ...
      throw new ExceptionInBsPackage(
          "PID:USER_CMD_RUNTIME_READ_CONTROL Unknown Action: 0x"
              + Integer.toHexString(b1).toUpperCase());
    }
  }
}
