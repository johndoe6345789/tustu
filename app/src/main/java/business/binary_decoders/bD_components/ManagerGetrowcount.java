...
public class ManagerGetrowcount extends AbstractTableModel {
  ...
  private String a(String paramString) {
    return (paramString == null)
        ? ""
        : (paramString.toUpperCase().endsWith(".MS3")
            ? "MS3 SD Log"
            : (paramString.toUpperCase().endsWith(".FRD")
                ? "Formatted Raw Data"
                : (paramString.toUpperCase().endsWith(".MSL")
                    ? "MegaSquirt Log"
                    : (paramString.toUpperCase().endsWith(".MLG")
                        ? "MLG Data Log"
                        : (paramString.toUpperCase().endsWith(".CSV")
                            ? "CSV Data Log"
                            : ((paramString.lastIndexOf(".") != -1)
                                ? paramString.substring(paramString.lastIndexOf(".") + 1)
                                : "Other"))))));
  }
