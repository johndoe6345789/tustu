...
public class BhInterfaceVictor implements ExceptionInVPackage {
    ...
    if (n != null) {
      for (j j : n) {
        ...
        SerializableImplExceptionprintstacktrace SerializableImplExceptionprintstacktrace =
            ExceptionInVPackage(r, str2);
        if (SerializableImplExceptionprintstacktrace != null)
          try {
            SerializableImpl SerializableImpl =
                r.g(SerializableImplExceptionprintstacktrace.ExceptionInVPackage());
            byte[] arrayOfByte = ExceptionInVPackage(r);
            if (SerializableImpl != null
                && !SerializableImpl.ExceptionInVPackage(arrayOfByte, j.d(paramInt))) {
              arrayList.add(new w(this, str1, SerializableImpl, j.d(paramInt)));
              if (!this.c.contains(SerializableImpl)) this.c.add(SerializableImpl);
            }
      ...
      for (String str : this.ExceptionInVPackage.keySet()) {
        try {
          ManagerUsingArrayList.ExceptionInVPackage()
              .ExceptionInVPackage(str, (byte[]) this.ExceptionInVPackage.get(str));
        } catch (Exception exception) {
        }
      }
      for (w w : arrayList) {
        ...
        try {
          ManagerUsingArrayList.ExceptionInVPackage()
              .ExceptionInVPackage(w.c(), w.ExceptionInVPackage(), w.b());
  ...
  private SerializableImplExceptionprintstacktrace ExceptionInVPackage(
      R paramR, String paramString) {
    Map<String, SerializableImplExceptionprintstacktrace> map = ExceptionInVPackage(paramR.c());
    SerializableImplExceptionprintstacktrace SerializableImplExceptionprintstacktrace =
        (SerializableImplExceptionprintstacktrace) map.get(paramString);
    if (SerializableImplExceptionprintstacktrace != null)
