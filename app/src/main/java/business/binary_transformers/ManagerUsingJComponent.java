...
class ManagerUsingJComponent implements LayoutManager {
  ...
  ManagerUsingJComponent(
      an paraman, JComponent paramJComponent, JButton paramJButton1, JButton paramJButton2) {
  ...
  public void layoutContainer(Container paramContainer) {
    ...
    if (this.d != null)
      if (this.c.E) {
        ...
        this.d.setBounds(
            0,
            paramContainer.getHeight() - dimension.height,
            paramContainer.getWidth(),
            dimension.height);
      } else {
        Dimension dimension = this.d.getPreferredSize();
        int j = this.a.getY() + this.a.getHeight();
        int k =
            (dimension.height < paramContainer.getHeight() - j)
                ? dimension.height
                : (paramContainer.getHeight() - j);
        int m =
            (dimension.width < paramContainer.getWidth())
                ? dimension.width
                : paramContainer.getWidth();
