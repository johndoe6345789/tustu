...
public class BtInterfaceWhiskey implements BsInterfaceAlpha {
  ...
  public void BsInterfaceAlpha(BnInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.BsInterfaceAlpha() != BsInterfaceAlpha())
      throw new ExceptionInBsPackage("PID:STD_SET_SETMENT_MODE Wrong handler! this handler is for 0x" + Integer.toHexString(BsInterfaceAlpha()).toUpperCase()); 
    ...
    if (arrayOfByte1.length != 2)
      throw new ExceptionInBsPackage("PID:STD_SET_SETMENT_MODE invalid packet data length, expected 3 bytes, found: " + (arrayOfByte1.length + 1)); 
