...
public class BtInterfaceDelta implements ExceptionInVPackage {
  ...
  public void ExceptionInVPackage(BnInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.ExceptionInVPackage() != ExceptionInVPackage())
      throw new ExceptionInBsPackage(
          "PID:DAQ_WRITE_DAQ Wrong handler! this handler is for 0x"
              + Integer.toHexString(ExceptionInVPackage()).toUpperCase());
    ...
    if (arrayOfByte.length != 7)
      throw new ExceptionInBsPackage(
          "PID:DAQ_WRITE_DAQ invalid packet data length, expected 7 bytes, found: "
    ...
    try {
      ...
      if (TostringInBoPackage == null
          || i > BsInterfaceDelta.b()
          || ExceptionInBoPackage > TostringInBoPackage.ExceptionInVPackage()
          || m > TostringInBoPackage.TostringInBoPackage()) {
      ...
      for (SerializableImpl SerializableImpl : list) {
        try {
          ManagerUsingArrayList.ExceptionInVPackage()
              .ExceptionInVPackage(
                  r.TostringInBoPackage(), SerializableImpl.aL(), TostringInBoPackage.n());
      ...
      Logger.getLogger(getClass().getName())
          .log(Level.SEVERE, (String) null, (Throwable) ExceptionInBoPackage);
      TostringInBnPackage TostringInBnPackage = BnInterfaceUniform.ExceptionInVPackage().b();
      TostringInBnPackage.ExceptionInVPackage(254);
  ...
  private List ExceptionInVPackage(R paramR, int paramInt1, int paramInt2) {
    ...
    while (iterator.hasNext()) {
      ...
      if (SerializableImpl.ExceptionInVPackage() >= paramInt1
          && SerializableImpl.ExceptionInVPackage() < paramInt1 + paramInt2)
  ...
  private int ExceptionInVPackage(R paramR, long paramLong) {
    if (paramLong < paramR.O().n()) return (int) paramLong;
    SerializableImpl SerializableImpl = b(paramR, paramLong);
    if (SerializableImpl == null) {
      bH.D.ExceptionInVPackage(
          "Can not find ExceptionInVPackage channel at address 0x"
              + Long.toHexString(paramLong).toUpperCase());
  ...
  private SerializableImpl b(R paramR, long paramLong) {
    ...
    while (iterator.hasNext()) {
      SerializableImpl SerializableImpl = iterator.next();
      if (SerializableImpl.x() == paramLong) return SerializableImpl;
    }
    return null;
  }
}
