  ...
  public void BsInterfaceAlpha(BnInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.BsInterfaceAlpha() != BsInterfaceAlpha())
      throw new ExceptionInBsPackage(
          "PID:CAL_DOWNLOAD_NEXT Wrong handler! this handler is for 0x"
    ...
    if (arrayOfByte.length < 1)
      throw new ExceptionInBsPackage(
          "PID:CAL_DOWNLOAD_NEXT invalid packet data length, must have atleast 1 byte");
    ...
    i1 -= r.O().y(j);
    if (TostringInBnPackage.I()) D.c("DOWNLOAD_NEXT: Got: " + n + ", expectedRemaining: " + b());
    try {
      if (i > this.BsInterfaceAlpha) {
        D.BsInterfaceAlpha(
            "PID:CAL_DOWNLOAD_NEXT Element count not as expected: "
                + this.BsInterfaceAlpha
                + ", received: "
                + i);
        ...
        try {
          ...
          D.c(
              "expectedRemainingElements = "
                  ...
                  + ", packetElementCount="
                  + m
                  + ", setting expectedRemainingElements to: "
                  + (i - m));
