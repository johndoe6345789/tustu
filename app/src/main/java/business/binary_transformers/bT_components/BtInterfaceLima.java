...
public class BtInterfaceLima implements BsInterfaceAlpha {
  ...
  public void BsInterfaceAlpha(BnInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.BsInterfaceAlpha() != BsInterfaceAlpha())
      throw new ExceptionInBsPackage("PID:DAQ_GET_DAQ_LIST_INFO Wrong handler! this handler is for 0x" + Integer.toHexString(BsInterfaceAlpha()).toUpperCase()); 
    ...
    if (arrayOfByte.length != 3)
      throw new ExceptionInBsPackage("PID:DAQ_GET_DAQ_LIST_INFO invalid packet data length, expected 4 bytes, found: " + (arrayOfByte.length + 1)); 
    try {
      ...
      TostringInBoPackage.BsInterfaceAlpha(arrayOfByte1, TostringInBoPackage.d(), 3, 2, BnInterfaceKilo.g());
      l1.b(arrayOfByte1);
