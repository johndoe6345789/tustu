  ...
  public void BsInterfaceAlpha(BnInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.BsInterfaceAlpha() != BsInterfaceAlpha())
      throw new ExceptionInBsPackage(
          "PID:STD_BUILD_CHECKSUM Wrong handler! this handler is for 0x"
    ...
    if (arrayOfByte.length != 7)
      throw new ExceptionInBsPackage(
          "PID:STD_BUILD_CHECKSUM invalid packet data length, expected 8 bytes, found: "
    ...
    int m = j - r.O().y(i);
    int n =
        c.BsInterfaceAlpha(arrayOfByte, 3, 4, BnInterfaceKilo.g(), false)
            * paramo.d().BnInterfaceUniform();
    try {
      if (n > 4096) {
        ...
        arrayOfByte2 =
            c.BsInterfaceAlpha(this.BsInterfaceAlpha.b(), arrayOfByte2, BnInterfaceKilo.g());
        TostringInBnPackage TostringInBnPackage = BnInterfaceUniform.BsInterfaceAlpha().b();
