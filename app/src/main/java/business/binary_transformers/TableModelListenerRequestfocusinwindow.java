...
public class TableModelListenerRequestfocusinwindow extends JPanel
    implements GInterfaceAn, L, h, CInterfaceDelta, bc, p, TableModelListener {
  ...
  public TableModelListenerRequestfocusinwindow(R paramR, CloneableImpl parambe) {
    this(paramR, parambe, true);
  }

  public TableModelListenerRequestfocusinwindow(
      R paramR, CloneableImpl parambe, boolean paramBoolean) {
    ...
    try {
      ...
      if (parambe.p() != null) RInterfaceIndia.ExceptionInVPackage(paramR.c(), parambe.p(), this);
      if (parambe.q() != null) RInterfaceIndia.ExceptionInVPackage(paramR.c(), parambe.q(), this);
    ...
    try {
      if (paramBoolean) {
        s =
            bQ.ExceptionInVPackage()
                .ExceptionInVPackage(paramR, parambe.aL(), parambe.l(), parambe.aL());
      } else {
      ...
      this.f
          ...
          .c(
              (RInterfaceIndia <= 3
                  && this.CInterfaceDelta.v() < 1
                  && ExceptionInVPackage(this.b) <= 3
      ...
      if (parambe.RInterfaceIndia() > 0) {
        ...
        int k =
            ExceptionInVPackage.ExceptionInVPackage()
                .ExceptionInVPackage(ExceptionInVPackage.aH, j);
        this.f.c(k);
      }
      this.f.ExceptionInVPackage(this);
    } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
      D.ExceptionInVPackage(
          "Unable to load table. See log for details.", (Exception) ExceptionPrintstacktrace, this);
    }
      ...
      if (parambe.p() == null) {
        ...
        try {
          ...
          Logger.getLogger(bu.class.getName())
              .log(
                  Level.WARNING,
                  "Failed to resolve X Axis Label",
    ...
    if (s.v() != null) {
      ...
      if (parambe.q() == null) {
        ...
        try {
          ...
          Logger.getLogger(bu.class.getName())
              .log(
                  Level.WARNING,
                  "Unable to resolve Y Axis Label",
                  (Throwable) ExceptionPrintstacktrace);
          str = ExceptionPrintstacktrace.b(s.v());
        }
      }
      if (this.c.p() != null && !this.c.p().equals("")) {
        ...
        if (RInterfaceIndia.ExceptionInVPackage().ExceptionInVPackage("645fds645fds  fdsd098532#@")
            && ExceptionInVPackage.ExceptionInVPackage()
                .c(ExceptionInVPackage.cc, ExceptionInVPackage.cd)
            && this.c.p().toLowerCase().equals("kpa")) {
          str1 = "PSI";
        } else {
          str1 = this.c.p();
        }
    ...
    if (parambe.m() || parambe.o()) this.f.b(false);
    c();
    ExceptionInVPackage(paramR, this.c);
  }

  private void ExceptionInVPackage(R paramR, Manager paramaM) {
    if (paramaM.p() != null && paramaM.p().equalsIgnoreCase("kpa")) {
      try {
        ...
        for (byte b = 0; b < arrayOfDouble.length; b++) {
          ...
          stringBuilder
              .append(X.b(d1 * 0.145038D, 2))
              .append(" ")
              .append(ExceptionPrintstacktrace.b("PSI Absolute"))
              .append("<br>");
          stringBuilder
              .append(X.b((d1 - m) * 0.145038D, 2))
              .append(" ")
              .append(ExceptionPrintstacktrace.b("PSI Relative to " + m + " kPa"))
              .append("<br>");
          stringBuilder
              .append(X.b((d1 - m) * 0.29538D, 2))
              .append(" ")
              .append(ExceptionPrintstacktrace.b("inHg Relative to " + m + " kPa"))
              .append("<br>");
  ...
  private int ExceptionInVPackage(Manager paramaM) {
  ...
  public void e() {
    if (this.RInterfaceIndia != null) this.RInterfaceIndia.b();
  }
  ...
  public void ExceptionInVPackage(String paramString1, String paramString2) {
    ...
    if (this.c.aL().equals(paramString2)) ExceptionInVPackage(this.ExceptionInVPackage, this.c);
  }
  ...
  private void b(String paramString) {
    for (K k : this.j) k.b(paramString);
  }

  private void c(String paramString) {
    for (K k : this.j) k.ExceptionInVPackage(paramString);
  }
