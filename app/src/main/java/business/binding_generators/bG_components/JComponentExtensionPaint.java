...
public class JComponentExtensionPaint extends JComponent {
  ...
  public void paint(Graphics paramGraphics) {
    ...
    if (arrayList.size() > 0) {
      double d =
          l()
              - 360.0D
              - ((k) arrayList.get(arrayList.size() - 1)).a()
  ...
  public void b(Graphics paramGraphics) {
    ...
    Point2D point2D2 =
        a(i, i, this.q - d3 / 2.0D, (getWidth() / 2), (getHeight() / 2), d5 - d4, false);
        ...
        Math.sqrt(
                Math.pow(paramInt1 / 2.0D, 2.0D)
                    * Math.pow(paramInt2 / 2.0D, 2.0D)
                    / (Math.pow(paramInt1 / 2.0D * Math.sin(d1), 2.0D)
                        + Math.pow(paramInt2 / 2.0D * Math.cos(d1), 2.0D)))
            * paramDouble4;
  ...
  private double b(int paramInt1, int paramInt2) {
    ...
    if (paramInt2 > point.y) d = 6.283185307179586D - d;
  ...
  public int b() {
    ...
    for (byte b = 0; b < arrayList.size(); b++) {
      ...
      if (d() == c) {
        if (k1.a() > d) return (k == null) ? (b - 1) : (b - 1);
      } else if (k1.a() > d) {
  ...
  private int m() {
    return (int)
        ((getWidth() > getHeight())
            ? Math.round(getHeight() * this.a)
            : Math.round(getWidth() * this.a));
  }
  ...
  private void a(
    ...
    Line2D.Double double_ =
        new Line2D.Double(paramDouble1, paramDouble2, paramDouble3, paramDouble4);
  ...
  private void a(
    ...
    if (bool) {
      Point2D point2D1 =
          a(paramInt1, paramInt2, paramDouble3, getWidth() / 2.0D, getHeight() / 2.0D, 1.0D, false);
      Point2D point2D2 =
          a(
              paramInt1,
              paramInt2,
      ...
      Arc2D.Double double_ =
          new Arc2D.Double(
              paramDouble1, paramDouble2, paramInt1, paramInt2, paramDouble3, paramDouble4, 0);
      ((Graphics2D) paramGraphics).draw(double_);
    }
  }
  ...
  private void d(Graphics paramGraphics) {
    ...
    paramGraphics.drawString(
        str, (int) point2D3.getX() - j / 2, (int) point2D3.getY() + fontMetrics.getHeight() / 2);
  }
  ...
  public void j() {
    for (byte b = 0; b < e().a().size(); b++) a(Integer.valueOf(b));
  }
}
