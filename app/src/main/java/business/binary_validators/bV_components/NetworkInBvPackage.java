...
public class NetworkInBvPackage {
  String[] a =
      new String[] {
        "https://updates.efianalytics.com/register/CheckForUpdates?",
        "https://updates2.efianalytics.com/register/CheckForUpdates?",
        "https://updates3.efianalytics.com/register/CheckForUpdates?"
      };

  String[] b =
      new String[] {
        "http://updates.efianalytics.com/register/CheckForUpdates?",
        "http://updates2.efianalytics.com/register/CheckForUpdates?",
        "http://updates3.efianalytics.com/register/CheckForUpdates?"
      };
  ...
  public d a(e parame) {
    ...
    String str3 =
        str2
            ...
            + parame.c()
            + "&appName="
            + URLEncoder.encode(parame.b(), "UTF-8")
            + "&appEdition="
            + URLEncoder.encode(parame.g(), "UTF-8")
            + "&installDate="
            + parame.d()
            + "&javaVersion="
            ...
            + parame.e()
            + "&regKey="
            + URLEncoder.encode(parame.f(), "UTF-8")
            + "&firmwareSignature="
            + URLEncoder.encode(parame.h(), "UTF-8")
            + "&langCode="
            + parame.i();
    try {
      ...
      String str = null;
      while ((str = bufferedReader.readLine()) != null) str1 = str1 + str;
      ...
      for (File file1 : arrayOfFile) {
        ...
        if (!file2.exists() || file2.delete()) {
          ...
          D.d(
              "Lock on file: "
                  + file2.getName()
                  + " will leave a temp version to be renamed on restart.");
          File file3 = new File(".", file1.getName() + ".temp");
          try {
            t.a(file1, file3);
            file1.delete();
  ...
  public void a(File paramFile, d paramd) {
    ...
    while (iterator.hasNext()) {
      ...
      if (c.a().equals("fileUpdate")) {
        ...
        String str = null;
        if (arrayOfString.length > 2) str = arrayOfString[2];
  ...
  public void a(File paramFile, String paramString1, String paramString2, String paramString3) {
    ...
    do {
      ...
      try {
        ...
        if (paramString3 != null
            && !paramString3.isEmpty()
            && file1.exists()
            && file1.length() > 1000000L
            && file1.length() < l
            && b2 < 3) {
        ...
        if (!this.f.contains(file1)) this.f.add(file1);
        ...
        if (paramString3 != null && !paramString3.isEmpty()) {
          ...
          if (!paramString3.toLowerCase().equals(str.toLowerCase())) {
            if (b2 > b1) {
              D.b(
                  "Download of: "
                      + paramString1
                      + ", file size failed md5 checksum! server reported: "
                      + paramString3.toLowerCase()
                      + ", calculated: "
                      + str.toLowerCase()
                      + ", retrying");
              throw new IOException(
                  "Download file " + paramString1 + " after " + b1 + " attempts!");
            }
            this.d -= file1.length();
            D.b("Download of: " + paramString1 + ", file size failed md5 checksum! retrying");
            if (file1.length() >= l) file1.delete();
          } else {
            bool = true;
          }
          ...
          D.b(
              "Download of: "
                  + paramString1
                  + ", file size not as expected! Expected: "
                  + l
                  + " bytes, got: "
                  + file1.length()
                  + ", retrying");
      ...
      if (file1 != null && l == file1.length()) continue;
    ...
    if (file2.getName().toLowerCase().endsWith(".zip")) this.f.add(file2);
    if (file2.exists() && !file2.delete())
