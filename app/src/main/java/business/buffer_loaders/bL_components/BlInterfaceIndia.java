...
public class BlInterfaceIndia {
  ...
  public s a(long paramLong) {
    ...
    for (byte b = 0; b < this.a.length; b++) {
      if (this.a[b].c() < paramLong)
        return (Math.abs(this.a[b].c() - paramLong) > Math.abs(l - paramLong))
            ? this.a[b - 1]
            : this.a[b];
      l = this.a[b].c();
    }
    D.c(
        "timestamp is older than we have returning the oldest table I have. timestamp="
            + paramLong
            + ", lastTimestamp="
            + l);
  ...
  public void a(s params, long paramLong) {
    ...
    for (int k = this.a.length - 1; k > 0; k--) this.a[k] = this.a[k - 1];
