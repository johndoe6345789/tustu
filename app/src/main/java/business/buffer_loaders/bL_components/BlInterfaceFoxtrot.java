...
public class BlInterfaceFoxtrot extends k {
  ...
  public boolean a(GInterfaceAi paramaI, byte[] paramArrayOfbyte) {
    ...
    try {
      ...
      if (Manager != null) d2 = Manager.j(paramaI.h());
      if (d1 < 2.0D) d1 *= d2;
      double d3 = 0.843D * d2;
      double d4 = d2 * 1.156D;
      if (d1 > d3 && d1 < d4) {
        if (d1 <= this.b) this.b = d1;
        if (d1 >= this.c) this.c = d1;
        return false;
      }
      if (d1 < this.b) {
        this.b = d1;
      ...
      if (bool) this.d = System.currentTimeMillis();
    } catch (ExceptionPrintstacktrace ExceptionPrintstacktrace) {
      D.b("unable to do simple compare for channels: " + b());
      return false;
    }
    if (!bool) bool = (System.currentTimeMillis() - this.e < this.d) ? true : false;
    if (bool) this.a++;
    return bool;
  }
