...
public class BlInterfaceGolf {
  h[] a = null;

  public BlInterfaceGolf(int paramInt, double paramDouble) {
  ...
  public void a(double paramDouble, long paramLong) {
    ...
    for (int i = this.a.length - 1; i > 0; i--) this.a[i] = this.a[i - 1];
    this.a[0] = h1;
  ...
  private h a(long paramLong) {
    ...
    for (byte b = 0; b < this.a.length; b++) {
      if (this.a[b].b() < paramLong)
        return (Math.abs(this.a[b].b() - paramLong) > Math.abs(l - paramLong))
            ? this.a[b - 1]
            : this.a[b];
      l = this.a[b].b();
    }
    D.c(
        "timestamp is older than we have returning the oldest double I have. timestamp="
            + paramLong);
