...
public class UIInBrPackage extends JPanel {
  ...
  private void a(JButton paramJButton) {
    ...
    if (TostringInBlPackage.e() == 128) {
      ...
      str =
          bV.a(
              "{" + SComponentGolf.b("Custom filter condition") + "}",
              false,
              SComponentGolf.b("Performs custom filter as evaluated by " + r.a.b + ".")
                  + "\n"
                  + SComponentGolf.b("See website for more details.")
                  + " \nex. (coolant > 200) && rpm > 5000 \n"
                  + SComponentGolf.b(
                      "filters any record where coolant temp is greater than 200 and rpm is greater"
                          + " than 500.")
                  + "\n"
                  + SComponentGolf.b(
