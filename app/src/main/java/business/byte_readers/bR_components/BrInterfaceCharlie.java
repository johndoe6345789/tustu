    ...
    if (i != this.f.c())
      D.b(
          "Checksum Error: Expected: 0x"
              ...
              + ", found: 0x"
              + Integer.toHexString(this.f.c()).toUpperCase());
    return (i == this.f.c());
  }
  ...
  public int g() {
    ...
    if (this.a.f() == 1)
      try {
        ...
        int i = 0;
        byte b1;
        for (b1 = 0; b1 < arrayOfByte1.length; b1++) i += bH.c.a(arrayOfByte1[b1]);
        for (b1 = 0; b1 < arrayOfByte2.length; b1++) i += bH.c.a(arrayOfByte2[b1]);
        for (b1 = 0; b1 < arrayOfByte3.length; b1++) i += bH.c.a(arrayOfByte3[b1]);
        i %= 256;
    ...
    D.b(
        "Unsupported CRC Size: "
            + this.a.f()
            + " skipping check, currently only 1 and 4 are supported.");
    return -1;
  }
