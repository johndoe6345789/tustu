...
public class BmInterfaceBravo {
  ...
  public double a(int paramInt) {
    if (paramInt >= b() && paramInt <= a()) return c(paramInt).c();
    if (paramInt > a()) {
  ...
  public double b(int paramInt) {
    if (this.b.size() < 2) return 0.0D;
    int i = f(paramInt);
    int j = i;
    int k = 0;
    int m = 0;
    if (i != -1) {
      while (i > 0 && ((c) this.b.get(i - 1)).d() > paramInt - 5) i--;
      while (j < this.b.size() - 1 && ((c) this.b.get(j + 1)).d() < paramInt + 5) j++;
    } else if (paramInt < b()) {
      i = f(b());
