...
public class ae {
    ...
    try {
      ...
      while (enumeration2.hasMoreElements()) {
        ...
        if (zipEntry.getName().contains(".")) {
          str2 =
              zipEntry.getName().substring(zipEntry.getName().lastIndexOf(".") + 1).toLowerCase();
        } else {
          str2 = "";
        }
        if (str1.length() > 4)
  ...
  private static List a(String paramString) {
    ...
    if (paramString != null && !paramString.isEmpty())
      for (String str : paramString.split(";")) arrayList.add(str.toLowerCase());
    return arrayList;
  }
