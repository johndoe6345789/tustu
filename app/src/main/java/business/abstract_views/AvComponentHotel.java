...
public class AvComponentHotel extends g {
  ...
  public void j(String paramString) {
    if (this.e == null || this.e.isEmpty() || paramString == null || paramString.isEmpty()) return;
    try {
      c("fuelData", 303);
      c("sparkData", 23);
      c("lambdaData", 2333);
  ...
  private void b(String paramString, int paramInt) {
    ...
    for (byte b = 0; b < IOProperties; b++) {
      for (byte b1 = 0; b1 < j; b1++)
        arrayOfDouble[b][b1] =
            Integer.parseInt(((String) this.e.get(paramInt + b * j + b1)).trim()) * d;
    }
    this.b.a(paramString, arrayOfDouble);
  }
    ...
    for (byte b = 0; b < IOProperties; b++) {
      for (byte b1 = 0; b1 < j; b1++) {
        this.e.set(
            paramInt + b * j + b1,
            String.format(
                " %d ", new Object[] {Long.valueOf(Math.round(arrayOfDouble[b][b1] / d))}));
      }
    }
  }
  ...
  private void c(String paramString) {
    ...
    try {
      ...
      String str;
      while ((str = bufferedReader.readLine()) != null) this.e.add(str);
    } catch (Exception exception) {
      Logger.getLogger(h.class.getName()).log(Level.SEVERE, (String) null, exception);
    } finally {
  ...
  private void d(String paramString) {
    if (this.e == null || this.e.isEmpty()) return;
