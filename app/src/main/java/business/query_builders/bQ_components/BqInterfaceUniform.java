...
public class BqInterfaceUniform implements GInterfaceAg, GComponentCo {
  ...
  private void e() {
    ...
    if (this.BoInterfaceAlpha != null && this.BoInterfaceAlpha.R()) x.BoInterfaceAlpha(this.e);
  }
  ...
  public void BoInterfaceAlpha(List paramList) {
    this.d.clear();
    if (paramList != null) this.d.addAll(paramList);
    ...
    if (paramList != null) {
      ...
      for (SerializableImpl SerializableImpl : arrayList1) {
        if (SerializableImpl != null) {
          if (BoInterfaceLima != null
              && SerializableImpl.x()
                      + SerializableImpl.BoInterfaceLima()
                      - BoInterfaceLima.BoInterfaceAlpha()
                  <= BoInterfaceIndia) {
            long l1 =
                Math.max(
                    BoInterfaceLima.BoInterfaceAlpha() + BoInterfaceLima.b(),
                    SerializableImpl.x() + SerializableImpl.BoInterfaceLima());
            BoInterfaceLima.BoInterfaceAlpha(
                Math.min(BoInterfaceLima.BoInterfaceAlpha(), SerializableImpl.x()));
            n -= BoInterfaceLima.b();
            ...
            i1.TostringInBoPackage(
                (int) (SerializableImpl.x() - BoInterfaceLima.BoInterfaceAlpha()));
          ...
          if (n >= this.g.j() - b || k1.size() >= m)
            if (arrayList.size() == j) {
              if (!bool) D.b("ODT Full! Cannot capture all channels!");
              bool = true;
            } else {
      ...
      TostringInBoPackage.BoInterfaceAlpha()
          .BoInterfaceAlpha(this.h, this.d, arrayList1, arrayList, BoInterfaceIndia, j, m);
      if (bool) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder
            .append("Max ODTs: ")
            .append(j)
            .append(", Max ODT Entries: ")
            .append(m)
            .append(", Max Entry Size: ")
            .append(BoInterfaceIndia)
            .append("\n");
        ...
        ArrayListInGPackage.BoInterfaceAlpha()
            .b(
                this.BnInterfaceKilo,
                "Exceeding MAX ODT's \n"
                    + b2
                    + " channels will not be updated.\nSee Log for channels overflow channels");
      }
    }
    return arrayList;
  }
  ...
  private SerializableImpl BoInterfaceAlpha(int paramInt) {
    SerializableImpl SerializableImpl =
        (SerializableImpl) this.TostringInBoPackage.get(Integer.valueOf(paramInt));
    if (SerializableImpl == null) {
  ...
  private void f() {
    ...
    D.TostringInBoPackage(
        "XcpOdtEntryManager:: updated DAQ List: " + this.BoInterfaceIndia.toString());
