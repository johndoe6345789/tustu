...
class EncodedDataLoader implements GInterfaceBq {
  ...
  public IOInGPackage a(String paramString1, String paramString2) {
    if (this.b == null) throw new IOException("Cache Dir not set! Cannot load Encoded Data.");
    ...
    if (IOInGPackage != null) {
      ...
      if (!str.equalsIgnoreCase(paramString2)) {
        D.a(
            "Data Checksum does not match the ini file, caching will not work!!!!!!!! INI MD5: "
                + paramString2
                + ", calc md5: "
                + str);
        paramString2 = str;
