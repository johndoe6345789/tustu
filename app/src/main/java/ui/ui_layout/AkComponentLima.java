package ak;

import W.a;
import W.c;
import bH.T;

public class AkComponentLima {
  public static String a = "Record Number";

  public static String b = "Time";

  public static String c = "Engine Speed";

  public static String d = "Vehicle Speed";

  public static String e = "MAP";

  public static String f = "Engine Temp";

  public static String g = "Intake Air Temp";

  public static String h = "TPS";

  public static String i = "IAC Position";

  public static String j = "Battery Volts";

  public static String k = "AFR Desired";

  public static String l = "Spark Adv Front";

  public static String m = "Spark Adv Rear";

  public static String n = "VE Front";

  public static String o = "VE Rear";

  public static String p = "VE New Front";

  public static String q = "VE New Rear";

  public static String r = "Front AFR";

  public static String s = "Rear AFR";

  public static String t = "Accel Enrich";

  public static String u = "Decel Enrich";

  public static String v = "Throttle Volt";

  public static String w = "Injector BPW Front";

  public static String x = "Injector BPW Rear";

  public static String y = "IAT Volt";

  public static String z = "Desired Idle";

  public static String A = "Warm Up AFR";

  public static String B = "MAP Read 2";

  public static String C = "MAP Load";

  public static String D = "MAP Volt";

  public static String E = "MAP Default";

  public static String F = "Factory 1";

  public static String G = "Factory 2";

  public static String H = "Factory 3";

  public static String I = "ECT Volts";

  public static String J = "Barometer";

  public static String K = "Knock Retard Front";

  public static String L = "Knock Retard Rear";

  public static String M = "O2 Sensor Front";

  public static String N = "O2 Sensor Rear";

  public static String O = "O2 integrator Front";

  public static String P = "O2 integrator Rear";

  public static String Q = "TGS Volts 1";

  public static String R = "TGS Volts 2";

  public static String S = "TGS";

  public static String T = "CC Target Speed";

  public static c a(c paramc, int paramInt) {
    paramc.a(c(a));
    if (paramInt >= 25) paramc.a(c(b));
    if (paramInt >= 32) paramc.a(c(c));
    if (paramInt >= 72) paramc.a(c(d));
    if (paramInt >= 47) paramc.a(c(e));
    if (paramInt >= 37) paramc.a(c(f));
    if (paramInt >= 38) paramc.a(c(g));
    if (paramInt >= 36) paramc.a(c(h));
    if (paramInt >= 66) paramc.a(c(i));
    if (paramInt >= 34) paramc.a(c(j));
    if (paramInt >= 43) paramc.a(c(k));
    if (paramInt >= 58) paramc.a(c(l));
    if (paramInt >= 60) paramc.a(c(m));
    if (paramInt >= 61) paramc.a(c(n));
    if (paramInt >= 62) paramc.a(c(o));
    if (paramInt >= 63) paramc.a(c(p));
    if (paramInt >= 64) paramc.a(c(q));
    if (paramInt >= 44) paramc.a(c(r));
    if (paramInt >= 45) paramc.a(c(s));
    if (paramInt >= 50) paramc.a(c(t));
    if (paramInt >= 49) paramc.a(c(u));
    if (paramInt >= 42) paramc.a(c(v));
    if (paramInt >= 52) paramc.a(c(w));
    if (paramInt >= 54) paramc.a(c(x));
    if (paramInt >= 40) paramc.a(c(y));
    if (paramInt >= 33) paramc.a(c(z));
    if (paramInt >= 65) paramc.a(c(A));
    if (paramInt >= 70) paramc.a(c(J));
    if (paramInt >= 71) paramc.a(c(G));
    if (paramInt >= 48) paramc.a(c(B));
    if (paramInt >= 35) paramc.a(c(C));
    if (paramInt >= 41) paramc.a(c(D));
    if (paramInt >= 77) paramc.a(c(K));
    if (paramInt >= 78) paramc.a(c(L));
    if (paramInt >= 79) paramc.a(c(M));
    if (paramInt >= 80) paramc.a(c(N));
    if (paramInt >= 81) paramc.a(c(O));
    if (paramInt >= 82) paramc.a(c(P));
    if (paramInt >= 93) paramc.a(c(Q));
    if (paramInt >= 94) paramc.a(c(R));
    if (paramInt >= 90) paramc.a(c(S));
    if (paramInt >= 1000) paramc.a(c(T));
    return paramc;
  }

  public static a a(String paramString) {
    a a = null;
    if (paramString.equals(d)) {
      a = new a();
      a.a(d);
      a.c(1);
      a.b(36);
      a.a(1);
      a.d(1);
      a.b(0.623D);
      a.a(0.0D);
      return a;
    }
    if (paramString.equals(l)) {
      a = new a();
      a.a(l);
      a.c(1);
      a.b(46);
      a.a(1);
      a.d(2);
      a.b(0.25D);
      a.a(0.0D);
      a.b(T.a() + "");
      return a;
    }
    if (paramString.equals(m)) {
      a = new a();
      a.a(m);
      a.c(1);
      a.b(48);
      a.a(1);
      a.d(2);
      a.b(0.25D);
      a.a(0.0D);
      a.b(T.a() + "");
      return a;
    }
    if (paramString.equals(J)) {
      a = new a();
      a.a(J);
      a.c(1);
      a.b(34);
      a.a(0);
      a.d(1);
      a.b(0.3691D);
      a.a(10.57D);
      a.b("kPa");
      return a;
    }
    return c(paramString);
  }

  public static a b(String paramString) {
    a a = null;
    if (paramString.equals(l)) {
      a = c(paramString);
      a.b(0.5D);
      a.b(37);
      return a;
    }
    if (paramString.equals(m)) {
      a = c(paramString);
      a.b(0.5D);
      a.b(38);
      return a;
    }
    if (paramString.equals(K)) {
      a = c(paramString);
      a.b(39);
      return a;
    }
    if (paramString.equals(L)) {
      a = c(paramString);
      a.b(40);
      return a;
    }
    if (paramString.equals(f)) {
      a = c(paramString);
      a.b(45);
      return a;
    }
    if (paramString.equals(h)) {
      a = c(paramString);
      a.b(36);
      return a;
    }
    if (paramString.equals(g)) {
      a = c(paramString);
      a.b(43);
      return a;
    }
    if (paramString.equals(z)) {
      a = c(paramString);
      a.b(33);
      return a;
    }
    if (paramString.equals(j)) {
      a = c(paramString);
      a.b(34);
      return a;
    }
    if (paramString.equals(e)) {
      a = c(paramString);
      a.b(35);
      return a;
    }
    if (paramString.equals(v)) {
      a = c(paramString);
      a.b(48);
      return a;
    }
    return c(paramString);
  }

  public static a c(String paramString) {
    a a = null;
    if (paramString.equals(a)) {
      a = new a();
      a.a(a);
      a.c(4);
      a.b(0);
      a.a(1);
      a.d(0);
      a.b(1.0D);
      a.a(0.0D);
      return a;
    }
    if (paramString.equals(b)) {
      a = new a();
      a.a(b);
      a.c(4);
      a.b(22);
      a.a(1);
      a.d(3);
      a.b(0.001D);
      a.a(0.0D);
      a.b("s");
      return a;
    }
    if (paramString.equals(c)) {
      a = new a();
      a.a(c);
      a.c(2);
      a.b(31);
      a.a(0);
      a.d(0);
      a.b(1.0D);
      a.a(0.0D);
      return a;
    }
    if (paramString.equals(d)) {
      a = new a();
      a.a(d);
      a.c(1);
      a.b(72);
      a.a(1);
      a.d(1);
      a.b(0.623D);
      a.a(0.0D);
      return a;
    }
    if (paramString.equals(e)) {
      a = new a();
      a.a(e);
      a.c(1);
      a.b(35);
      a.a(0);
      a.d(1);
      a.b(0.3691D);
      a.a(10.41D);
      a.b("kPa");
      return a;
    }
    if (paramString.equals(f)) {
      a = new a();
      a.a(f);
      a.c(1);
      a.b(37);
      a.a(0);
      a.d(1);
      a.b(1.83D);
      a.a(0.0D);
      a.b(T.a() + "F");
      return a;
    }
    if (paramString.equals(g)) {
      a = new a();
      a.a(g);
      a.c(1);
      a.b(38);
      a.a(0);
      a.d(1);
      a.b(1.785D);
      a.a(4.0D);
      a.b(T.a() + "F");
      return a;
    }
    if (paramString.equals(h)) {
      a = new a();
      a.a(h);
      a.c(1);
      a.b(36);
      a.a(0);
      a.d(1);
      a.b(0.45454545D);
      a.a(0.0D);
      a.b("%");
      return a;
    }
    if (paramString.equals(i)) {
      a = new a();
      a.a(i);
      a.c(1);
      a.b(66);
      a.a(0);
      a.d(0);
      a.b(1.0D);
      a.a(0.0D);
      a.b("#");
      return a;
    }
    if (paramString.equals(j)) {
      a = new a();
      a.a(j);
      a.c(1);
      a.b(34);
      a.a(0);
      a.d(1);
      a.b(0.1D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(k)) {
      a = new a();
      a.a(k);
      a.c(1);
      a.b(43);
      a.a(0);
      a.d(1);
      a.b(0.1D);
      a.a(0.0D);
      a.b(":1");
      return a;
    }
    if (paramString.equals(l)) {
      a = new a();
      a.a(l);
      a.c(1);
      a.b(58);
      a.a(1);
      a.d(2);
      a.b(0.25D);
      a.a(0.0D);
      a.b(T.a() + "");
      return a;
    }
    if (paramString.equals(m)) {
      a = new a();
      a.a(m);
      a.c(1);
      a.b(60);
      a.a(1);
      a.d(2);
      a.b(0.25D);
      a.a(0.0D);
      a.b(T.a() + "");
      return a;
    }
    if (paramString.equals(n)) {
      a = new a();
      a.a(n);
      a.c(1);
      a.b(61);
      a.a(0);
      a.d(1);
      a.b(0.5D);
      a.a(0.0D);
      a.b("%");
      return a;
    }
    if (paramString.equals(o)) {
      a = new a();
      a.a(o);
      a.c(1);
      a.b(62);
      a.a(0);
      a.d(1);
      a.b(0.5D);
      a.a(0.0D);
      a.b("%");
      return a;
    }
    if (paramString.equals(p)) {
      a = new a();
      a.a(p);
      a.c(1);
      a.b(63);
      a.a(0);
      a.d(1);
      a.b(0.5D);
      a.a(0.0D);
      a.b("");
      return a;
    }
    if (paramString.equals(q)) {
      a = new a();
      a.a(q);
      a.c(1);
      a.b(64);
      a.a(0);
      a.d(1);
      a.b(0.5D);
      a.a(0.0D);
      a.b("");
      return a;
    }
    if (paramString.equals(r)) {
      a = new a();
      a.a(r);
      a.c(1);
      a.b(44);
      a.a(0);
      a.d(1);
      a.b(0.1D);
      a.a(0.0D);
      a.b(":1");
      return a;
    }
    if (paramString.equals(s)) {
      a = new a();
      a.a(s);
      a.c(1);
      a.b(45);
      a.a(0);
      a.d(1);
      a.b(0.1D);
      a.a(0.0D);
      a.b(":1");
      return a;
    }
    if (paramString.equals(I)) {
      a = new a();
      a.a(I);
      a.c(1);
      a.b(39);
      a.a(0);
      a.d(3);
      a.b(0.0196078431D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(t)) {
      a = new a();
      a.a(t);
      a.c(1);
      a.b(50);
      a.a(0);
      a.d(3);
      a.b(0.00401D);
      a.a(0.0D);
      a.b("ms");
      return a;
    }
    if (paramString.equals(u)) {
      a = new a();
      a.a(u);
      a.c(1);
      a.b(56);
      a.a(0);
      a.d(3);
      a.b(0.00401D);
      a.a(0.0D);
      a.b("ms");
      return a;
    }
    if (paramString.equals(v)) {
      a = new a();
      a.a(v);
      a.c(1);
      a.b(42);
      a.a(0);
      a.d(3);
      a.b(0.0196078431D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(w)) {
      a = new a();
      a.a(w);
      a.c(2);
      a.b(51);
      a.a(0);
      a.d(3);
      a.b(0.004D);
      a.a(0.0D);
      a.b("ms");
      return a;
    }
    if (paramString.equals(x)) {
      a = new a();
      a.a(x);
      a.c(2);
      a.b(53);
      a.a(0);
      a.d(3);
      a.b(0.004D);
      a.a(0.0D);
      a.b("ms");
      return a;
    }
    if (paramString.equals(y)) {
      a = new a();
      a.a(y);
      a.c(1);
      a.b(40);
      a.a(0);
      a.d(3);
      a.b(0.0196078431D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(z)) {
      a = new a();
      a.a(z);
      a.c(1);
      a.b(33);
      a.a(0);
      a.d(0);
      a.b(8.0D);
      a.a(0.0D);
      a.b("RPM");
      return a;
    }
    if (paramString.equals(A)) {
      a = new a();
      a.a(A);
      a.c(1);
      a.b(65);
      a.a(0);
      a.d(1);
      a.b(0.1D);
      a.a(0.0D);
      a.b(":1");
      return a;
    }
    if (paramString.equals(F)) {
      a = new a();
      a.a(F);
      a.c(1);
      a.b(70);
      a.a(0);
      a.d(0);
      a.b(1.0D);
      a.a(0.0D);
      a.e(2);
      a.b("bits");
      return a;
    }
    if (paramString.equals(G)) {
      a = new a();
      a.a(G);
      a.c(1);
      a.b(71);
      a.a(0);
      a.d(0);
      a.b(1.0D);
      a.a(0.0D);
      a.b("bits");
      a.e(2);
      return a;
    }
    if (paramString.equals(H)) {
      a = new a();
      a.a(H);
      a.c(1);
      a.b(72);
      a.a(0);
      a.d(0);
      a.b(1.0D);
      a.a(0.0D);
      a.b("bits");
      a.e(2);
      return a;
    }
    if (paramString.equals(E)) {
      a = new a();
      a.a(E);
      a.c(1);
      a.b(47);
      a.a(0);
      a.d(1);
      a.b(0.3691D);
      a.a(10.41D);
      a.b("kPa");
      return a;
    }
    if (paramString.equals(J)) {
      a = new a();
      a.a(J);
      a.c(1);
      a.b(70);
      a.a(0);
      a.d(1);
      a.b(0.3691D);
      a.a(10.57D);
      a.b("kPa");
      return a;
    }
    if (paramString.equals(B)) {
      a = new a();
      a.a(B);
      a.c(1);
      a.b(48);
      a.a(0);
      a.d(1);
      a.b(0.3691D);
      a.a(10.41D);
      a.b("kPa");
      return a;
    }
    if (paramString.equals(C)) {
      a = new a();
      a.a(C);
      a.c(1);
      a.b(35);
      a.a(0);
      a.d(1);
      a.b(0.3691D);
      a.a(10.41D);
      a.b("kPa");
      return a;
    }
    if (paramString.equals(D)) {
      a = new a();
      a.a(D);
      a.c(1);
      a.b(41);
      a.a(0);
      a.d(3);
      a.b(0.0196078431D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(K)) {
      a = new a();
      a.a(K);
      a.c(1);
      a.b(75);
      a.a(1);
      a.d(1);
      a.b(0.5D);
      a.a(0.0D);
      a.b(T.a() + "");
      return a;
    }
    if (paramString.equals(L)) {
      a = new a();
      a.a(L);
      a.c(1);
      a.b(76);
      a.a(1);
      a.d(1);
      a.b(0.5D);
      a.a(0.0D);
      a.b(T.a() + "");
      return a;
    }
    if (paramString.equals(M)) {
      a = new a();
      a.a(M);
      a.c(1);
      a.b(79);
      a.a(0);
      a.d(3);
      a.b(0.02D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(N)) {
      a = new a();
      a.a(N);
      a.c(1);
      a.b(80);
      a.a(0);
      a.d(3);
      a.b(0.02D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(O)) {
      a = new a();
      a.a(O);
      a.c(1);
      a.b(81);
      a.a(0);
      a.d(1);
      a.b(0.78125D);
      a.a(0.0D);
      a.b("%");
      return a;
    }
    if (paramString.equals(P)) {
      a = new a();
      a.a(P);
      a.c(1);
      a.b(82);
      a.a(0);
      a.d(1);
      a.b(0.78125D);
      a.a(0.0D);
      a.b("%");
      return a;
    }
    if (paramString.equals(Q)) {
      a = new a();
      a.a(Q);
      a.c(1);
      a.b(93);
      a.a(0);
      a.d(3);
      a.b(0.0196078431D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(R)) {
      a = new a();
      a.a(R);
      a.c(1);
      a.b(94);
      a.a(0);
      a.d(3);
      a.b(0.0196078431D);
      a.a(0.0D);
      a.b("v");
      return a;
    }
    if (paramString.equals(S)) {
      a = new a();
      a.a(S);
      a.c(1);
      a.b(90);
      a.a(0);
      a.d(1);
      a.b(0.78125D);
      a.a(0.0D);
      a.b("%");
      return a;
    }
    return a;
  }
}

/* Location:              /home/rewrich/Downloads/TunerStudioMS/TunerStudioMS/!/ak/l.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */
