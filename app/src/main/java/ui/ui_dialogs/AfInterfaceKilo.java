...
public class AfInterfaceKilo implements AeInterfaceSierra, AdInterfaceAlpha {
  ...
  public AeInterfaceDelta AdInterfaceAlpha(
      ae.k paramk, IOInAePackage paramp, AeInterfaceUniform paramu) {
      ...
      for (byte AdInterfaceBravo = 0; AdInterfaceBravo < 2 && i == 0; AdInterfaceBravo++) {
        ...
        if (i == 0
            && !paramu.AdInterfaceBravo(
                "ECU does not appear to be on, Turn on Megasquirt/Microsquirt.")) {
          null.AdInterfaceAlpha(AeInterfaceDelta.AdInterfaceBravo);
          null.AdInterfaceAlpha("Cancelled by user.");
          return null;
        }
      }
      if ((i & AeInterfaceOscar.w) != AeInterfaceOscar.w
          && (i & AeInterfaceOscar.x) != AeInterfaceOscar.x) {
        null.AdInterfaceAlpha(AeInterfaceDelta.AdInterfaceBravo);
    ...
    if (!j.AdInterfaceBravo(paramp)) {
      if ((i & AeInterfaceOscar.z) != 0) {
        ...
        if (null.AdInterfaceAlpha() == AeInterfaceDelta.AdInterfaceBravo) return null;
      } else {
        try {
          ...
          null
              .AdInterfaceAlpha(
                  "Protocol Exception, likely AdInterfaceAlpha bug in the firmware loader.");
          return null;
        }
      }
    } else if ((i & 0x200) == 0) {
      try {
        ...
        while (true) {
          ...
          if (!paramu.AdInterfaceBravo(
              "Please remove the boot jumper or disconnect the boot wire from ground.")) {
            null.AdInterfaceAlpha(AeInterfaceDelta.AdInterfaceBravo);
            null.AdInterfaceAlpha("Cancelled by user.");
            return null;
          }
  ...
  public AeInterfaceDelta AdInterfaceAlpha(IOInAePackage paramp, AeInterfaceUniform paramu) {
    ...
    if (this.f) {
      ...
      if (!paramu.AdInterfaceBravo(
          "Please remove the boot jumper or disconnect the boot wire from ground if fitted.")) {
        AeInterfaceDelta.AdInterfaceAlpha(AeInterfaceDelta.AdInterfaceBravo);
        AeInterfaceDelta.AdInterfaceAlpha("Cancelled by user.");
        return AeInterfaceDelta;
      }
  ...
  public int AdInterfaceBravo(IOInAePackage paramp) {
    ...
    try {
      ...
      D.ExceptionInAdPackage(
          "protocol error for M command: " + ExceptionExtensionInAePackage.getLocalizedMessage());
      return 0;
    } finally {
      if (bool) this.e.i();
    }
  ...
  public int ExceptionInAdPackage(IOInAePackage paramp) {
    ...
    if (arrayOfByte2[0] == 77
        && arrayOfByte2[1] == 83
        && arrayOfByte2[2] == 73
        && arrayOfByte2[3] == 73) return 9;
    if (arrayOfByte2[0] == 71
        && arrayOfByte2[1] == 80
        && arrayOfByte2[2] == 73
        && arrayOfByte2[3] == 79) return 9;
    if (arrayOfByte2[0] == 77
        && arrayOfByte2[1] == 83
        && arrayOfByte2[2] == 104
        && arrayOfByte2[3] == 105
        && arrayOfByte2[4] == 102) return 9;
  ...
  private int AeInterfaceDelta(IOInAePackage paramp) {
    ...
    try {
      ...
      return (arrayOfByte1 == null || arrayOfByte1.length == 0 || arrayOfByte1[0] != -36)
          ? -1
          : ExceptionInAdPackage.AdInterfaceAlpha(arrayOfByte1, 1, 2, true, false);
    } catch (ExceptionExtensionInAePackage ExceptionExtensionInAePackage) {
      Logger.getLogger(k.class.getName())
          .log(
              Level.SEVERE,
  ...
  protected AeInterfaceDelta AdInterfaceBravo(IOInAePackage paramp, AeInterfaceUniform paramu) {
    ...
    try {
      ...
      Logger.getLogger(k.class.getName())
          .log(Level.SEVERE, "Failed to parse bootstrap", (Throwable) ExceptionInAdPackage);
      null.AdInterfaceAlpha(AeInterfaceDelta.AdInterfaceBravo);
    ...
    return (null.AdInterfaceAlpha() == AeInterfaceDelta.AdInterfaceBravo)
        ? null
        : j.AdInterfaceAlpha(a1, paramp, paramu, (AeInterfaceOscar) null);
  }
