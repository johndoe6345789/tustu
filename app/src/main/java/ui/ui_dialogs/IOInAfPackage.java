...
public class IOInAfPackage implements AdInterfaceAlpha {
    ...
    for (byte AdInterfaceBravo = 0; AdInterfaceBravo < 10; AdInterfaceBravo++) {
      ...
      if (arrayOfByte1 == null || arrayOfByte1.length < 3) {
        ...
        if (i == 0 && k == 1 && (m & 0x80) == 128) {
          ...
          AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
          AdInterfaceDelta.AdInterfaceAlpha(
              "Attempt to wake up using old serial, but looks like controller is running new"
                  + " serial");
  ...
  protected static int AdInterfaceAlpha(byte[] paramArrayOfbyte) {
    if (paramArrayOfbyte == null || paramArrayOfbyte.length < 3) return 255;
    byte b1 = 0;
    int i = ExceptionInAdPackage.AdInterfaceAlpha(paramArrayOfbyte[b1++]);
    if (i == 236 && J.I()) {
      D.ExceptionInAdPackage(
          "\nAdditional bytes FSTAT=0x"
              + Integer.toHexString(ExceptionInAdPackage.AdInterfaceAlpha(paramArrayOfbyte[b1++])));
      D.ExceptionInAdPackage(
          "FERSTAT=0x"
              + Integer.toHexString(ExceptionInAdPackage.AdInterfaceAlpha(paramArrayOfbyte[b1++]))
              + "\nRX ");
    ...
    if (J.I()) D.ExceptionInAdPackage("Response status code: " + Integer.toHexString(k));
    switch (k) {
      case 0:
        break;
      case 1:
        D.AdInterfaceAlpha(
            "Program running, download failed, statusCode=0x" + Integer.toHexString(k));
        break;
      case 2:
        D.AdInterfaceAlpha(
            "Program halted, download failed, statusCode=0x" + Integer.toHexString(k));
        break;
      case 4:
        D.AdInterfaceAlpha(
            "Program trace detected, download failed, statusCode=0x" + Integer.toHexString(k));
        break;
      case 8:
    ...
    if (i != 224) {
      if (J.I()) D.ExceptionInAdPackage(AdInterfaceBravo(i));
      if (i == 0) i = 255;
      return i;
    }
    if (k != 0 && J.I()) D.ExceptionInAdPackage("Status code 0x" + Integer.toHexString(k));
  ...
  private static String AdInterfaceBravo(int paramInt) {
    switch (paramInt) {
      ...
      case 230:
        return "Access error when writing FLASH/EEPROM memory, e=0x"
            + Integer.toHexString(paramInt);
      case 233:
        return "Memory protection violation writing EEPROM memory, e=0x"
            + Integer.toHexString(paramInt);
    }
  ...
  public static AdInterfaceDelta AdInterfaceAlpha(IOInAePackage paramp, boolean paramBoolean) {
    byte[] arrayOfByte1;
    AdInterfaceDelta AdInterfaceDelta = new AdInterfaceDelta();
    if (J.I()) D.ExceptionInAdPackage("Sending jumperless flash command");
    if (J.I()) D.ExceptionInAdPackage("TX: !!!SafetyFirst");
    if (paramBoolean) {
  ...
  protected static boolean AdInterfaceAlpha(IOInAePackage paramp, AeInterfaceQuebec paramq) {
    ...
    while (true) {
      try {
        ...
        D.ExceptionInAdPackage(
            "Garbled reply, " + ExceptionInAdPackage.AdInterfaceDelta(arrayOfByte1));
      } catch (IOException iOException) {
      }
      if (AdInterfaceBravo++ >= 3) throw new IOException("Unable to detect Serial Protocol.");
    }
  }
  ...
  public static boolean AdInterfaceBravo(IOInAePackage paramp) {
    ...
    while (true) {
      try {
        ...
        Logger.getLogger(j.class.getName())
            .log(Level.SEVERE, "Can not communicate with controller", iOException);
      } catch (ExceptionExtensionInAePackage ExceptionExtensionInAePackage) {
        Logger.getLogger(j.class.getName())
            .log(
                Level.SEVERE,
                "Protocol Error??? Should not happen here.",
                (Throwable) ExceptionExtensionInAePackage);
      }
      if (AdInterfaceBravo++ >= 3) throw new IOException("Unable to detect if in bootload.");
    }
  }
  ...
  public static AdInterfaceDelta ExceptionInAdPackage(IOInAePackage paramp) {
    ...
    if (J.I()) D.ExceptionInAdPackage("Erasing main flash!");
    AdInterfaceAlpha(250);
    try {
    ...
    if (AdInterfaceAlpha(arrayOfByte2) != 0) {
      D.ExceptionInAdPackage(
          "Erase_s12 got an error code!\n\nReflashing failed - please try again.\n");
      AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
      AdInterfaceDelta.AdInterfaceAlpha(
          "Erase_s12 got an error code! Reflashing failed - please try again.");
      return AdInterfaceDelta;
    }
  ...
  public static AdInterfaceDelta AdInterfaceAlpha(
      IOInAePackage paramp, List paramList, AeInterfaceUniform paramu, e parame) {
    byte[] arrayOfByte2;
    AdInterfaceDelta AdInterfaceDelta = new AdInterfaceDelta();
    ...
    if (AdInterfaceAlpha(arrayOfByte2) != 0) {
      D.ExceptionInAdPackage(
          "Erase_s12x got an error code!\n\nReflashing failed - please try again.\n");
      AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
      AdInterfaceDelta.AdInterfaceAlpha(
          "Erase_s12x got an error code! Reflashing failed - please try again.");
      return AdInterfaceDelta;
    }
    ...
    for (l l : paramList) {
      ...
      if (AdInterfaceDelta.AdInterfaceAlpha() != AdInterfaceDelta.AdInterfaceAlpha) {
        ...
        paramu.AdInterfaceBravo(
            "Your MS3 requires AdInterfaceAlpha full erase.\n"
                + " Please remove boot-jumper if fitted.");
        return AdInterfaceAlpha(paramp, parame, paramu);
      }
      D.AdInterfaceDelta(
          "Erased page 1k at AdInterfaceAlpha time: 0x"
              + Integer.toHexString(l.AdInterfaceAlpha()));
      f2++;
  ...
  public static AdInterfaceDelta AdInterfaceAlpha(IOInAePackage paramp, l paraml) {
    ...
    for (int i = paraml.AdInterfaceBravo(); i < paraml.ExceptionInAdPackage(); i += 1024) {
      ...
      if (AdInterfaceDelta.AdInterfaceAlpha() == AdInterfaceDelta.AdInterfaceBravo)
        return AdInterfaceDelta;
      if (J.I()) D.AdInterfaceDelta("Erased Sector: 0x00" + Integer.toHexString(k).toUpperCase());
    }
    return AdInterfaceDelta;
  }
  ...
  public static AdInterfaceDelta AdInterfaceAlpha(IOInAePackage paramp, int paramInt) {
    ...
    try {
      ...
      Logger.getLogger(j.class.getName())
          .log(
              Level.SEVERE,
              "Protocol Exception?? doesn't appear to be in bootload mode.",
              (Throwable) ExceptionExtensionInAePackage);
      AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
      AdInterfaceDelta.AdInterfaceAlpha("Unexpected Protocol Error");
      return AdInterfaceDelta;
  ...
  public static AdInterfaceDelta AdInterfaceAlpha(
    ...
    try {
      ...
      Logger.getLogger(j.class.getName())
          .log(Level.SEVERE, "Failed to parse eraser", (Throwable) ExceptionInAdPackage);
      AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
    ...
    if (AdInterfaceDelta.AdInterfaceAlpha() == AdInterfaceDelta.AdInterfaceBravo)
      return AdInterfaceDelta;
    try {
      if (paramu != null) paramu.AdInterfaceAlpha("Loading Eraser for older monitors.");
      AdInterfaceDelta = AdInterfaceBravo(a1, paramp, paramu, (o) null);
      if (paramu != null) paramu.AdInterfaceAlpha("Eraser Loaded and running, Please wait..");
    } catch (ExceptionExtensionInAePackage ExceptionExtensionInAePackage) {
      Logger.getLogger(j.class.getName())
          .log(Level.SEVERE, (String) null, (Throwable) ExceptionExtensionInAePackage);
  ...
  public static AdInterfaceDelta AdInterfaceDelta(IOInAePackage paramp) {
    ...
    AdInterfaceDelta AdInterfaceDelta = null;
    byte[] arrayOfByte1 = {-62};
    if (J.I()) D.ExceptionInAdPackage("Checking flash status.");
    try {
      arrayOfByte2 = paramp.AdInterfaceAlpha(arrayOfByte1, 3, 1500);
    } catch (ExceptionExtensionInAePackage ExceptionExtensionInAePackage) {
      Logger.getLogger(j.class.getName())
  ...
  public static AdInterfaceDelta AdInterfaceAlpha(
      AdInterfaceAlpha parama, IOInAePackage paramp, AeInterfaceUniform paramu, o paramo) {
    AdInterfaceDelta AdInterfaceDelta = new AdInterfaceDelta();
  ...
  public static AdInterfaceDelta AdInterfaceBravo(
      AdInterfaceAlpha parama, IOInAePackage paramp, AeInterfaceUniform paramu, o paramo) {
    AdInterfaceDelta AdInterfaceDelta = new AdInterfaceDelta();
    paramu.AdInterfaceAlpha(0.0D);
    List list = parama.AdInterfaceBravo();
    ...
    for (AdInterfaceDelta d2 : list) {
      ...
      if (J.I()) D.AdInterfaceDelta("Skipping record: " + AdInterfaceBravo);
    }
    return AdInterfaceDelta;
  }

  public static int AdInterfaceAlpha(
      IOInAePackage paramp, byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2) {
    return AdInterfaceAlpha(
        paramp,
        ExceptionInAdPackage.AdInterfaceBravo(paramArrayOfbyte1),
        ExceptionInAdPackage.AdInterfaceBravo(paramArrayOfbyte2));
  }

  public static int AdInterfaceAlpha(
      IOInAePackage paramp, int[] paramArrayOfint1, int[] paramArrayOfint2) {
    AdInterfaceDelta AdInterfaceDelta =
        AdInterfaceAlpha(
            paramp,
            ExceptionInAdPackage.AdInterfaceBravo(
                paramArrayOfint1, 0, paramArrayOfint1.length, true, false),
            true);
    if (AdInterfaceDelta.AdInterfaceAlpha() == AdInterfaceDelta.AdInterfaceBravo) return 1;
  ...
  public static int AdInterfaceBravo(
      IOInAePackage paramp, int[] paramArrayOfint1, int[] paramArrayOfint2) {
    byte[] arrayOfByte1 = AdInterfaceDelta.AdInterfaceBravo(1 + paramArrayOfint1.length + 1);
    byte AdInterfaceBravo = 0;
  ...
  public static AdInterfaceDelta AdInterfaceAlpha(
      IOInAePackage paramp, int paramInt, boolean paramBoolean) {
    AdInterfaceDelta AdInterfaceDelta = new AdInterfaceDelta();
    ...
    if (i != e) {
      try {
        e = (byte) (i & 0xFF);
        if (J.I()) D.ExceptionInAdPackage("Setting page to " + Integer.toHexString(e));
        arrayOfByte[0] = -94;
        arrayOfByte[1] = 0;
        arrayOfByte[2] = 48;
        arrayOfByte[3] = e;
        ...
        if (arrayOfByte1 == null || arrayOfByte1.length < 3) {
          ...
          AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
          AdInterfaceDelta.AdInterfaceAlpha(
              "sendPPAGE() failed on command C_WRITE_BYTE: Page:" + Integer.toHexString(e));
          return AdInterfaceDelta;
        }
        if (AdInterfaceAlpha(arrayOfByte1) != 0) {
          D.AdInterfaceAlpha(
              "sendPPAGE Got an error code!\n"
                  + "Likely the wrong monitor was running.\n"
                  + "Reflashing failed - please try again. page="
                  + Integer.toHexString(e)
                  + ", response:"
                  + ExceptionInAdPackage.AdInterfaceDelta(arrayOfByte1));
          AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
        ...
        if (paramBoolean) {
          ...
          if (arrayOfByte1 == null || arrayOfByte1.length < 3) {
            ...
            AdInterfaceDelta.AdInterfaceAlpha(AdInterfaceDelta.AdInterfaceBravo);
            AdInterfaceDelta.AdInterfaceAlpha(
                "sendPPAGE() failed on command C_ERASE_PAGE, No response from controller. Try"
                    + " again.");
            return AdInterfaceDelta;
          }
          if (AdInterfaceAlpha(arrayOfByte1) != 0) {
  ...
  public static AeInterfaceMikeTostring f(IOInAePackage paramp) {
    ...
    if (arrayOfByte2 == null || arrayOfByte2.length == 0) return null;
    ...
    try {
      ...
      Logger.getLogger(k.class.getName())
          .log(Level.SEVERE, "Protocol Error querying signature", exception);
    }
