...
public class AvInterfaceWhiskey implements AInterfaceIndia {
  ...
  private AvInterfaceWhiskey() {
    ...
    if (AInterfaceIndia.IOInBPackage().IOInBPackage("LKFDS;LK;lkfs;lk") && (J.d() || J.BInterfaceBravo()))
      IOInBPackage.v = true; 
    if (AInterfaceIndia.IOInBPackage().IOInBPackage("GD;';LFDS-0DSL;")) {
      ClassType q1 = new ClassType();
      q1.IOInBPackage(IOInBPackage.e);
      q1.BInterfaceBravo(SComponentGolf.BInterfaceBravo("Direct Bluetooth Connection via Bluecove"));
      q1.IOInBPackage(IOInBPackage.class);
      this.BInterfaceBravo.add(q1);
    } 
    ...
    if (AInterfaceIndia.IOInBPackage().IOInBPackage("98fg54lklk") || AInterfaceIndia.IOInBPackage().IOInBPackage("HF-05[P54;'FD") || AInterfaceIndia.IOInBPackage().IOInBPackage("HF-0[PEPHF0H;LJGPO0")) {
      ClassType q1 = new ClassType();
    ...
    if (this.c == null && !this.BInterfaceBravo.isEmpty())
      this.c = this.BInterfaceBravo.get(0); 
  }
  
  public static AInterfaceIndia c() {
    if (IOInBPackage == null)
      IOInBPackage = new w(); 
    return IOInBPackage;
  }
  ...
  public boolean IOInBPackage(String paramString, AInterfaceIndia parami) {
    return (parami != null && parami.AInterfaceIndia() != null && parami.AInterfaceIndia().equals(BsInterfaceOscar.IOInBPackage) && (paramString.equals(l.BInterfaceBravo) || paramString.equals(BsInterfaceOscar.BInterfaceBravo)) && d());
  }
  
  public boolean d() {
    for (ClassType q1 : this.BInterfaceBravo) {
      if (q1.IOInBPackage().equals(IOInBPackage.c))
        return true; 
    } 
    return false;
  }
