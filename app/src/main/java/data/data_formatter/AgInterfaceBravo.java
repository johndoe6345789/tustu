...
public class AgInterfaceBravo {
  ...
  public AgInterfaceBravo(R paramR) {
    this.a = paramR;
  }
    ...
    try {
      ...
      for (byte b1 = 0; b1 < k; b1++) {
        ...
        if (GComponentBb.s()) {
          ...
          if (b1 < k - 1) bufferedWriter.append(",");
        }
      }
      bufferedWriter.newLine();
        ...
        for (byte b3 = 0; b3 < k; b3++) {
          ...
          try {
            if (GComponentBb.b().equals("formula")) {
              ArrayList<SerializableImpl> arrayList =
                  i.a(GComponentBb.v(), (SerializableImpl) GComponentBb);
              GComponentBb bb1 = null;
              for (byte b4 = 0; b4 < arrayList.size(); b4++) {
                if (arrayList.get(b4) instanceof GComponentBb
                    && !((SerializableImpl) arrayList.get(b4)).b().equals("formula")) {
          ...
          if (b3 < k - 1) {
            ...
            stringBuilder.append(",");
            bufferedWriter.append(stringBuilder.toString());
            if (b3 < j - 1) bufferedWriter.newLine();
            bufferedWriter.flush();
          }
        }
      ...
      throw new RemoteAccessException(
          "Error processing Replay Data, check Replay Entry addressing offsets. "
              + indexOutOfBoundsException.getMessage());
    } catch (Exception exception) {
      D.a(exception);
      throw new RemoteAccessException(
          "Error processing Replay Data, check Replay Entrys. " + exception.getMessage());
    } finally {
  ...
  public int[] a(n paramn) {
    ...
    try {
      boolean bool1 = b();
      boolean bool2 = c();
      if (bool2) return c(paramn);
      if (bool1) return b(paramn);
  ...
  public int[] b(n paramn) {
    ...
    try {
      ...
      if (o.a() == 3) throw new RemoteAccessException("Replay Read Failed! " + o.c());
      return o.e();
    } finally {
      this.a.O().e(bool);
      this.a.O().d(j);
  ...
  private void a(String paramString) {
    for (h h : this.b) h.a(paramString);
  }
}
