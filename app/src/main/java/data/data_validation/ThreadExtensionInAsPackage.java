...
class ThreadExtensionInAsPackage extends Thread {
  ...
  public void run() {
    if (IOPropertiesUsingFile.b(this.IOPropertiesUsingFile.SComponentGolf)) {
      ...
      if (this.IOPropertiesUsingFile.SComponentGolf.d) {
        bV.IOPropertiesUsingFile(
            jPanel,
            ThreadedJFrameIsoptimizeddrawingenabled.IOPropertiesUsingFile().c(),
            SComponentGolf.b("Attention") + "!",
            d);
      } else {
        bV.IOPropertiesUsingFile(
            jPanel,
            ThreadedJFrameIsoptimizeddrawingenabled.IOPropertiesUsingFile().c(),
            SComponentGolf.b("Error") + "!",
            d);
      }
    }
    try {
      ...
      Logger.getLogger(IOPropertiesUsingFile.class.getName())
          .log(Level.SEVERE, (String) null, interruptedException);
    }
    String str =
        SComponentGolf.b("Settings Configuration Error!")
            + "\n"
    ...
    this.IOPropertiesUsingFile.f =
        ThreadedJFrameIsoptimizeddrawingenabled.IOPropertiesUsingFile().b();
