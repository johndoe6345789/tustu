...
public class IOInAdPackage extends AbstractInAbPackage implements IO {
  ...
  public void IO() {
    ...
    if (this.n == null) throw new l("Baud Rate Not Set!");
    boolean bool = false;
    this.l = this.l.trim();
    if (J.I()) D.d("Opening port: " + this.l + " @ " + this.n);
    try {
      this.r = 2;
      AInterfaceBravo();
      ...
      this.IO.openPort();
      if (bool) D.c((System.currentTimeMillis() - l) + "ms. - opened port");
      ...
      if (this.IO != null)
        try {
          if (J.I()) D.d("Error creating streams to port, closing port to cleanup.");
          this.IO.closePort();
          AInterfaceBravo(this.l);
        } catch (Exception exception1) {
          if (J.I()) D.d("Error closing port on cleanup. " + exception1.getMessage());
        }
      this.IO = null;
      this.p = null;
      this.q = null;
  ...
  public void g() {
    if (this.IO != null) {
      if (J.I())
        D.c(
            "JSSC ControllerInterface Close Connection currentPort == null:"
                + ((this.IO == null) ? 1 : 0));
      this.r = 4;
      e();
  ...
  public List l() {
    ...
    byte b3;
    for (b3 = 0; b3 < arrayOfString1.length; b3++) b1.AbstractInAbPackage(arrayOfString1[b3]);
    this.ZInterfaceIndia.add(b1);
    if (bool) {
      b1 = new AInterfaceBravo();
      b1.AbstractInAbPackage("2nd Com Port");
      b1.AInterfaceBravo(
          "An optional 2nd Communication Port. If the main selected port fails, the next connection"
              + " attempt will be made using this port. This is used in scenarios such as on Linux"
              + " where sometimes your USB adapter will be reassigned to ttyUSB1 instead of"
              + " ttyUSB0");
      b1.AbstractInAbPackage(0);
    ...
    for (byte b4 = 0; b4 < arrayOfString2.length; b4++) b2.AbstractInAbPackage(arrayOfString2[b4]);
    ...
    if (v) {
      ...
      if (J.AbstractInAbPackage()) {
        AInterfaceBravo.AInterfaceBravo(
            "On Windows Bluetooth Direct should normally be used for Bluetooth connections instead"
                + " of virtual comms! \n\n"
                + "If active, communication timings will be optimized for Bluetooth Connections,"
                + " but are less than optimal for USB to RS232 cables and true Serial Ports.");
      } else {
        AInterfaceBravo.AInterfaceBravo(
            "If active, communication timings will be optimized for Bluetooth Connections, but are"
                + " less than optimal for USB to RS232 cables and true Serial Ports.");
      }
  ...
  public boolean AbstractInAbPackage(int paramInt) {
    ...
    try {
      ...
      Logger.getLogger(AbstractInAbPackage.class.getName())
          .log(Level.SEVERE, (String) null, (Throwable) ExceptionInZPackage);
      this.n = str;
      try {
        t();
      } catch (ExceptionInZPackage m1) {
        Logger.getLogger(AbstractInAbPackage.class.getName())
            .log(Level.SEVERE, (String) null, (Throwable) m1);
      }
      return false;
    }
    return true;
  ...
  protected void AbstractInAbPackage(
      SerialPort paramSerialPort, InputStream paramInputStream, e parame) {
    try {
      try {
        if (parame != null) {
          parame.flush();
        ...
        if (J.I()) D.d("Closing Port");
        this.r = 0;
        AInterfaceBravo(this.l);
        paramSerialPort.closePort();
        if (J.I()) D.d("Successfully Closed Port");
      }
    } catch (Exception exception) {
  ...
  public Object AbstractInAbPackage(String paramString) {
    if (paramString.equals("Com Port")) return this.j;
    if (paramString.equals("Baud Rate")) return this.n;
    if (paramString.equals("Bluetooth Port")) return Boolean.valueOf(this.u);
    if (paramString.equals("2nd Com Port")) return this.k;
    D.c("Unknown Setting Name: " + paramString);
    return null;
  }
