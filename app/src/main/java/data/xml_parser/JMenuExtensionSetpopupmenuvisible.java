...
public class JMenuExtensionSetpopupmenuvisible extends JMenu implements BaInterfaceFoxtrot, bs {
      ...
      for (byte b2 = 0; b2 < getItemCount(); b2++) {
        if (getItem(b2) instanceof bs) {
          ...
          if (bool) {
            ...
            boolean bool1 =
                ((bs1.i() == null || bs1.i().a())
                        && (bs1.JMenuItemExtensionInBaPackage() == null
                            || bs1.JMenuItemExtensionInBaPackage().a()))
                    ? true
                    : false;
            bs1.setVisible(bool1);
            if (bs1.JMenuItemExtensionInBaPackage() != null)
        ...
        if (getItem(b2) instanceof JCheckBoxMenuItemExtension) {
          JCheckBoxMenuItemExtension JCheckBoxMenuItemExtension =
              (JCheckBoxMenuItemExtension) getItem(b2);
  ...
  public void BaInterfaceFoxtrot() {
    ...
    for (int i = arrayOfComponent.length - 1;
        i >= 0
            && !(arrayOfComponent[i] instanceof JMenuItem)
            && !(arrayOfComponent[i] instanceof JMenu);
        i--) remove(i);
  }
  ...
  private void j() {
    ...
    while (iterator.hasNext()) ((dr) iterator.next()).a();
  }
  ...
  private void k() {
    ...
    if (getWidth() >= dimension.width
        && (this.b.length() <= 12 || getWidth() > dimension.width * 1.5D)) {
      ...
      for (byte b = 0; b < arrayOfString.length; b++) {
        if ((arrayOfString.length > 1 && b == 0 && arrayOfString[b].length() <= 2)
            || (arrayOfString.length > 2 && b == 0 && arrayOfString[b].length() <= 5)) {
