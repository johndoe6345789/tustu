...
public class AcInterfaceUniform extends h {
  ...
  protected void a(R[] paramArrayOfR, OutputStream paramOutputStream) {
    try {
      ...
      if (str != null) stringBuilder1.append(str).append("\n");
    ...
    if (this.G > 0)
      try {
        ...
        Logger.getLogger(u.class.getName())
            .log(Level.WARNING, "Unable to write Mark to data log", iOException);
      }
  }
  ...
  protected synchronized void a(OutputStream paramOutputStream, byte[][] paramArrayOfbyte) {
    ...
    for (j = 0; j < this.B.size(); j++) {
      ...
      if (Double.isNaN(d1)) b++;
    }
    j = 0;
    for (int k = this.r; k < arrayOfByte1.length - 1; k++) j += arrayOfByte1[k];
    ...
    try {
      ...
      if (arrayOfByte1.length != i + 1) D.b("Record size does not match the defined size!");
    } catch (IOException iOException) {
      Logger.getLogger(u.class.getName())
          .log(Level.SEVERE, "Failed to write log record", iOException);
    } finally {
      ...
      if (t == null || t.c().size() >= t.m() * 8 || t.b() != q.b().e()) {
        try {
          ...
          Logger.getLogger(u.class.getName())
              .log(
                  Level.WARNING,
                  "Unable to create MlvLgBitField",
                  (Throwable) ExceptionPrintstacktrace);
        }
        continue;
      }
