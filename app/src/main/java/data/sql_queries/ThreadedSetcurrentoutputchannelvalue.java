...
public class ThreadedSetcurrentoutputchannelvalue implements S, GComponentCq {
  ...
  private void e() {
    try {
      ManagerUsingArrayList.ExceptionInVPackage()
          .ExceptionInVPackage(this.ExceptionInVPackage.c(), d.ExceptionInVPackage, this);
  ...
  public synchronized G.o ExceptionInVPackage(m paramm, boolean paramBoolean, int paramInt) {
    long l = System.currentTimeMillis();
    if (this.m) ExceptionInVPackage("Instruction Entering " + paramm.aL());
    while (this.i > System.currentTimeMillis()) {
      if (System.currentTimeMillis() - l > this.e)
        throw new RemoteAccessException(
            "Timeout waiting for SD to become Ready while executing " + paramm.aL());
      try {
        Thread.sleep(40L);
        if (this.m) ExceptionInVPackage("under processing hysteresis, sleeping.");
    ...
    if (this.m)
      ExceptionInVPackage(
          "Instruction Released for Processing. "
              + paramm.aL()
              + ", triggerNotReady="
              + paramBoolean);
    if (paramBoolean) {
      ...
      try {
        ...
        if (this.m) ExceptionInVPackage("Instruction About to process. " + paramm.aL());
        this.i = System.currentTimeMillis() + this.j;
        return this.b.ExceptionInVPackage(this.ExceptionInVPackage, paramm, paramInt);
      } finally {
        this.p = false;
        this.c = false;
        if (this.m) ExceptionInVPackage("Instruction Completed. " + paramm.aL());
        this.i = System.currentTimeMillis() + this.j;
      }
    }
    try {
      if (this.m)
        ExceptionInVPackage(
            "Instruction About to process " + paramm.aL() + ", triggerNotReady=" + paramBoolean);
      return this.b.ExceptionInVPackage(this.ExceptionInVPackage, paramm, paramInt);
    } finally {
      if (this.m)
        ExceptionInVPackage(
            "Instruction Completed " + paramm.aL() + ", triggerNotReady=" + paramBoolean);
    }
  }
  ...
  public void setCurrentOutputChannelValue(String paramString, double paramDouble) {
    ...
    if (!this.c && !this.p && bool) {
      ...
      if (this.m) ExceptionInVPackage("SD Status Changed to Not Ready.");
      this.p = false;
    ...
    if (!this.c && this.q && !bool1) {
      ...
      if (this.m) ExceptionInVPackage("Told, SD Status Changed to Logging. ");
      this.q = true;
      b(this.q);
    ...
    if (!this.q && this.h < System.currentTimeMillis()) {
      if (this.m) ExceptionInVPackage("publishing logging status: " + System.currentTimeMillis());
      b(this.q);
      this.h = Long.MAX_VALUE;
    }
  }
  ...
  protected void ExceptionInVPackage(boolean paramBoolean) {
    ...
    while (iterator.hasNext()) ((q) iterator.next()).ExceptionInVPackage(paramBoolean);
  }
  ...
  private void b(boolean paramBoolean) {
    ...
    Iterator<q> iterator = this.d.iterator();
    while (iterator.hasNext()) ((q) iterator.next()).b(paramBoolean);
  }
