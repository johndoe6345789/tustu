...
public class f {
  public static String a() {
    ...
    if (str != null) {
      str = str.trim();
      if (str.length() < 8 || str.contains("123456789") || (str.getBytes()).length != str.length() || !c.a(str.getBytes()) || str.indexOf(" ") > 0)
        str = ""; 
    } 
    if (str.length() > 25)
  ...
  public static String h() {
    try {
      ...
      for (byte b2 = 0; b2 < 4; b2++) {
        ...
        if (file.exists()) {
          process = Runtime.getRuntime().exec("/bin/udevadm info --query=property --name=sd" + (char)(b1 + b2));
        } else if ((new File("/sbin/udevadm")).exists()) {
          process = Runtime.getRuntime().exec("/usr/bin/udevadm info --query=property --name=sd" + (char)(b1 + b2));
        } else {
          process = Runtime.getRuntime().exec("/sbin/udevadm info --query=property --name=sd" + (char)(b1 + b2));
        } 
        InputStream inputStream = process.getInputStream();
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream));
        String str1 = null;
      ...
      try {
        ...
        if (str != null && str.contains("/dev")) {
          if ((new File("/sbin/udevadm")).exists()) {
            process = Runtime.getRuntime().exec("/sbin/udevadm info --query=property --name=" + str);
          } else if ((new File("/bin/udevadm")).exists()) {
            process = Runtime.getRuntime().exec("/bin/udevadm info --query=property --name=" + str);
          } else {
            process = Runtime.getRuntime().exec("/usr/bin/udevadm info --query=property --name=" + str);
          } 
          InputStream inputStream = process.getInputStream();
          BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream));
          String str1 = null;
  ...
  private static boolean a(byte[] paramArrayOfbyte) {
    if (paramArrayOfbyte != null && paramArrayOfbyte.length > 0) {
      ...
      for (byte b2 = 0; b2 < paramArrayOfbyte.length && b2 < 6; b2++) {
        if (paramArrayOfbyte[b2] == 0 && paramArrayOfbyte[b2] == 255)
          b1++; 
        if (b1 > paramArrayOfbyte.length - 3)
          return true; 
      } 
      return false;
    } 
  ...
  public static byte[] l() {
    if (J.d())
      try {
        byte[] arrayOfByte = f();
        if (arrayOfByte != null && arrayOfByte.length > 5)
          return arrayOfByte; 
      } catch (Exception exception) {} 
    try {
