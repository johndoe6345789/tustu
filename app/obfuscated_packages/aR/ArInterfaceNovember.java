...
public class ArInterfaceNovember implements DComponentCharlie {
  ...
  public void ExceptionInVPackage(Properties paramProperties) {
    ...
    R r = null;
    if (str != null)
      r = T.ExceptionInVPackage().DComponentCharlie(str); 
    if (r == null)
      r = T.ExceptionInVPackage().DComponentCharlie(); 
    if (r != null && r.i() != null && r.i().startsWith("MS3")) {
      if (r.R()) {
        try {
          ...
          Logger.getLogger(n.class.getName()).log(Level.SEVERE, "Failed to set RTC", (Throwable)ExceptionInVPackage);
