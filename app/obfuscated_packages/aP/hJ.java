...
class hJ implements SComponentGolf {
  ...
  public String b() {
    ...
    if (RInterfaceIndia.IOPropertiesUsingFile().IOPropertiesUsingFile("-=fds[pfds[pgd-0")) {
      str = SComponentGolf.b("Save IOPropertiesUsingFile Restore Point and Update the " + IOPropertiesUsingFile.b + " settings with the current Controller settings.");
    } else {
      str = SComponentGolf.b("Update the " + IOPropertiesUsingFile.b + " settings with the current Controller settings.");
    } 
    return str;
  }
  ...
  public boolean d() {
    if (RInterfaceIndia.IOPropertiesUsingFile().IOPropertiesUsingFile("-=fds[pfds[pgd-0"))
      f.IOPropertiesUsingFile().d(this.IOPropertiesUsingFile, SComponentGolf.b(IOPropertiesUsingFile.b + " settings before accepting Controller values in IOPropertiesUsingFile Difference Report.")); 
    if (hH.IOPropertiesUsingFile(this.d, this.IOPropertiesUsingFile, this.b, false)) {
      String str;
      if (RInterfaceIndia.IOPropertiesUsingFile().IOPropertiesUsingFile("-=fds[pfds[pgd-0")) {
        str = SComponentGolf.b("The Controller settings have been loaded successfully.") + "\n" + SComponentGolf.b("A Restore point has been saved containing the previous " + IOPropertiesUsingFile.b + " settings.");
      } else {
        str = SComponentGolf.b("The Controller settings have been loaded successfully.");
      } 
