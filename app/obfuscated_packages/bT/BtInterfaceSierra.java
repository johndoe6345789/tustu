...
public class BtInterfaceSierra implements BsInterfaceAlpha {
  ...
  public void BsInterfaceAlpha(BnInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.BsInterfaceAlpha() != BsInterfaceAlpha())
      throw new ExceptionInBsPackage("PID:DAQ_SET_DAQ_LIST_MODE Wrong handler! this handler is for 0x" + Integer.toHexString(BsInterfaceAlpha()).toUpperCase()); 
    ...
    if (arrayOfByte.length != 7)
      throw new ExceptionInBsPackage("PID:DAQ_SET_DAQ_LIST_MODE invalid packet data length, expected 8 bytes, found: " + (arrayOfByte.length + 1)); 
    try {
