...
public class BtInterfaceBravo implements BsInterfaceAlpha {
  ...
  public void BsInterfaceAlpha(BsInterfaceOscar paramo, BnInterfaceTango paramt) {
    if (paramt.BsInterfaceAlpha() != BsInterfaceAlpha())
      throw new ExceptionInBsPackage("PID:STD_SHORT_UPLOAD Wrong handler! this handler is for 0x" + Integer.toHexString(BsInterfaceAlpha()).toUpperCase()); 
    ...
    if (arrayOfByte.length != 7)
      throw new ExceptionInBsPackage("PID:STD_SHORT_UPLOAD invalid packet data length, expected 8 bytes, found: " + (arrayOfByte.length + 1)); 
