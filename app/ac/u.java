  ...
  protected synchronized void a(OutputStream paramOutputStream, byte[][] paramArrayOfbyte) {
    ...
    for (j = 0; j < this.B.size(); j++) {
      ...
      try {
        ...
        if (this.p < 10)
          D.a("problem getting log value for " + w.j() + ": " + g.getMessage()); 
        this.p++;
        i += w.m();
    ...
    for (q q : paramList) {
      if (!q.b().l()) {
        try {
          ...
          Logger.getLogger(u.class.getName()).log(Level.WARNING, "Unable to create Logger Field", (Throwable)g);
        } 
        continue;
      } 
      if (t == null || t.c().size() >= t.m() * 8 || t.b() != q.b().e()) {
        try {
          ...
          Logger.getLogger(u.class.getName()).log(Level.WARNING, "Unable to create MlvLgBitField", (Throwable)g);
        } 
        continue;
      } 
      t.a(q);
    ...
    for (a a : j()) {
      if (a instanceof D) {
        ...
        try {
          ...
          Logger.getLogger(u.class.getName()).log(Level.SEVERE, (String)null, (Throwable)g);
        } 
      } 
    } 
    int i = this.K;
